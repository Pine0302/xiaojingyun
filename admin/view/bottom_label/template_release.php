...
<div class="WSY_content">
    ...
    <div class="WSY_columnbox">
        ...
        <!-- </form> -->
        <!--权限管理代码开始-->
        <form action="/mshop/admin/index.php?m=bottom_label&a=template_release_edit" method="post" >
                            ...
                            <div style="display:none"><!-- 商城系统 -->
                                <ul data-id="0" class="WSY_competenceul shop_system" data-id="a1"  style="border:none;float:left;margin-left: 0px;">
                                  ...
                                  <!--       <?php if($show > 0 || !in_array("mshop_product_detail",$funs)){ ?>
                                    ...
                                    <dl class="scdl">
                                        <input type="checkbox" <?php if($show==1){echo "disabled='disabled'";}?> class="links" <?php if(in_array("weixin_commonshop_diy_template",$funs)) echo "style='display:none'"; ?> name="links2[]" data-id="" value="weixin_commonshop_diy_template" <?php if(in_array("weixin_commonshop_diy_template",$selected_funs2)){echo 'checked';} ?> /><label <?php if(in_array("weixin_commonshop_diy_template",$funs)) echo "style='display:none'"; ?>>自定义模板</label>
                                        <?php
                                            $link = mysql_connect(DB_HOST,DB_USER,DB_PWD);
                                            mysql_select_db(DB_NAME) or die('Could not select database');
                                            _mysql_query("SET NAMES UTF8");
                                            $query="select id,name from weixin_commonshop_diy_template where isvalid=true  and customer_id={$this->customer_id} order by isused desc,is_open desc,id desc";
                                            $result=_mysql_query($query) or die ('query_temid faild' .mysql_error());
                                            while($row=mysql_fetch_assoc($result)){
                                                $diy_temid  = $row['id'];
                                                $name       = $row['name'];
                                                if($show > 0 || !in_array("weixin_commonshop_diy_template_{$diy_temid}",$funs)){ 
                                        ?>
                                                <dd>
                                                    <input type="checkbox" <?php if($show==1){echo "disabled='disabled'";}?> class="links2" name="links2[]" value="weixin_commonshop_diy_template_<?php echo $diy_temid;?>" <?php if(in_array("weixin_commonshop_diy_template_{$diy_temid}",$selected_funs2)){echo 'checked';} ?> /><label><?php echo $name;?></label>
                                                </dd>
                                        <?php }} ?>
                                    </dl>
                                </ul>
...
<!--                                    <dl class="scdl">-->
<!--                                        <input type="checkbox" --><?php //if($show==1){echo "disabled='disabled'";}?><!-- class="links" name="links2[]" value="cityarea_artist" --><?php //if(in_array("cityarea_artist",$selected_funs2)){echo 'checked';} ?><!--  /><label>艺人服务首页</label>-->
<!--                                    </dl>-->
                                </ul>
                            </div>
