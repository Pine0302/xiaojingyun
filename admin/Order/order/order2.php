<?php
    ...
    $proxy_id = i2get("proxy_id",0); //订货系统中的订货商id
    ...
    if($from_page > 0){
        ...
        if($is_shop == 1){
            $query_order .= ' and sso.current_proxy_id='.$proxy_id;
        }
    }
        ...
        <div style="width:100%;display:block;">
			<ul>
         		<li class="WSY_bottonli WSY_bottonli2" style="display:block;float:left;margin-left:20px;">
					...
					<!--<input type="button"  value="新导出方法测试" onClick="export_excel(5);" >-->
					...
					<?php if($from_page < 1){ //F2C系统不要?><a href="http://admin.weisanyun.cn/weixinpl/back_newshops/Order/order/attach/payondelivery.xls" target="_blank" class="payon_btn WSY-skin-bg"  value="回收库存"  >签收模板</a><?php } ?>
					<input type="button"  value="导出数据库" onClick="export_excel(11);" >
				 </li>
			</ul>
		...
		<table width="97%" class="CP_table " id="order_table">
			 ...
			 <tr class="CP_table_chanpina">
				...
				if($o2_isvalid==1){
                    ...
                    if($o_status==0 and $o_paystatus==1 and ($o_sendstatus == 0 or $o_sendstatus == 1)){ ?>

                        <a id="change_courier_number_<?php echo $o_batchcode; ?>" title="修改快递单号" class="change_courier_number"   onclick="change_courier_number('<?php echo $o_batchcode; ?>')" ><img src="../../../common/images_V6.0/operating_icon/icon52.png" /></a>
            	...
            	<div class="WSY_modifydiv order_hide div_show" id="refund_<?php echo $o_batchcode; ?>" >
                    <dl class="order_date" style="min-width: 280px;">
						...
						<dd>
							...
							<b style="width:40px;"><?php echo OOF_T ?></b>
							<span style="font-size:12px;width:100%;color:#f00;">(使用<?php echo $o_paystyle; ?>方式支付了 <?php echo $settlementprice; ?><?php echo OOF_T ?>)</span>
						</dd>
						<dd
                        <?php
						if( $o_paystyle == "找人代付"){
                        ...
                        >
							...
							<?php if($i_cost_integral>0){ ?>
								...
								<span style="font-size:12px;width:100%;color:#f00;">(该订单使用了 <?php echo $i_cost_integral;?>商城积分)</span>
							<?php }?>
						</dd>
                        <dd class="WSY_bottonli con-button"><input  type="button" value="确定" class="good_refund" data-batchcode="<?php echo $o_batchcode;?>" data-money="<?php echo $row_order->return_account;?>" data-paystyle="<?php echo $o_paystyle; ?>"  data-costintegral="<?php echo $i_cost_integral;?>"></dd>
                        <dd class="WSY_bottonli con-button2"><input  type="button" value="取消" onclick="hideDetail()"></dd>
                    </dl>
             </div>
				<!-- 退款 End -->
...
$(".good_refund").click(function(){
	...
	layer.confirm('订单:'+batchcode+'</br>将退款 <b style="color:red">'+parseFloat(total_refundMoney).toFixed(2)+'</b> <?php echo OOF_T ?></br><b style="color:red">'+warning_str+'</b>', {
		btn: ['退款','取消']
	}, function(confirm){
		...
		$.ajax({
			url: "order.class.php",
			type:"POST",
			data:{'batchcode':batchcode,'totalprice':refundMoney,'totalintegral':refundIntegral,'currencyMoney':currencyMoney,'refundSupplyMoney':refundSupplyMoney,'retype':returntype,'op':"goodRefund"},
			dataType:"json",
			success: function(res){
				 layer.close(index_layer);
				if(res.status==0){
...
function KuaiDi100(obj){
	...
	layer.open({
		...
		shade: 0.5,
		area: ['450px', '70%'],
		content: 'http://m.kuaidi100.com/index_all.html?type='+KDName+'&postid='+KDNum+'#result'
	});
...
function export_excel(num){
	...
	if(name=='commonshop_feidou_excel2'){
			var emails = '';
	//url = url_base + 'limit_count/1000/limit_p/-1';
		//console.log(url);
		var op = 'iscount';
