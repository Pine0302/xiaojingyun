...
$selected_funs_arr = i2get("selected_funs","");      //选择发布的栏目
...
if(!empty($selected_funs_arr)){
    ...
    for($i=0;$i<count($selected_funs_arr);$i++){
        /*查找对应的栏目标志*/
        $sql = "select funs from page_column_t where isvalid=true and type=1 and id=".$selected_funs_arr[$i];
        ...
        $publish_id = -1;
        $check_sql = "select id from publish_page_t where customer_id=".$customer_id." and type=1 and page_id=".$selected_funs_arr[$i]." limit 1";
        ...
        if($publish_id >0){
            $update_sql = "update publish_page_t set isvalid = true  where customer_id=".$customer_id." and type=1 and page_id=".$selected_funs_arr[$i];

            _mysql_query($update_sql) or die('update_sql failed: ' .  mysql_error());
        }else{
            $insert_sql2 .= "(".$customer_id.",".(int)$selected_funs_arr[$i].",true,1,'".$funs."',now()),";
        }
    }
    ...
    echo "<script>alert('发布成功');parent.location.href='setting.php?customer_id=".$customer_id_en."';</script>";
}

$query="select id,name,funs from page_column_t where isvalid=true and type=1 and funs<>'customer_service' order by id asc";

$result = _mysql_query($query) or die('Query failed: ' . mysql_error());

/*查找发布了哪些页面*/
$select_page = array();  //已发布页面
$select_page_sql = "select page_id from publish_page_t where isvalid=true and customer_id=".$customer_id." and type=1";
    ...
    <script>
        ...
        function submitV(){
            ...
            if(selected_funs.length == 0){
                ...
                location.href = "navigation_release.php?customer_id=<?php echo $customer_id_en;?>&selected_funs="+selected_funs;
            }
        }
    </script>
<?php
