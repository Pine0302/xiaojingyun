...
if($op=="del"){
	...
	_mysql_query("update weixin_commonshop_userdefined_nav set isvalid=false where id=".$id) or die('QUERY faild_del:'.mysql_error());
	mysql_close($link);
// echo $error; 
...
//导航链接
if($chk_submenu==0){
	if($navigation_id>0 or $navigation_id==-40){	//-40是多级分类	
		...
		if($navigation_id==-40){
				...
				if(!empty($_POST["product_type"])){
					$product_types = $configutil->splash_new($_POST["product_type"]);
				}
				$product_type = explode('_',$product_types);
				$type = $product_type[0];
				$query3="select name from weixin_commonshop_types where isvalid=true and id=".$type;
				$result3 = _mysql_query($query3) or die('Query failed: ' . mysql_error());
				$typename="";
				while ($row3 = mysql_fetch_object($result3)) {
				   $typename = $row3->name;
				}
				$url="../../mshop/list.php?customer_id=".$customer_id_en."&tid=".$type."&tname=".$typename;
				$navigation_id = $type.'_1';
		}else if($linktype==2){
		   //图文
			...
			$url = $website_url;
			$navigation_id = $navigation_id.'_2';
		}		 
	 }else{
	...
	for($i=0;$i<$count;$i++){
		if($type_submenu[$i]>0){		    
			...
			if($linktype==1){				
					...
					$url="../../mshop/list.php?customer_id=".$customer_id_en."&tid=".$type_submenu[$i]."&tname=".$typename;
			}else if($linktype==2){
			   //图文
...
if($keyid>0){
	...
	_mysql_query($query) or die('QUERY faild_1:'.mysql_error());
}
