...
//"[<a href=\"healthpay_detail.php?pay_batchcode=$pay_batchcode&batchcode=$o_batchcode\">". $callBackBatchcode ."(点击查看)</a>]";
...
$sendstatus = 0;
$order_createtime = "";
$query="select status,paystatus,sendstatus,return_status,payother_trade_no,paystyle,createtime from weixin_commonshop_orders where isvalid=true and batchcode='".$batchcode."' group by batchcode";
...
while ($row = mysql_fetch_object($result)) {
	...
	$order_createtime = $row->createtime;
	break;
}
//echo "下单时间：".$createtime."<br/>";
$out_trade_no = $pay_batchcode;
if($paystyle == "找人代付"){
	$out_trade_no = $payother_trade_no;
}
$price = 0; //实际支付
$healthpay = "select price,callBackBatchcode from paycallback_t where isvalid=true and pay_batchcode='".$pay_batchcode."'";
...
while ($row_result_healthpay= mysql_fetch_object($result_healthpay)) {
	$transaction_id = $row_result_healthpay->callBackBatchcode;
	$price = $row_result_healthpay->price;
}
...
//echo $sql;
$result = _mysql_query($sql) or die('Sql in table weixin_commonshop_order_prices failed: ' . mysql_error());
while ($row = mysql_fetch_object($result)) {
...
$result5 = _mysql_query($query5) or die('Query table weixin_commonshop_refunds failed: ' . mysql_error());
	...
	<div class="WSY_columnbox">
		...
		<div class="WSY_remind_main" style="min-height:250px;">
			...
			<dl class="WSY_remind_dl02" style="margin-top:20px;margin-left:100px;">
				<dt style="line-height:20px;" class="WSY_left">健康钱包交易单号：</dt>
				<dd><span class="input">
			...
			<dl class="WSY_remind_dl02" style="margin-top:20px;margin-left:100px;">
				<dt style="line-height:20px;" class="WSY_left">健康钱包支付：</dt>
				<dd><span class="input">
					<?php  echo $price; ?>
				</span></dd>
			</dl>
			...
			<dl class="WSY_remind_dl02" style="margin-top:20px;margin-left:100px;">
				...
				<span class="input">
					...
					<?php if($return_status_str==0){ ?>
						&nbsp;&nbsp;<a href="javascript:showReturn();" style="color:blue" >退款</a><span style="color:red"></span>
					<?php } ?>
			...
			<dl class="WSY_remind_dl02" id="div_return" style="display:none;margin-top:20px;margin-left:100px;">
				...
				<dd style="line-height:40px;">
					<span><input class="not_agent_tip" value="0" name="return_money" id="return_money" style="width:50px;height:24px;text-align:center;border:solid 1px #ccc;border-radius:2px;background-color: #EEEEEE;" type="text" readonly="readonly"><?php echo $currency_text;?></span>
				</dd>
...
function showReturn(){
   ...
   document.getElementById("return_money").value="<?php echo $price; ?>";
}
...
function subReturn(){
   	...
   	document.location = "../../../../wsy_pay/web/healthpay/refund.php?customer_id=<?php echo $customer_id_en; ?>&pay_batchcode=<?php echo $pay_batchcode; ?>&total_fee=<?php echo $price; ?>&batchcode=<?php echo $batchcode; ?>&transaction_id=<?php echo $transaction_id; ?>&order_createtime=<?php echo $order_createtime; ?>";
}
