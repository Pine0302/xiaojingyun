	...
	for( j in province ){
		...
		if( j == 0 ){
			if( province_arr.indexOf(province[j]) >= 0 ){
				html_p = "<div class='cellSelected province_"+j+" selectedProvince' data-p_id='"+j+"'><img class='select' src='"+imgPath+"/select1.png' /> <span>"+province[j]+"</span></div>";
			} else {
				html_p = "<div class='cellSelected province_"+j+"' data-p_id='"+j+"'><img class='select' src='"+imgPath+"/select1.png' /> <span>"+province[j]+"</span></div>";
			}
		} else {
			if( province_arr.indexOf(province[j]) >= 0 ){
				html_p += "<div class='cell province_"+j+" selectedProvince' data-p_id='"+j+"'><img class='select' src='"+imgPath+"/select1.png' /> <span>"+province[j]+"</span></div>";
			} else {
				html_p += "<div class='cell province_"+j+"' data-p_id='"+j+"'><img class='select' src='"+imgPath+"/select1.png' /> <span>"+province[j]+"</span></div>";
			}
		}
		...
		for( k in city ){
			if( k == 0 ){
				if( city_arr.indexOf(city[k]) >= 0 || checkAll ){
					html_c += "<div class='city-div cellSelected province_city_"+j+" selectedCity' data-c_id='"+k+"' data-p_id='"+j+"'><img class='select' src='"+imgPath+"/select1.png' /><span>"+city[k]+"</span></div>";
				} else {
					html_c += "<div class='city-div cellSelected province_city_"+j+"' data-c_id='"+k+"' data-p_id='"+j+"'><img class='select' src='"+imgPath+"/select1.png' /><span>"+city[k]+"</span></div>";
				}
			} else {
				if( city_arr.indexOf(city[k]) >= 0 || checkAll ){
					html_c += "<div class='city-div cell province_city_"+j+" selectedCity' data-c_id='"+k+"' data-p_id='"+j+"'><img class='select' src='"+imgPath+"/select1.png' /><span>"+city[k]+"</span></div>";
				} else {
					html_c += "<div class='city-div cell province_city_"+j+"' data-c_id='"+k+"' data-p_id='"+j+"'><img class='select' src='"+imgPath+"/select1.png' /><span>"+city[k]+"</span></div>";
				}
			}
...
function openProvince(){
	...
	$("#province").css("height",($('#province').height() - 42)+'px');
}
...
$('body').on('click','#province img',function () {
	...
	if( $(this).attr("src") == imgPath+"/select2.png" ){
		$(this).attr("src",imgPath+"/select1.png");
		$(this).parent().removeClass("selectedProvince");
		$('.city_'+province_id).find('img').attr('src',imgPath+'/select1.png');
		$('.city_'+province_id).find('div').removeClass("selectedCity");
		$('.area_'+province_id).find('div').removeClass('cellSelected').addClass('cell');
	} else {
		$(this).attr("src",imgPath+"/select2.png");
		$(this).parent().addClass("selectedProvince");
		$('.city_'+province_id).find('img').attr('src',imgPath+'/select2.png');
...
function checkSingleSelected(obj_province_id,obj_city_id){
	...
	if( objCityAreaNum == selectedAreaNum ){	//如果该市的所有区都已选，则该市为全选
		$objCity.find('img').attr('src',imgPath+'/select2.png');
	} else {
		$objCity.find('img').attr('src',imgPath+'/select1.png');
	}
	...
	for( var i = 0; i < objProvinceCitiesNum; i++ ){
		...
		if( allSelectedCityImg == imgPath+'/select2.png' ){
			allSelectedCityNum++;
		} else {
			break;
		}
	...
	if( objProvinceCitiesNum == allSelectedCityNum ){	//如果该省的所有市都已选，则该省为全选
		$objProvince.find('img').attr('src',imgPath+'/select2.png');
	} else {
		$objProvince.find('img').attr('src',imgPath+'/select1.png');
	}
...
function checkAllSelectedBtn(){
	...
	for( var j = 0; j < allProvinceNum; j++ ){
		if( $selectedProvinceImg.eq(j).attr('src') == imgPath+'/select1.png' ){
			$('.all-select .select').attr('src',imgPath+'/select1.png');
			return;
		}
	}
	
	$('.all-select .select').attr('src',imgPath+'/select2.png');
}

//全选
$('body').on('click','.all-select .select',function(){
	if( $(this).attr('src') == imgPath+'/select1.png' ){
		$(this).attr('src',imgPath+'/select2.png');
		
		$('#province').find('div').addClass('selectedProvince');
		$('#province').find('.select').attr('src',imgPath+'/select2.png');
		$('.cityBox').find('.city-div').addClass('selectedCity');
		$('.cityBox').find('.select').attr('src',imgPath+'/select2.png');
		$('#city_area').find('.area-div').removeClass('cell').addClass('cellSelected');
		$('#city_area').find('.select').attr('src',imgPath+'/select2.png');
	} else {
		$(this).attr('src',imgPath+'/select1.png');
		
		$('#province').find('div').removeClass('selectedProvince');
		$('#province').find('.select').attr('src',imgPath+'/select1.png');
		$('.cityBox').find('.city-div').removeClass('selectedCity');
		$('.cityBox').find('.select').attr('src',imgPath+'/select1.png');
		$('#city_area').find('.area-div').removeClass('cellSelected').addClass('cell');
		$('#city_area').find('.select').attr('src',imgPath+'/select1.png');
	}
});
