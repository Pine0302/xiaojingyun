...
while ($row_paySql = mysql_fetch_object($result_paySql)) {
	...
	$transaction_id 	= $row_paySql->callBackBatchcode;
}
	...
	<div class="WSY_columnbox">
		...
		<div class="WSY_remind_main" style="min-height:250px;">
			...
			<dl class="WSY_remind_dl02" style="margin-top:20px;margin-left:100px;">
				<dt style="line-height:20px;" class="WSY_left">京东交易单号：</dt>
				<dd><span class="input">
			...
			<dl class="WSY_remind_dl02" style="margin-top:20px;margin-left:100px;">
				...
				<?php
				$Pbatchcode = -1;
				$query = "select batchcode from weixin_commonshop_order_prices where isvalid=true and pay_batchcode='".$pay_batchcode."'";
			...
			<dl class="WSY_remind_dl02" style="margin-top:20px;margin-left:100px;">
				<dt style="line-height:20px;" class="WSY_left">京东支付：</dt>
				<dd><span class="input">
					<?php  echo $price; ?>
				</span></dd>
			</dl>
			<?php
			$coupon		= 0;
			$query = "select coupon from order_currencyandcoupon_t where pay_batchcode='".$pay_batchcode."'";
			$result = _mysql_query($query) or die('Query_weipay failed: ' . mysql_error());
			while ($row = mysql_fetch_object($result)) {
				$coupon 	= $row->coupon;
			}
			$currency = 0;
			$query = "select pay_currency from weixin_commonshop_order_prices where batchcode='".$batchcode."'";
			$result = _mysql_query($query) or die('Query_weipay failed: ' . mysql_error());
			while ($row = mysql_fetch_object($result)) {
				$currency 	= $row->pay_currency;
			}
			if( $currency > 0 and  $paystyle != "购物币支付" ){
			?>
			<dl class="WSY_remind_dl02" style="margin-top:10px;margin-left:100px;">
				<dt style="line-height:20px;" class="WSY_left"><?php echo defined('PAY_CURRENCY_NAME')? PAY_CURRENCY_NAME: '购物币'; ?>支付：</dt>
				<dd><span class="input">
					<?php  echo $currency; ?>
				</span></dd>
			</dl>
			...
			?>
			<dl class="WSY_remind_dl02" style="margin-top:10px;margin-left:100px;">
				<dt style="line-height:20px;" class="WSY_left">优惠券支付：</dt>
				<dd><span class="input">
					<?php  echo $coupon; ?>
				</span></dd>
			</dl>
...
function subReturn(){
   ...
   document.location = "/weixinpl/common_shop/jiushop/refund.php?customer_id=<?php echo $customer_id_en; ?>&pay_batchcode=<?php echo $pay_batchcode; ?>&total_fee=<?php echo $price; ?>&batchcode=<?php echo $batchcode?>&out_trade_no=<?php echo $out_trade_no; ?>&refund_fee="+return_money;
}
</script>
