...
var shop_obj={
	...
	home_init:function(){
		...
		for(i=0; i<shop_skin_data.length; i++){
			//console.log(shop_skin_data[i]['foreign_id']+'__'+i);
			var obj;
			...
			if(contenttype==1){
				...
				for(m=0;m<clen;m++){
					...
					var product_type_1_m = document.getElementById("product_type_1_"+(m+1));
				//	product_type_1_m.style.display="none";
					var options = sobj.options;	//获取轮播图所有的下拉项
					//console.log(options);
					//document.getElementById("div_products_1_"+(m+1)).style.display="none";
					
					//console.log(m+"linktypearr========"+linktypearr[m]);
					...
					if(temp_arr.length>1){
                        temp_arr[temp_arr.length-1] = temp_arr[temp_arr.length-1].replace('&henggan&', '-');
						$("input[name='selector_title_"+(m+1)+"']").attr("value",temp_arr[temp_arr.length-1]);
                        $("input[name='selector_id_"+(m+1)+"']").attr("value",linktypearr[m]);
						options[0].selected =true;
					}else{
						$("input[name='selector_title_"+(m+1)+"']").attr("value","");
                        $("input[name='selector_id_"+(m+1)+"']").attr("value","");
						if(linktypearr[m]==1 || linktypearr[m]==2 || linktypearr[m]==3 || linktypearr[m]==10 || linktypearr[m]==11 || linktypearr[m]==12 || linktypearr[m]==13 || linktypearr[m]==50 || linktypearr[m]==51 || linktypearr[m]==52 || linktypearr[m]==53|| linktypearr[m]==60 || linktypearr[m]==61|| linktypearr[m]==62|| linktypearr[m]==63|| linktypearr[m]==64|| linktypearr[m]==65){
			
						   for(var j=0;j<options.length;j++){
							   //链接类型
							   var ov = options[j].value;	//获取下拉项值
							   var ovlen = ov.length;
							   ...
							   if(ov.indexOf('_')!=-1){
								   ...
								   ov = ovarr[0];
								   ovtype = ovarr[1];		//下拉项类型						   
							   }
							   var type_id = -1;
							   if(ov == -40 && linktypearr[m]==1){	//选择了多级分类
								   // console.log(foreign_idarr[m]);
								   var foreign_id_arr = foreign_idarr[m].split('_');
								   foreign_idarr[m] = -40;
							   ...
							   */
							  //console.log(ov);
							   /* if(ovlen>2 && foreign_idarr[m] > -10){
								  ov = ov.substring(0,ov.length-2);
							   }
							   if(ovlen>2 && foreign_idarr[m] <-9){ //解决链接页面，id小于-9之后，后台模板不显示链接 1.29 by cdr
									ov = ov.substring(0,ov.length);
									//console.log(ov+'***');
							   } */
								
								// console.log(m+"========"+ov+"===="+foreign_idarr[m]+"===="+ovtype+"===="+linktypearr[m]);

							   if(ov==foreign_idarr[m] && ovtype==linktypearr[m]){
								   options[j].selected ="selected";
								   
								   //break;
								   if(linktypearr[m]==1){
									   if(foreign_idarr[m] == -40){
										   var product_type_1_m_option = product_type_1_m.options;
										   var product_type_1_m_option_len = product_type_1_m_option.length;
										   for(var k=0;k<product_type_1_m_option_len;k++){
											   var product_type_1_m_option_arr = product_type_1_m_option[k].value.split('_');
											   var product_type_1_m_option_type = product_type_1_m_option_arr[0];
											  // product_type_1_m.style.display="block";
											   if(type_id == product_type_1_m_option_type){
												   product_type_1_m_option[k].selected = "selected";
												   break;
											   }
										   }
									   }
								   ...
								   //产品详情
									   if(d_id>0){
											...
											changeProductType3(type_id,d_id,parseInt(m+1,10)); 
									   }else{
											changeProductType3(type_id,-1,m+1); 
									   }
				...
				var product_type_2 = document.getElementById('product_type_2');
				product_type_2.style.display="none";
			    if(iscate && iscate==1){
				...
				if(temp_arr.length>1){
                    ...
                    $("#selector_id").attr("value",linktype);
					options[0].selected =true;
				}else{
					/* 旧选择链接 */
					$('#selector_title').attr("value","");
					$("#selector_id").attr("value","");
					//链接类型
					if(linktype==1 || linktype==2 || linktype==3 || linktype==10 || linktype==11 || linktype==12 || linktype==13 || linktype==50 || linktype==51 || linktype==52 || linktype==53 || linktype==54 || linktype==60 || linktype==61 || linktype==62 || linktype==63 || linktype==64|| linktype==65){
					   ...
					   for(var j=0;j<options.length;j++){
						   ...
						   if(ovlen>2 && foreign_id <-9){ //解决链接页面，id小于-9之后，后台模板不显示链接 1.29 by cdr
							 ...
							 // console.log(ov+"===="+foreign_id+"===="+ovtype+"===="+linktype);
						   if(ov==foreign_id && ovtype==linktype){
							   ...
							   if(linktype==1){
								   ...
								   options[j].selected ="selected";
								   if(foreign_id == -40){
									   var option_2 = product_type_2.options;
									   var option_2_len = option_2.length;
									   for(var k=1;k<option_2_len;k++){
										   var option_2_arr = option_2[k].value.split('_');
										   var option_2_type = option_2_arr[0];
										   product_type_2.style.display="block";
										   if(type_id == option_2_type){
											   option_2[k].selected = "selected";
											   break;
										   }
									   }
								   }
								   ...
								   if(detail_id>0){
										 changeProductType2(type_id,detail_id); 
								   }else{
										changeProductType2(type_id,-1); 
								   }
							   }else{
								  options[j].selected ="selected";
							   }
		...
		//alert('index====');
			...
			if(contenttype==1){
							...
							if(ov==foreign_id && ovtype==linktype){
								if(linktype==1){
									...
									if(foreign_id == -40){
										var option_2     = product_type_2.options;
										var option_2_len = option_2.length;
										for(var i=0;i<option_2_len;i++){
											var option_2_arr  = option_2[i].value.split('_');
											var option_2_type = option_2_arr[0];
										//	product_type_2.style.display="block";
											if(type_id == option_2_type){
											   option_2[i].selected = "selected";
											   break;
											}
									   }
									}
			   ...
			   //文字
				...
				if(temp_arr.length>1){
                    temp_arr[temp_arr.length-1] = temp_arr[temp_arr.length-1].replace('&henggan&', '-');
					$('input[name=selector_title_font]').attr("value",temp_arr[temp_arr.length-1]);
                    $('input[name=selector_id_font]').attr("value",linktype);
					options[0].selected =true;
				}else{
					$('input[name=selector_title_font]').attr("value",'');
                    $('input[name=selector_id_font]').attr("value",'');
					if(linktype==1 || linktype==2 || linktype==3 || linktype==10 || linktype==11 || linktype==12 || linktype==13 || linktype==50 || linktype==51 || linktype==52 || linktype==53 || linktype==54 || linktype==60  || linktype==61 || linktype==62 || linktype==63 || linktype==64 || linktype==65){
