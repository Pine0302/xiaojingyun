...
<head>
	...
	<style type="text/css">
		...
		.temp_data{margin-left: 20px;padding-left:10px;width: 800px;height:30px;line-height: 30px;border-radius: 5px;border: 1px solid #ccc;font-size: 16px;}
		...
		<div class="WSY_columnbox">	
		    ...
		    <div class="WSY_data">
		    	<div class="WSY_agentsbox">
					<form style="display:block" method="get" action="/mshop/admin/index.php?m=block_chain&a=integral_reward_activity">
						<input type="hidden" id="m" name="m" value="block_chain">
						<input type="hidden" id="a" name="a" value="integral_reward_activity">
						...
						<ul class="WSY_search_q">
							...
							<li><input type="button" class="WSY-skin-bg form-btn add_activity" onclick="location.href='/mshop/admin/index.php?m=block_chain&a=integral_reward_activity_add&bonus_id=<?php echo $bonus_id;?>&customer_id=<?php echo $customer_id_en ?>'" <?php echo $bonus_data['exchange_bonus_money'] <=0 || $on_off == false?"disabled='disabled' style='color:#ccc;'":"style='color:#fff;'";?> value="添加兑换零钱产品">
							</li>
					...
					<ul class="temp_data">
						...
						<li>
							可兑换奖金总额：￥<?php echo number_format($bonus_data['exchange_bonus_money'],4);?><?php echo $bonus_data['exchange_bonus_money'] <=0 ? '（<span style="color:red;">奖金已全部分配</span>）' : '' ;?>
						</li>
						<li>
						...
						<tbody class="tbody-main">
							<?php foreach ($data as $row){ ?>
								<tr>
									...
									<?php }?>
										<button type="button" class="WSY-skin-bg form-btn caozuo" onclick="location.href='/mshop/admin/index.php?m=block_chain&a=integral_reward_exchange_log&bonus_id=<?php echo $bonus_id;?>&activity_id=<?php echo $row['id'];?>'">兑换日志</button>
									</td>
								</tr>
							<?php } ?>
						</tbody>
...
var product_name = $("#product_name").val();//订单号
	...
	bonus_id     = <?php echo $bonus_id ?>;//奖金池id
...
$(".WSY_page").createPage({
	...
	current:pagenum,
	backFn:function(p){
	var url="/mshop/admin/index.php?m=block_chain&a=integral_reward_activity&pagenum="+p+param;	
	location.href = url;
   }
});
...
function jumppage(){
	...
	if((a<1) || (a>count) || isNaN(a)){
		...
		var url="/mshop/admin/index.php?m=block_chain&a=integral_reward_activity&pagenum="+a+param;
...
function export_excel(){
	var url='/weixin/plat/app/index.php/Excel/block_chain_activity_excel/customer_id/'+'<?php echo $customer_id;?>';
	...
	layer.confirm(remark, {
		...
		else if(op == 'del_many')
		{
			...
			$(".tbody-main :checkbox").each(function(){
				if($(this).attr('checked') && $(this).attr('stat') == 1)//如果是已删除就不传ID过去
				{
