...
$query = "SELECT c.id,c.user_id,u.name,u.weixin_name,c.currency,c.after_currency,c.createtime FROM weixin_users u RIGHT JOIN currency_luokeyun_log c ON u.id=c.user_id WHERE c.isvalid=true {$where} and c.customer_id=".$customer_id;
		...
		<table class="WSY_t2"  width="95%"  style="border: 1px solid #D8D8D8;border-collapse: collapse;">
			...
			?>
				<tr style="border:1px solid #D8D8D8">
					<td><?php echo $id;?></td>
					<td><a href="pay_currency_luokeyun_log.php?promoter=<?php echo $c_user_id;?>" style="color:#06a7e1;"><?php echo $c_user_id;?></a></td>
		 ...
		 var pagenum = <?php echo $pagenum ?>;
		  var count =<?php echo $page ?>;//总页数
			...
			$(".WSY_page").createPage({
				...
				current:pagenum,
				backFn:function(p){
				 document.location= "pay_currency_luokeyun_log.php?pagenum="+p+"&begintime="+begintime+"&endtime="+endtime+"&promoter="+user_id;
			   }
			});
		  ...
		  function jumppage(){
			...
			document.location= "pay_currency_luokeyun_log.php?pagenum="+a+"&begintime="+begintime+"&endtime="+endtime+"&promoter="+user_id;
			}
		  }	
		</script>
