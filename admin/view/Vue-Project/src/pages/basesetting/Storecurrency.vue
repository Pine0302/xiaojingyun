    ...
    <div class="WSY_content">
        <div class="WSY_remind_main">
          <dl class="WSY_remind_dl02">
            <dt>{{storeintegralname}}转换{{customname}}开关：</dt>
            <dd>
      ...
      <div v-show="turn_on_off==1">
          <dl class="WSY_remind_dl02">
            <dt>转换条件：</dt>
            <dd>
              <span>最低转换{{storeintegralname}} </span><input type="number" name="trans_min" placeholder="" v-model="trans_min" @keyup="transMin(trans_min)"><span> (-1表示不限)</span>
            </dd>
          </dl>
          <dl class="WSY_remind_dl02">
            <dt>转换系数：</dt>
          ...
          <dl class="WSY_remind_dl02">
            <dt>转换规则：</dt>
            <dd>
              <span>{{customname}}： </span><span>{{storeintegralname}} 1 : </span><input type="number" name="trans_rule" placeholder="" v-model="trans_rule" @blur="regfun()" ref="trans_rule"><span> (可输入大于0的整数)</span>
            </dd>
          </dl>
          <dl class="WSY_remind_dl02">
            <dt>转换说明：</dt>
  ...
  created: function() {
    ...
    self.$http.post(urlget, {
      ...
      .then(res => {
        ...
        //  console.log(res.data.errmsg);
        // }
        var datajsonget = res.data.data_store;
