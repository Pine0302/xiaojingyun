...
<body>
	...
	<div class="WSY_content">
		...
		<div class="WSY_columnbox">
			...
			?>
			<!--列表头部切换结束-->
			<form action="save_documentation.php?customer_id=<?php echo $customer_id_en; ?>" enctype="multipart/form-data" method="post" id="upform" name="upform">
				<div class="WSY_remind_main">
					...
					<dl class="WSY_member">			
						<div>
							...
							<dd class="spa">
								...
								<input style="width:208;border:1 solid #9a9999; font-size:9pt; background-color:#ffffff; height:18;margin-top: 5px;margin-bottom: 5px;" size="17" name="upfile1" id="upfile" class="upfile" type=file value=""> (图片尺寸：宽220*高200)
					...
					<div id="add_rotation">
						<?php 
							if(!empty($right_img)){
								for($i=0;$i<count($right_img);$i++){ 
								$j = $i+1;
								$z = $i+2;

						?>
							<dl class="WSY_member img_div" id="<?php echo $z; ?>" style="float:left;width:500px;">
								<div>
									<dt>轮播图片<?php echo $j; ?></dt>
									<div style="display: block;position: absolute;margin-left: 110px;">
										<select id="foreign_id<?php echo $z; ?>" name="foreign_id<?php echo $z; ?>" onchange="getproduct(this.options[this.options.selectedIndex].value,1,<?php echo $z; ?>)">
											<optgroup label="---------------产品分类---------------"></optgroup>
											<?php 
												for($n=0;$n<count($typearr);$n++){
													$typestr=explode("_",$typearr[$n]);
											?>	  
												<option value="<?php echo $typestr[0];?>_1" <?php if($pro_str[$i][0]==$typestr[0]){echo 'selected';} ?>><?php echo $typestr[1];?></option>
											<?php 	
												}
											?>
										</select>
									</div>
									<div style="display: block;position: absolute;margin-left: 110px;margin-top:20px;">
										
									<select id="detail_id<?php echo $z; ?>" name="detail_id<?php echo $z; ?>" style="width:160px;margin-top:5px;float:left;">
										<?php 
											$product_id = -1;
											$product_name = '';
											$typeson_id = array();
											$query_pro = "select id,name from weixin_commonshop_products where isvalid=true and isout=0 and customer_id=".$customer_id;	
											if($pro_str[$i][0] >0){
												$type_son="select id from weixin_commonshop_types where isvalid=true and is_shelves=1 and parent_id=".$pro_str[$i][0]." and customer_id=".$customer_id."";
												...
												if(empty($typeson_id)){
													$typeson_id=$pro_str[$i][0]; 
												}else{
													array_push($typeson_id,$pro_str[$i][0]);
													$typeson_id=implode(',',$typeson_id);
												}
													$query_pro = $query_pro." and (";
													...
													for($j=0;$j<$typeson_id_count;$j++){
														...
														if($j==0){
															$query_pro = $query_pro."( LOCATE(',".$o_typeid.",', type_ids)>0)";
															}else{
															$query_pro = $query_pro." or (LOCATE(',".$o_typeid.",', type_ids)>0)";
														}
													}
													$query_pro = $query_pro.")";		
												}
											$result_pro=_mysql_query($query_pro) or die ('query_pro faild ' .mysql_error());
											while($row_pro = mysql_fetch_object($result_pro)){
												$product_id = $row_pro->id;
												$product_name = $row_pro->name;
										?>
											<option value="<?php echo $product_id;?>" <?php if($pro_str[$i][1]==$product_id){echo 'selected';} ?>><?php echo $product_name;?></option>
										<?php } ?>
									</select>
									</div>
									<dd class="spa" style="margin-top: 50px;">
										<img src="<?php echo $right_img[$i]; ?>"  id="img_v<?php echo $z; ?>" style="width:208px;height:50px;" /><br/>
										<input style="width:208;border:1 solid #9a9999; font-size:9pt; background-color:#ffffff; height:18;margin-top: 5px;margin-bottom: 5px;width: 160px;" size="17"  class="upfile" name="upfile<?php echo $z; ?>" id="upfile<?php echo $z; ?>" type=file value=""><input type=botton style="float:none;margin-left:10px;" class="WSY_button1 WSY-skin-bg" onclick="delect_rotation(<?php echo $z; ?>)" value='删除'>	
										<br/>(图片尺寸：宽831*高200)
										<input type=hidden value="<?php echo $right_img[$i]; ?>" name="imgurl<?php echo $z; ?>" id="imgurl<?php echo $z; ?>" /> 
									</dd>	
								</div>
							</dl>
							<?php } }?>
					...
					<div class="WSY_text_input" ><button style="  margin-top: 200px;" class="WSY_button" onclick=" return subBase();">提交保存</button><br class="WSY_clearfloat"></div>
				</div>
			</form>
		</div>
	</div>
