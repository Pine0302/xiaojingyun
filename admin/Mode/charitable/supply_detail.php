...
$phone             = "";//用户头像
$query = "select name,weixin_name,phone from weixin_users where isvalid=true and id=". $user_id ." and customer_id=".$customer_id." limit 0,1";
...
$query = "select charitable from weixin_commonshop_applysupplys where isvalid=true and user_id=". $user_id;
$result = _mysql_query($query) or die('Query failed: ' . mysql_error());				   
while ($row = mysql_fetch_object($result)) {
	$charitable = $row->charitable;
}
/*输出数据语句*/
$query = "select batchcode,reward,createtime,paytype,charitable from charitable_log_t where supply_id=". $user_id ." and isvalid=true and customer_id=".$customer_id;
/*输出数据语句*/

/*统计数据数量*/

$query_num ="select count(1) as wcount from charitable_log_t where isvalid=true and customer_id=".$customer_id." and  supply_id=". $user_id;
/*统计数据数量*/
$sql = "";
		...
		<div  class="WSY_data">
			<div id="WSY_list" class="WSY_list">
				<div class="WSY_left" style="background: none; float:none;">
					...
					合作商慈善分：<span style="font-weight:bold;font-size:22px;color:red"><?php echo $charitable; ?></span>
				</div>
				...
				<?php if(!empty($begintime)){
						...
						$query1 = "select IFNULL(sum(charitable),0) as month_charitables from charitable_log_t where isvalid=true and paytype in (0,1) and  supply_id=".$user_id." and UNIX_TIMESTAMP(createtime)>".strtotime($begintime)." and customer_id=".$customer_id;
						if(!empty($endtime)){			   
							$query1 .= " and UNIX_TIMESTAMP(createtime)<".strtotime($endtime);
						}else{
							$endtime = "现在";
						}
						$month_charitables = 0;
						$result=_mysql_query($query1) or die('L122 '.mysql_error());
						while($row=mysql_fetch_object($result)){
...
<script type="text/javascript" src="../../Common/js/Mode/charitable/supply_detail.js"></script>
</body></html>