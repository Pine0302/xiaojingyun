...
if(!empty($_GET["microshop_adimg"])){
	...
	//  if($customer_id>0){
	//    $query2="select microshop_adimg from weixin_commonshop_customer_microshop where customer_id=".$customer_id;
	//    //echo $query2;
	// 	$result2 = _mysql_query($query2) or die('Query failed2: ' . mysql_error());
	// 	while ($row2 = mysql_fetch_object($result2)) {
	// 	   $microshop_adimg=$row2->microshop_adimg;
	// 	}
...
<form action="save_supplier_adimg.php?customer_id=<?php echo $customer_id_en; ?>&id=<?php echo $id; ?>&num=<?php echo $num; ?>" id="frm_typeimg" enctype="multipart/form-data" method="post">
			...
			<p style=" padding-left: 10px;">上传1张图片，微店首页广告图片。图片大小建议：340*170像素</p>
			<?php if(empty($adimg[$i])){?>
				<a href="" class="classify_content_img"><img id="show_img" src="../../Common/images/Product/contenticon/classify_content1.png" style="width:278px;height:120px" ></a>
			<?php }else{?>
			<a href="" class="classify_content_img"><img id="show_img" src="<?php echo $adimg[$i];?>" style="width:278px;height:120px" ></a>
			<?php } ?>
			
</form>
<a href="del_supplier_adimg.php?customer_id=<?php echo $customer_id_en; ?>&src=<?php echo $adimg[$i];?>"><div class="del"><span style="color:#FFFFFF">删除</span></div></a>
<?php 
...
<script type="text/javascript">  
	...
	parent.setTypeImg('<?php echo $adimg[$i]; ?>',<?php echo $i; ?>);
