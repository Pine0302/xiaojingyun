<?php
    ...
    require_once('../../../../weixinpl/sphinx/sphinx.php');//搜索引擎
    /* redis start */
	...
	$sphinx = new sphinx();//搜索引擎
	$sphinx_on = SPHINX_ON;//搜索引擎开关
	$sphinx_idx_name = DB_NAME == 'weixin_platform' ? '' : DB_NAME.'_';//一般客户的为空就可以了，主要是兼容ST环境
	$log_ = new Log_();
	...
	// {
	// 	$delKey = $configutil->splash_new($_GET["delKey"]);
	// }
	// if ($delKey == 1) 
	// {
	// 	//清空缓存
	// 	$redis = redis_construct();
	//  	$md5_sql = $redis->keys('md5_sql*');
	//  	foreach ($md5_sql as $value) 
	//  	{
	//  		$redis->delKey($value);
	//  	}
    ...
    $shop_branch_id='';//子门店id
    if(!empty($_GET["shop_branch_id"])){
        $shop_branch_id = $configutil->splash_new($_GET["shop_branch_id"]);
    }
    if($shop_branch_id!=''){
    	$query_branch = "select apply_name,apply_phone,branch_name,user_id from orderingretail_shop_branch where id = ".$shop_branch_id;
		$apply_name_branch = '';
		$apply_phone_branch = '';
		$branch_name = '';
		$user_id_branch=-1;
		$res_branch_shop = _mysql_query($query_branch);
        if($row_branch_shop = mysql_fetch_object($res_branch_shop)){
            $apply_name_branch  = $row_branch_shop -> apply_name;
            $apply_phone_branch = $row_branch_shop -> apply_phone;
            $branch_name = $row_branch_shop -> branch_name;
            $user_id_branch=$row_branch_shop -> user_id;
        }
        $parent_id_b =-1;
	    $query_exp_b= "select parent_id from weixin_users where id=" . $user_id_branch . " limit 1";
	    $result_exp_b = _mysql_query($query_exp_b);
		while ($row_exp_b = mysql_fetch_object($result_exp_b)) {
			$parent_id_b=$row_exp_b->parent_id;
		}
		$name_p_b='';
		$id_p_b='';
		if ($parent_id_b!=-1) {
			$query_exp_b= "select name,id from orderingretail_proxy where user_id=" . $parent_id_b . " AND customer_id=".$customer_id." AND isvalid=1 limit 1";
		    $result_exp_b = _mysql_query($query_exp_b);
			while ($row_exp_b = mysql_fetch_object($result_exp_b)) {
				$name_p_b=$row_exp_b->name;
				$id_p_b=$row_exp_b->id;
			}
		}
    }
    ...
    $or_shop_id = i2get("or_shop_id",0); //订货系统中的订货商id
    $is_shop = i2get("is_shop",0); //门店查订单
    $store_lev = i2get("store_lev","all"); //订货系统仓库级别
    // echo "store_lev: ".$store_lev;
    $search_or_shop_id = i2get("search_or_shop_id", ""); //订货系统门店ID
    // $search_or_shop_name = i2get("search_or_shop_name", ""); //订货系统门店名
    if($is_shop == 1){
    	$search_or_shop_id = $or_shop_id;
    }
    ...
    /* 贺卡祝福语按钮 */
    $is_blessing = 0;
    $query_blessing = "select is_blessing from weixin_commonshops_extend where isvalid=true and customer_id=".$customer_id." limit 1";
    $result_blessing = _mysql_query($query_blessing) or die('Query_is_remind failed: ' . mysql_error());
    while ($row_blessing = mysql_fetch_object($result_blessing)) {
...
/* 区块链积分 自定义名称end*/
	...
	$o_block_chain_price   = 0;//区块链实际支付的金额
	/*$query_order = "SELECT
	...
	$query_order = "SELECT orders.batchcode,
						orders.pay_batchcode,
						orders.user_id,
						orders.createtime,
						orders.paystyle,
						...
						orders.is_sendorder,
						orders.is_sign,
						orders.cost_integral,
						orders.shopactivity_mark,
						orders.is_exchange,
						orders.yundian_id,
						orders.yundian_self,
						orders.bs_relate_prod_id,
						orders.block_chain_price
						";
            if($from_page > 0){
    ...
    $search_integral = 0;//积分兑换产品
	if(!empty($_GET["search_integral"])){    //积分兑换产品
		$search_integral = $_GET["search_integral"];
    }

    if($search_integral!=1){
        $query_search = " WHERE orders.customer_id = ".$customer_id." AND orders.is_sendorder ".$is_sendorder;
    }else{//积分兑换的把派单的也显示出来
        $query_search = " WHERE orders.customer_id = ".$customer_id;
    }


	if($shop_branch_id!='' && $from_page==1){
        	$query_search.=' AND op.o_shop_id='.$shop_branch_id;//子门店的id
        }

	if($from_page > 0){ //订货系统 or f2c
        $query_order .= ' INNER JOIN system_send_order AS sso on orders.batchcode = sso.order_id and sso.send_type='.$from_page;

        if($shop_branch_id!='' && $from_page==1){
        	$query_order.=' AND sso.or_shop_type=2 INNER JOIN weixin_commonshop_order_prices AS op on orders.batchcode = op.batchcode';//子门店的
        }
        ...
        if($from_page == 1){
            ...
            if($store_lev != "all"){
                $grade=4;
                switch ($store_lev) {
                    case 'province':  $grade=2; break;
                    case 'city':  $grade=1; break;
                    case 'area':  $grade=0; break;
                    case 'custom_area':  $grade=3; break;
                    default: break;
                }
                $query_area_set="SELECT DISTINCT c.store_id FROM system_send_order c INNER JOIN orderingretail_area_set a ON a.id=c.store_id WHERE c.customer_id = ".$customer_id." AND a.area_id IN (SELECT id FROM weixin_commonshop_team_area WHERE grade=".$grade.")";
                // echo $query_area_set;
                $result = _mysql_query($query_area_set);
	            ...
	            if(empty($store_id_str)){
	                $store_id_str="-1";
	            }
	            $query_search .= ' AND sso.store_id in ('.$store_id_str.')';

            }

            if(!empty($search_or_shop_id)){
                //$query_or_shop_search = "SELECT proxy_id FROM orderingretail_shop WHERE customer_id=".$customer_id." AND isvalid=true AND is_freeze=false AND (id='".$search_or_shop_id."' OR shop_name LIKE '%".$search_or_shop_id."%')";
				$query_or_shop_search = "SELECT proxy_id FROM orderingretail_shop WHERE customer_id=".$customer_id." AND isvalid=true AND is_freeze=false";
				$query_or_shop_search .= is_numeric($search_or_shop_id)? " AND id='".$search_or_shop_id."'" : " AND shop_name LIKE '%".$search_or_shop_id."%'";
                // echo $query_or_shop_search."<br/>";
                $res_or_shop_search = _mysql_query($query_or_shop_search);
                while ($row = mysql_fetch_array($res_or_shop_search,MYSQL_ASSOC)) {
                    array_push($proxy_ids, $row["proxy_id"]);
                }

                /*if($is_shop == 1){
	                array_push($proxy_ids, $proxy_id);
	            }*/
	            if(count($proxy_ids) > 0){
	                $proxy_id_str = implode(',',$proxy_ids);
	            }

	            if(empty($proxy_id_str)){
	                $proxy_id_str="-1";
	            }

	            $query_search .= ' AND sso.current_proxy_id in ('.$proxy_id_str.')';
            }

        }
    }
	$query_search .= " AND orders.yundian_self = 0 ";//云店自营订单不显示在商城订单中
    if($from_page > 0){ //订货系统 or f2c
        // $query_order .= '  and sso.isvalid = true ';
        $query_search .= '  and sso.isvalid = true ';
        if($param_store_id > 0){
            // $query_order .= '  and sso.store_id = '.$param_store_id;
            $query_search .= '  and sso.store_id = '.$param_store_id;
        }
    }
	...
	if( $orgin_type > 0 ){
		...
		switch( $orgin_type ){
			...
			break;
            case 5:
                $query_search .= ' AND orders.is_restricted_purchase=1';
            break;
            case 6:
                $query_search .= ' AND (orders.yundian_id > 0 or orders.yundian_self = 1) ';
            break;
            case 7:
                $query_search .= ' AND orders.shopactivity_mark=3 and orders.bs_relate_prod_id != 0 ';
            break;
		}
	}
	...
	if(!empty($_GET["search_product_name"])){    //产品名称
		...
		if($sphinx_on == false){
			...
			$res_spn   = _mysql_query($query_spn);
			while ($row = mysql_fetch_object($res_spn)) {
				$da_spn[] =  $row->id;
			}
			if($da_spn)
			{
				$da_spn_str = implode("','",$da_spn);
				$da_spn_str = "'".$da_spn_str."'";
				$query_search .= " AND orders.pid in (".$da_spn_str.")";
			}	
		}else{
			$pro_list = $sphinx->get_sphinx_index($search_product_name,$sphinx_idx_name.'commonshop_products,'.$sphinx_idx_name.'commonshop_products_delta',array($customer_id),'array',1,1000);
			if($pro_list['total'] > 1000){
				$pro_list = $sphinx->get_sphinx_index($search_product_name,$sphinx_idx_name.'commonshop_products,'.$sphinx_idx_name.'commonshop_products_delta',array($customer_id),'array',1,$pro_list['total']);
			}
			$pro_idstr = implode(',',$pro_list['result']);
			if($pro_idstr){
				$query_search .= " AND orders.pid in (".$pro_idstr.")";
			}
		}
	...
	/* 搜索条件 */
	 ...
	 $now_time  = time();
	 // $begintime = date('Y-m-d 00:00', strtotime('-7 days'));
	 // $endtime = date("Y-m-d H:i:s", time());

	$begintime = "";
	$endtime = "";

     if(!empty($_GET["if_search_time"])){ //搜索标识

         $begintime = "";
         if(!empty($_GET["begintime"])){  //下单时间
            $begintime = $_GET["begintime"];
            $query_search = $query_search." and orders.createtime>='".$begintime."'";
         }

         $endtime = "";
         if(!empty($_GET["endtime"])){   //下单时间 End
            $endtime = $_GET["endtime"];
            $query_search = $query_search." and orders.createtime<='".$endtime."'";
         }

     }else{//不是搜索的话，默认一个搜索时间，防止反应过慢
		 if(!empty($begintime)) {
			 // $query_search = $query_search . " and orders.createtime>='" . $begintime."'";
		 }
		 if(!empty($begintime)) {
	...
	if(!empty($_GET["pay_begintime"])){  //支付时间
	   ...
	   $query_search = $query_search." and orders.paytime>='".$pay_begintime."'";
	}

	$pay_endtime = "";
	if(!empty($_GET["pay_endtime"])){   //支付时间 End
	   $pay_endtime = $_GET["pay_endtime"];
	   $query_search = $query_search." and orders.paytime<='".$pay_endtime."'";
	}


	/*****  确认订单时间 start *****/
	$confirm_order_time_l = '';
	$confirm_order_time_r = '';
	if (!empty($_GET['confirm_order_time_l']) && strtotime($_GET['confirm_order_time_l'])!==false) {
		 $confirm_order_time_l = $_GET["confirm_order_time_l"];

		 $query_search .= " and orders.confirm_order_time>='".$confirm_order_time_l."' ";
	}

	if (!empty($_GET['confirm_order_time_r']) && strtotime($_GET['confirm_order_time_r'])!==false) {
		 $confirm_order_time_r = $_GET["confirm_order_time_r"];

		 $query_search .= " and orders.confirm_order_time<='".$confirm_order_time_r."' ";
	}
	/*****  确认订单时间 end *****/
	...
	if(!empty($_GET["search_batchcode"])){    //订单号
	   ...
	   if($sphinx_on == false){
	  		$query_search = $query_search." and (orders.batchcode='".$search_batchcode."' or orders.pay_batchcode= '".$search_batchcode."')";
		   	...
		   	$batch_list = $sphinx->get_sphinx_index($search_batchcode,$sphinx_idx_name.'commonshop_orders,'.$sphinx_idx_name.'commonshop_orders_delta',array($customer_id),'array',1,1000);
		   	if($batch_list['total'] > 1000){
				$batch_list = $sphinx->get_sphinx_index($search_batchcode,$sphinx_idx_name.'commonshop_orders,'.$sphinx_idx_name.'commonshop_orders_delta',array($customer_id),'array',1,$batch_list['total']);
			}
			$batch_idstr = implode(',',$batch_list['result']);
			if($batch_idstr){
				$query_search .= " AND (orders.id in(".$batch_idstr."))";
			}
	   }
	...
	$search_user_id = "";
	if(!empty($_GET["search_user_id"])){    //用户id
	   ...
	   $search_name = $configutil->splash_new($_GET["search_name"]);
		switch($search_name_type){
			case 1:
				...
				$res_sn1   = _mysql_query($query_sn);
				while ($row = mysql_fetch_object($res_sn1)) {
					$da_sn1[] =  $row->id;
				}
				$da_sn_str1 = implode("','",$da_sn1);
				if($da_sn_str1 == '')
				{
					$da_sn_str1 = -1;
				}	
				$da_sn_str1 = "'".$da_sn_str1."'";
				$query_search .= " AND orders.user_id IN (".$da_sn_str1.")";
			break;
			case 2:
			    ...
			    if($sphinx_on == false){
			    	$query_sn = "SELECT batchcode FROM weixin_commonshop_order_addresses WHERE name like '".$search_name."%'";
			    	$res_sn2   = _mysql_query($query_sn);
					while ($row = mysql_fetch_object($res_sn2)) {
						$da_sn2[] =  $row->batchcode;
					}
					$da_sn_str2 = implode("','",$da_sn2);
					if($da_sn_str2 == '')
					{
						$da_sn_str2 = -1;
					}
					$da_sn_str2 = "'".$da_sn_str2."'";
					$query_search .= " AND orders.batchcode IN (".$da_sn_str2.")";
				}else{
					$addresses_list = $sphinx->get_sphinx_index($search_name,$sphinx_idx_name.'order_addresses,'.$sphinx_idx_name.'order_addresses_delta','','array',1,1000);
					if($addresses_list['total'] > 1000){
						$addresses_list = $sphinx->get_sphinx_index($search_name,$sphinx_idx_name.'order_addresses,'.$sphinx_idx_name.'order_addresses_delta','','array',1,$addresses_list['total']);
					}
					$addresses_idstr = implode(',',$addresses_list['result']);
					if($addresses_idstr){
						$query_search .= " AND orders.batchcode IN (".$addresses_idstr.")";
					}
				} 
				//var_dump($addresses_list);
	...
	if(!empty($_GET["search_phone"])){    //收货人电话
	   $search_phone = $configutil->splash_new($_GET["search_phone"]);
		...
		$query_sp = 'SELECT batchcode FROM weixin_commonshop_order_addresses WHERE phone="'.$search_phone.'"';
		$res_sn3   = _mysql_query($query_sp);
		while ($row = mysql_fetch_object($res_sn3)) {
			$da_sn3[] =  $row->batchcode;
		}
		if($da_sn3)
		{
			$da_sn_str3 = implode("','",$da_sn3);
			$da_sn_str3 = "'".$da_sn_str3."'";
			$query_search .= " AND orders.batchcode IN (".$da_sn_str3.")";
		}
	}
    ...
    $search_sendstyle = "";	//配送方式
	if(isset($_GET["search_sendstyle"])){
	   $search_sendstyle = $configutil->splash_new($_GET["search_sendstyle"]);  //支付方式

	   if( $search_sendstyle != "" && $search_sendstyle != -1){
		    $query_search .= " AND orders.sendstyle='".$search_sendstyle."'";
	   }
	} 
	...
	if( !empty($_GET['search_pre_delivery_type']) ){
        ...
        $current_time_1 = date('Y-m-d H:i:s',$current_time);
		switch( $search_pre_delivery_type ){
			case 1:
				$target_time = strtotime("+1 day",$current_time);
                $target_time = date("Y-m-d H:i:s",$target_time);
				$query_search .= " AND orders.delivery_time_start > 0 AND orders.delivery_time_start<='".$target_time."' AND orders.delivery_time_start>='".$current_time_1."'";
			break;
			case 2:
	...
	/* 订单支付状态 */	
	$count_order_type = "";
	if(!empty($_GET["count_order_type"])){    //点单支付状态
		$count_order_type = $configutil->splash_new($_GET["count_order_type"]);
		if($count_order_type == '1'){
			$query_search .= " AND orders.paystatus=true AND orders.status=1  AND orders.isvalid=true";  //交易完成
		}else if($count_order_type == '2'){
			$query_search .= " AND orders.paystatus=false AND orders.isvalid=true and is_pay_on_delivery != 1";  //待付款
		}else if($count_order_type == '3'){
			$query_search .= " AND orders.paystatus=true AND orders.isvalid=true ";  //已付款
		}
	}
	/* 订单支付状态 end*/
	...
	//砍价订单和众筹订单
	if(!empty($_GET["shopactivity_mark"])){
		$shopactivity_mark=$_GET["shopactivity_mark"];
		$query_search .= " AND orders.shopactivity_mark=".$shopactivity_mark ;
	}else{
		$shopactivity_mark=0;
		$query_search .= " AND orders.shopactivity_mark in ('{$shopactivity_mark}',2,3) ";
	}
    ...
    if($is_sendorder == 1 || $from_page > 0){
        ...
        if($shop_branch_id!='' && $from_page==1){
        	$query_num.=' AND sso.or_shop_type=2 INNER JOIN weixin_commonshop_order_prices AS op on orders.batchcode = op.batchcode';//子门店的
        }
	...
	if( !empty($_GET['search_status']) ){
		...
		switch($search_status){
			case 0://全部
				$query_search = $query_search." and orders.is_QR=1";
			    //$query_search = $query_search." and orders.supply_id<0 and orders.agent_id<0";
			break;
			case 1://未核销
			    $query_search = $query_search." and orders.status=0 and orders.is_QR=1";
			break;
			case 2://已核销
				$query_search = $query_search." and orders.status=1 and orders.is_QR=1";
			break;
			case 3://即将过期
				$query_qr_select = "SELECT productend_tips FROM weixin_commonshops_extend WHERE customer_id=".$customer_id."";
				$query_qr_res = _mysql_query($query_qr_select) or die('Query_num failed: ' . mysql_error());
				$res_qr = mysql_fetch_assoc($query_qr_res);
				$result_qr=$res_qr['productend_tips']*86400;
				$query_search="
				INNER JOIN weixin_commonshop_products p
				ON orders.pid = p.id
				WHERE p.QR_isforever in (1,2)
				AND orders.status=0
				AND UNIX_TIMESTAMP(now())
				between
				CASE
				WHEN p.QR_isforever = 1 THEN
					UNIX_TIMESTAMP(p.QR_endtime)
				WHEN p.QR_isforever = 2 THEN
					UNIX_TIMESTAMP(orders.paytime) + UNIX_TIMESTAMP(p.QR_endtime) - UNIX_TIMESTAMP(p.QR_starttime) + 1
				END -".$result_qr."
				and
				CASE
				WHEN p.QR_isforever = 1 THEN
	...
	/*郑培强*/
 	$search_integral = 0;//积分兑换产品
    $search_integral_str = "";//拼到链接上的
	if(!empty($_GET["search_integral"])){    //积分兑换产品
		$search_integral = $_GET["search_integral"];
        if($search_integral==1){
            $query_search .= " AND cost_integral>0 ";
            $search_integral_str = "&search_integral=1";
        }

	}

	$query_num .= $query_search;
    //$query_num = "explain ".$query_num;
	...
	$query_order .=  "  ORDER BY orders.id DESC limit ".$start.",".$end;
	// echo $query_order;
	/*订单查询End*/
// if($from_page == 2){
// 	echo $query_order;
...
//查询商城/门店积分名称
$integral_sql = "select basic_json,store_json from ".WSY_SHOP.".integral_setting where cust_id=".$customer_id;
$integral_query = _mysql_query($integral_sql);
while($integral_row =mysql_fetch_object($integral_query)){
	$basic_json = $integral_row->basic_json;
	$store_json = $integral_row->store_json;
}
$basic_arr = json_decode($basic_json,TRUE);
$store_arr = json_decode($store_json,TRUE);
if(!empty($basic_arr['integral_name'])){
    $shop_integral_name = $basic_arr['integral_name'];
}else{
    $shop_integral_name = "商城积分";
}
if(!empty($basic_arr['integral_name'])){
    $store_integral_name = $store_arr['integral_name'];
}else{
    $store_integral_name = "门店积分";
}
...
<link rel="stylesheet" type="text/css" href="/weixinpl/common/css_V6.0/content.css?<?php echo time(); ?>">
...
<script type="text/javascript" src="/mshop/web/static/js/jquery.qrcode.min.js"></script>
...
<script src="/weixinpl/common/region_select.js"></script>
...
<script src="/weixinpl/common/utility.js"></script>
...
.WSY_modifydiv .order_refund dd span textarea{width: 175px !important;}
</style>
</head>
...
<div class="WSY_content" id="WSY_content_height">
	...
	<div class="WSY_columnbox">
    	...
    	<div class="WSY_column_header">
        	<div class="WSY_columnnav">
				...
				<a <?php if($search_class == 0){echo 'class="white1"';} ?> href="order.php?customer_id=<?php echo $customer_id_en;?>&from_page=<?php echo $from_page;?>&shop_branch_id=<?php echo $shop_branch_id;?><?php echo $search_integral_str;?>&f2c_id=<?php echo $f2c_id;?><?php if($shopactivity_mark){echo '&shopactivity_mark='.$shopactivity_mark; }?>">所有订单</a>

			<?php if($from_page <= 0){?>
				<a <?php if($search_class == 0.11){echo 'class="white1"';} ?> href="order.php?customer_id=<?php echo $customer_id_en;?>&search_class=0.11&from_page=<?php echo $from_page;?><?php echo $search_integral_str;?><?php if($shopactivity_mark){echo '&shopactivity_mark='.$shopactivity_mark; }?>"><?php echo $show_menu;?>待签收</a>
            <?php }?>


            <?php if($from_page <= 0){?>
				<a <?php if($search_class == 1){echo 'class="white1"';} ?> href="order.php?customer_id=<?php echo $customer_id_en;?>&search_class=1&from_page=<?php echo $from_page;?><?php echo $search_integral_str;?><?php if($shopactivity_mark){echo '&shopactivity_mark='.$shopactivity_mark; }?>"><?php echo $show_menu;?>待付款</a>
            <?php }?>

            <?php if($from_page > 0 ){?>
                    <!-- 订货和f2c的状态  待处理  -->
                 <a <?php if($search_class == 8){echo 'class="white1"';} ?> href="order.php?customer_id=<?php echo $customer_id_en;?>&search_class=8&from_page=<?php echo $from_page;?>&shop_branch_id=<?php echo $shop_branch_id;?><?php echo $search_integral_str;?>&f2c_id=<?php echo $f2c_id;?><?php if($shopactivity_mark){echo '&shopactivity_mark='.$shopactivity_mark; }?>">待处理</a>
                    <!-- 订货和f2c的状态  待处理  -->
            <?php }?>

				<a <?php if($search_class == 2){echo 'class="white1"';} ?> href="order.php?customer_id=<?php echo $customer_id_en;?>&search_class=2&from_page=<?php echo $from_page;?>&shop_branch_id=<?php echo $shop_branch_id;?><?php echo $search_integral_str;?>&f2c_id=<?php echo $f2c_id;?><?php if($shopactivity_mark){echo '&shopactivity_mark='.$shopactivity_mark; }?>">待发货</a>
				<a <?php if($search_class == 0.5){echo 'class="white1"';} ?> href="order.php?customer_id=<?php echo $customer_id_en;?>&search_class=0.5&from_page=<?php echo $from_page;?>&shop_branch_id=<?php echo $shop_branch_id;?><?php echo $search_integral_str;?>&f2c_id=<?php echo $f2c_id;?><?php if($shopactivity_mark){echo '&shopactivity_mark='.$shopactivity_mark; }?>">已发货</a>
				<a <?php if($search_class == 7){echo 'class="white1"';} ?> href="order.php?customer_id=<?php echo $customer_id_en;?>&search_class=7&from_page=<?php echo $from_page;?>&shop_branch_id=<?php echo $shop_branch_id;?><?php echo $search_integral_str;?>&f2c_id=<?php echo $f2c_id;?><?php if($shopactivity_mark){echo '&shopactivity_mark='.$shopactivity_mark; }?>">待完成</a>
				<a <?php if($search_class == 3){echo 'class="white1"';} ?> href="order.php?customer_id=<?php echo $customer_id_en;?>&search_class=3&from_page=<?php echo $from_page;?>&shop_branch_id=<?php echo $shop_branch_id;?><?php echo $search_integral_str;?>&f2c_id=<?php echo $f2c_id;?><?php if($shopactivity_mark){echo '&shopactivity_mark='.$shopactivity_mark; }?>">交易完成</a>

            <?php if($from_page <= 0){?>
				<a <?php if($search_class == 4){echo 'class="white1"';} ?> href="order.php?customer_id=<?php echo $customer_id_en;?>&search_class=4&from_page=<?php echo $from_page;?><?php echo $search_integral_str;?>">已关闭</a>
				<a <?php if($search_class == 5){echo 'class="white1"';} ?> href="order.php?customer_id=<?php echo $customer_id_en;?>&search_class=5&from_page=<?php echo $from_page;?><?php echo $search_integral_str;?>">未付款删除</a>
				<a <?php if($search_class == 6){echo 'class="white1"';} ?> href="order.php?customer_id=<?php echo $customer_id_en;?>&search_class=6&from_page=<?php echo $from_page;?><?php echo $search_integral_str;?>">已付款删除</a>
            <?php }?>

				<?php }elseif($search_class>=10 ){ ?>
				<a <?php if($search_class == 10){echo 'class="white1"';} ?> href="order.php?customer_id=<?php echo $customer_id_en;?>&search_class=10&from_page=<?php echo $from_page;?><?php echo $search_integral_str;?>">所有售后申请</a>
				<a <?php if($search_class == 11){echo 'class="white1"';} ?> href="order.php?customer_id=<?php echo $customer_id_en;?>&search_class=11&from_page=<?php echo $from_page;?><?php echo $search_integral_str;?>">换货申请</a>
				<a <?php if($search_class == 12){echo 'class="white1"';} ?> href="order.php?customer_id=<?php echo $customer_id_en;?>&search_class=12&from_page=<?php echo $from_page;?><?php echo $search_integral_str;?>">退款申请</a>
				<a <?php if($search_class == 13){echo 'class="white1"';} ?> href="order.php?customer_id=<?php echo $customer_id_en;?>&search_class=13&from_page=<?php echo $from_page;?><?php echo $search_integral_str;?>">退货申请</a>
				<a <?php if($search_class == 14){echo 'class="white1"';} ?> href="order.php?customer_id=<?php echo $customer_id_en;?>&search_class=14&from_page=<?php echo $from_page;?><?php echo $search_integral_str;?>">售后处理完毕</a>
				<?php }else{ ?>

						<a <?php if($search_class == -1){echo 'class="white1"';} ?> href="order.php?customer_id=<?php echo $customer_id_en;?>&search_class=-1&from_page=<?php echo $from_page;?><?php echo $search_integral_str;?>">所有维权申请</a>

						<a <?php if($search_class == -2){echo 'class="white1"';} ?> href="order.php?customer_id=<?php echo $customer_id_en;?>&search_class=-2&from_page=<?php echo $from_page;?><?php echo $search_integral_str;?>">换货申请</a>

						<a <?php if($search_class == -3){echo 'class="white1"';} ?> style="display:none;" href="order.php?customer_id=<?php echo $customer_id_en;?>&search_class=-3&from_page=<?php echo $from_page;?><?php echo $search_integral_str;?>">退款申请</a>
						<a <?php if($search_class == -4){echo 'class="white1"';} ?> href="order.php?customer_id=<?php echo $customer_id_en;?>&search_class=-4&from_page=<?php echo $from_page;?><?php echo $search_integral_str;?>">退货申请</a>
						<a <?php if($search_class == -5){echo 'class="white1"';} ?> href="order.php?customer_id=<?php echo $customer_id_en;?>&search_class=-5&from_page=<?php echo $from_page;?><?php echo $search_integral_str;?>">维权处理完毕</a>
        ...
        <div style="width:100%;display:block;">
			<ul>
         		<li class="WSY_bottonli WSY_bottonli2" style="display:block;float:left;margin-left:20px;">
					...
					<input type="button"  value="新导出飞豆" onClick="export_excel(5);" >
					<input type="button"  value="新导出记录" onClick="export_excel(6);" >
					<input type="button"  value="导出易掌柜" onClick="export_excel(7);" >
					...
					<?php if($from_page < 1){ //F2C系统不要?><a href="<?php echo BaseURL; ?>back_newshops/Order/order/attach/payondelivery.xls" target="_blank" class="payon_btn WSY-skin-bg"  value="回收库存"  >签收模板</a><?php } ?>
				 </li>
			</ul>
    ...
    <div class="WSY_data">
		...
		<div class="WSY_position1" >
			<ul>
				...
				<li class="WSY_position_date tate001" <?php if (!in_array($search_class, array(0, 3, 14, -5))) { echo 'style="display:none;"'; } ?> >
					<p>&nbsp;&nbsp;&nbsp;&nbsp;确认订单时间：<input class="date_picker" type="text" name="confirm_order_time_l" id="confirm_order_time_l" value="<?php echo $confirm_order_time_l; ?>" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm', maxDate:'#F{$dp.$D(\'confirm_order_time_r\')}'});"></p>

					<p>&nbsp;&nbsp;-&nbsp;&nbsp;<input class="date_picker" type="text" name="confirm_order_time_r" id="confirm_order_time_r" value="<?php echo $confirm_order_time_r; ?>" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm', minDate:'#F{$dp.$D(\'confirm_order_time_l\')}'});"></p>
				...
				<li class="WSY_position_text" style="margin-top:13px;margin-left:10px;margin-right: 10px;">
					<a>用户id：<input type="text" name="search_user_id" id="search_user_id" value="<?php echo $search_user_id; ?>"></a>
					...
					<select name="orgin_type" id="orgin_type" style="width:100px;<?php echo $from_page > 0 ? 'display:none':'';?>">
                        ...
                        <option value="5" <?php if($orgin_type==5){ ?>selected<?php } ?>>限购</option>
                        <option value="6" <?php if($orgin_type==6){ ?>selected<?php } ?>>云店</option>
                        <option value="7" <?php if($orgin_type==7){ ?>selected<?php } ?>>品牌订阅</option>
					</select>
                    <?php if($from_page <= 0){ //订货系统不要?>
					<span>订单归属：</span>
                    <?php }?>
					...
					<!--郑培强-->
					<span <?php if($from_page == 2 ){ ?>style="display:none;"<?php } ?>>核销状态：</span>  <!-- f2c系统屏蔽核销产品 -->
					<select name="search_status" id="search_status" <?php if($from_page == 2 ){ ?>style="display:none;"<?php } ?>>
						<option value="0" <?php if($search_status==0){ ?>selected <?php } ?>>全部</option>
						<option value="2" <?php if($search_status==2){ ?>selected <?php } ?>>已核销</option>
						<option value="1" <?php if($search_status==1){ ?>selected <?php } ?>>未核销</option>
						<option value="3" <?php if($search_status==3){ ?>selected <?php } ?>>即将过期</option>
						<option value="4" <?php if($search_status==4){ ?>selected <?php } ?>>已过期</option>
					</select>
					<!--郑培强-->
					<?php
						if($shopactivity_mark){
							?>
							<input type="text" value="<?php echo $shopactivity_mark;?>" name="shopactivity_mark" id="shopactivity_mark" hidden>
							<?php
						}
					?>
				 </li>
				...
				<li class="WSY_position_text" style="margin-top:13px;">
					...
					<!-- <a class="WSY_bottonliss"><input type="button" value="一键清除缓存" onclick="delKey()" style="width: 110px;" /></a> -->
				</li>
			 </ul>
		</div>
    	...
    	<div class="WSY_orderformbox" style="margin-top: 12px;" >
				...
				<p>支付方式：</p>
                	<select name="search_paystyle" id="search_paystyle" >
                    	<option value="" >-- 请选择 --</option>
							...
							<!--<option value="购物币支付" <?php if($search_paystyle=="购物币支付"){ ?>selected <?php } ?>><?php echo defined('PAY_CURRENCY_NAME')? PAY_CURRENCY_NAME: '购物币'; ?>支付</option>-->
							...
							<option value="通联分期支付" <?php if($search_paystyle=="通联分期支付"){ ?>selected <?php } ?>>通联分期支付</option>
                 </select>
            </li>
            
            <li class="WSY_righticon_li01">
				<p>配送方式：</p>
                	<select name="search_sendstyle" id="search_sendstyle" >
                    	<option value="" >-- 请选择 --</option>
							<option value="快递" <?php if($search_sendstyle=="快递"){ ?>selected <?php } ?>>快递</option>
							<option value="自提" <?php if($search_sendstyle=="自提"){ ?>selected <?php } ?>>自提</option>
							<option value="门店自提" <?php if($search_sendstyle=="门店自提"){ ?>selected <?php } ?>>门店自提</option>
                 </select>
            </li>
			...
			<li class="WSY_righticon_li03">
				<p>订货系统仓库级别：</p>
					<select name="store_lev" id="store_lev" style="width:100px;">
						<option value="all" <?php if($store_lev=="all"){ ?>selected<?php } ?>>--请选择--</option>
						<option value="province" <?php if($store_lev=="province"){ ?>selected<?php } ?>>省</option>
						<option value="city" <?php if($store_lev=="city"){ ?>selected<?php } ?>>市</option>
						<option value="area" <?php if($store_lev=="area"){ ?>selected<?php } ?>>区</option>
						<option value="custom_area" <?php if($store_lev=="custom_area"){ ?>selected<?php } ?>>自定义区域</option>
					</select>
            </li>

            <li class="WSY_righticon_li03">
				<a>订货系统门店ID/名称：<input style="border: #ccc 1px solid;border-radius: 2px;width: 100px;height: 22px;line-height: 20px;" type="text" name="search_or_shop_id" id="search_or_shop_id" value="<?php echo $search_or_shop_id; ?>"></a>
				<!-- <a style="padding-left: 20px;">订货系统门店名：<input style="border: #ccc 1px solid;border-radius: 2px;width: 100px;height: 22px;line-height: 20px;" type="text" name="search_or_shop_name" id="search_or_shop_name" value="<?php echo $search_or_shop_name; ?>"></a> -->
			</li>
	...
	<style>
		...
		.CP_table_chanpina_onep{word-break: break-all;}
	...
	<div style="width:100%;overflow:hidden">
		<table width="97%" class="CP_table " id="order_table">
			 <thead class="CP_table_header">
				...
				<th width="10%">发货状态</th>
				<th width="8%">订单状态</th>
				<?php if($shop_branch_id!=''){?>
					<th width="12%">子门店</th>
				<?php } ?>
                <?php if($from_page > 0 && $shop_branch_id==''){ ?>
                    <th width="12%">仓库/门店</th>
				<?php } ?>
				<th width="10%">邀请人</th>
				<th width="15%">操作</th>
			 </thead>
			 ...
			 <?php
				...
				// $redis_key    = md5($query_order);
				// $result_order = unserialize($redis->get($redis_key));
			 	$db           = DB::getInstance();
			 // 	$md5_sql = 'md5_sql'.md5($query_order);
				// $arr_order = redis_get($md5_sql);
				// if ($arr_order == NULL || empty($arr_order) || $arr_order[0]['batchcode'] == '') 
				// {
				// 	$result_order = $db->getAll($query_order);
				// 	redis_set($md5_sql, $result_order, 60);
				// }
				// else
				// {
				// 	$result_order = $arr_order;
				// }
				$result_order = $db->getAll($query_order);
				$batch_order  = array();
				$batch_order  = array();
				...
				{	
					if(!empty($v['batchcode'])     && $v['batchcode'] != -1)      $batch_order[$k]  = $v['batchcode'];     //订单
					if(!empty($v['user_id'])       && $v['user_id'] != -1)        $user_order[$k]   = $v['user_id'];       //用户id
					if(!empty($v['store_id'])      && $v['store_id'] != -1)       $store_order[$k]  = $v['store_id'];    //店铺id
					if(!empty($v['exp_user_id']) && $v['exp_user_id'] != -1)      $exp_order[$k]    = $v['exp_user_id']; //上级id 
					if(!empty($v['agent_id'])      && $v['agent_id'] != -1)       $agent_order[$k]  = $v['agent_id'];      //代理商 
					if(!empty($v['supply_id'])     && $v['supply_id'] != -1)      $supply_order[$k] = $v['supply_id'];     //供应商 
					if(!empty($v['pay_batchcode']) && $v['pay_batchcode'] != -1)  $pay_order[$k]    = $v['pay_batchcode']; //支付单号
 				}


				//zhou 去除循环查询			
				$batch_order_str  = implode("','",$batch_order);
				$user_order_str   = implode(',',$user_order);
				$store_order_str  = implode(',',$store_order);
				$exp_order_str    = implode(',',$exp_order);
				$agent_order_str  = implode(',',$agent_order);
				$supply_order_str = implode(',',$supply_order);
				$pay_order_str    = implode("','",$pay_order);

				
				if(count($batch_order) > 0)
				{	
					$batch_order_str = "'".$batch_order_str."'";
					$pay_order_str   = "'".$pay_order_str."'";
					//查询单笔订单总价格
					$sql1  = "select totalprice,batchcode FROM weixin_commonshop_orders WHERE batchcode in($batch_order_str) AND customer_id=".$customer_id; 
					$res1  = $db->getAll($sql1);
					$res2 = array();
					if($user_order_str != '' )
					{
						//获取下单人的信息
						$sql2  = "select name,weixin_name,id,weixin_fromuser,phone FROM weixin_users WHERE id in ($user_order_str)";
						$res2  = $db->getAll($sql2);
					}
					

					//获取收货地址信息
					$sql3  = "select name,phone,location_p,location_c,location_a,address,identity,identityimgt,identityimgf,batchcode FROM weixin_commonshop_order_addresses WHERE batchcode in($batch_order_str)";
					$res3  = $db->getAll($sql3);

					if(count($store_order) > 0)
					{
						$sql4 = "select name,id from weixin_card_shops where isvalid = true and id in ($store_order_str)";
						$res4 = $db->getAll($sql4);
					}

					/* 消耗积分类型 */
					$sql5   = "select type,batchcode from ".WSY_SHOP.".integral_order_log where batchcode in ($batch_order_str)";
					$res5   = $db->getAll($sql5);

					/*发票信息*/
					$sql6   = "select invoice_head,batchcode from order_invoice_t where batchcode in ($batch_order_str)";
					$res6   = $db->getAll($sql6);

					 /*是否有贺卡祝福语 */
                    if($is_blessing)
                    {
                        $sql7 = "select id,batchcode from weixin_commonshop_order_blessing WHERE isvalid = true and batchcode in ($batch_order_str) and customer_id='{$customer_id}'";
                        $res7 = $db->getAll($sql7);
                    }

                    /* 查询运费 */
					// $sql8   = "select sum(price) as price,batchcode from weixin_commonshop_order_express_prices where isvalid=true and batchcode in ($batch_order_str) group by batchcode"; 
					// $res8   = $db->getAll($sql8); 
					$sql8 = "select ExpressPrice,batchcode from weixin_commonshop_order_prices where batchcode in ($batch_order_str) group by batchcode ";
					$res8 = $db->getAll($sql8);

					$sql9   = "select price,CouponPrice,SupplyCostMoney,ExpressPrice,pay_currency,needScore,O_8reward,shareholder,batchcode from weixin_commonshop_order_prices where isvalid=true and batchcode in ($batch_order_str)";
					$res9   = $db->getAll($sql9); 

					$sql10  = "select totalprice,batchcode,id from weixin_commonshop_changeprices where status=1 and isvalid=1 and batchcode in ($batch_order_str)";
					$res10  = $db->getAll($sql10); 

					if(count($exp_order) > 0)
					{	
						$sql11  = "select name,phone,weixin_name,weixin_fromuser,id from weixin_users where id in ($exp_order_str)";
						$res11  = $db->getAll($sql11); 
					}	

					if(count($agent_order) > 0)
					{	
						/* 代理商 */
						$sql12 = "select name,weixin_name,weixin_fromuser,id FROM weixin_users WHERE id in($agent_order_str)";
						$res12 = $db->getAll($sql12); 

						/* 查看代理商发货方式 */
						$sql13 = "select sendway,user_id from promoters where isvalid=true and customer_id=".$customer_id." and user_id in ($agent_order_str)";
						$res13 = $db->getAll($sql13); 
					}

					/* 供应商 */
					if(count($supply_order) > 0)
					{
						$sql14 = "select name,weixin_name,weixin_fromuser,id FROM weixin_users WHERE id in ($supply_order_str)";

						$res14 = $db->getAll($sql14); 
					}
					...
					{
						//查询该订单是否有改过价
						$sql15 = "select price,batchcode FROM f2c_order_freight where customer_id = {$customer_id} and batchcode in ($batch_order_str) and isvalid = true";
						
						$res15 = $db->getAll($sql15); 
					}

					/* 返现 */
					$sql16 = "select id,batchcode from cashback where batchcode in ($batch_order_str)";
					$res16 = $db->getAll($sql16); 

					$sql17 = "select id,batchcode from cashback_t where batchcode in ($batch_order_str)";
					$res17 = $db->getAll($sql17); 

					//使用优惠券张数
					$sql18 = "select count(id) as coupons_count,batchcode from weixin_commonshop_order_coupons where isvalid=true and batchcode in ($batch_order_str) group by batchcode";
					$res18 = $db->getAll($sql18); 

					/* 产品信息 */
					$sql19 = "select orders2.rcount,orders2.prvalues,orders2.prvalues_name,orders2.totalprice,orders2.isvalid,orders2.merchant_remark,orders2.mb_order,orders2.is_exchange,orders2.need_score,product.name,product.foreign_mark,product.default_imgurl,product.id,product.cost_price,product.tax_type,orders2.batchcode as batchcode FROM weixin_commonshop_orders as orders2 LEFT JOIN weixin_commonshop_products as product on product.id=orders2.pid where orders2.batchcode in ($batch_order_str) and orders2.isvalid=true";
					$res19 = $db->getAll($sql19); 

					foreach($res19 as $k=>$v)
					{
						if(!empty($v['id'])) $pro_pid[$k] = $v['id']; //产品id
					}

					$pro_pid_str = implode(',',$pro_pid);

					if(count($pro_pid) > 0)
					{
						$sql20   = "select imgurl,product_id from weixin_commonshop_product_imgs where isvalid=true and  product_id in ($pro_pid_str)";
						$res20   = $db->getAll($sql20); 
					}	

					$sql21  = "select is_Pinformation from weixin_commonshops_extend where isvalid=true and customer_id=".$customer_id;
					$res21  = $db->getAll($sql21); 
					if($user_order_str != '')
					{
						$sql22  = "select count(1) AS is_promoter,user_id FROM promoters WHERE customer_id='.$customer_id.' AND status=1 AND isvalid=true AND user_id in($user_order_str) group by user_id";
						$res22  = $db->getAll($sql22); 
					}

					//订单日志
					$sql23  = "select operation,descript,operation_user,createtime,batchcode from weixin_commonshop_order_logs where isvalid = true and batchcode in ($batch_order_str)";
					$res23  = $db->getAll($sql23); 
					...
					{	
						...
						$sql24   = "select weixin_name,weixin_fromuser from weixin_users where isvalid = true and weixin_fromuser in ($op_user_str)";
						$res24   = $db->getAll($sql24);
					}	
					
					//获取下单人的信息
					// $sql25  = "select weixin_name,id FROM weixin_users WHERE id in ($user_order_str)";
					// $res25  = $db->getAll($sql25);

					$sql26 = "select images,batchcode from weixin_commonshop_order_rejects where isvalid=true and batchcode in ($batch_order_str)";
					$sql26  = $db->getAll($sql26);

					//支付
					if(count($pay_order) > 0)
					{
						$sql27 = "select callBackBatchcode,price,pay_batchcode from paycallback_t where isvalid=true and pay_batchcode in($pay_order_str)";
						$res27 = $db->getAll($sql27);
					}	
					
					//物流	
					if(count($supply_order) > 0)
					{
						$sql28 = "select id,expresses_name,supply_id FROM weixin_expresses_company where isvalid=true and customer_id = $customer_id and supply_id in ($supply_order_str) order by is_default desc";
						$res28 = $db->getAll($sql28);
					}

					$sql29 = 'select id,expresses_name FROM weixin_expresses_company where isvalid=true and customer_id='.$customer_id." and supply_id=-1 order by is_default desc";
					$res29 = $db->getAll($sql29);
					
					//顺丰
					$sql30 = 'select id FROM sf_import where ison=1 and customer_id='.$customer_id." limit 1";
					$res30 = $db->getAll($sql30);

					//留言
					$sql31 = "select message,createtime,type,sub_supplier_id,batchcode FROM weixin_commonshop_supply_message where isvalid=true and batchcode in ($batch_order_str)";
					$res31 = $db->getAll($sql31);
										  

				}	
				

				foreach ($result_order as $k => $row_order) 
				{	
					$o_batchcode           = $row_order['batchcode'];  //订单				
					$o_is_exchange         = $row_order['is_exchange'];  //是否赠送产品
					...
					$o_backgoods_reason    = addslashes($row_order['backgoods_reason']);	 //(退货/退款)原因
					...
					$yundian_self      	   = $row_order['yundian_self'];		//是否云店自营订单 0-不是 1-是'
					...
					$o_delivery_time_end   = $row_order['delivery_time_end']; //配送时
					...
					$i_cost_integral	   = $row_order['cost_integral']; //兑换产品消耗积分(积分系统使用)
					$o_shopactivity_mark   = $row_order['shopactivity_mark'];
					$o_bs_relate_prod_id   = $row_order['bs_relate_prod_id'];
                    // 派单仓库和代理商
                    $sso_store_id          = $row_order['sso_store_id'];
                    $sso_proxy_id          = $row_order['sso_proxy_id'];
                    $sso_is_accept         = $row_order['sso_is_accept'];
                    $o_yundian_id		   = $row_order['yundian_id'];
                    $o_yundian_self		   = $row_order['yundian_self'];
					$o_block_chain_price   = $row_order['block_chain_price'];//区块链实际支付的金额
                    ...
                    //如果用mysql-sum统计，3笔订单以上，会出现类似119.39999999999999的结果
                    $o_totalprice       = '';
                    foreach($res1 as $k => $v)
                    {
                    	if($o_batchcode == $v['batchcode'])
                    	{	
	                    	$totalprice_1   = $v['totalprice'];
	                    	$o_totalprice   = bcadd($o_totalprice,$totalprice_1,2);
	                    }	
                    }
                    // $query_totalprice   = "select totalprice FROM weixin_commonshop_orders WHERE batchcode='".$o_batchcode."' AND customer_id=".$customer_id;
                    // $result_totalprice  = _mysql_query($query_totalprice) or die('query_totalprice failed: ' . mysql_error());
                    // while ($row_totalprice = mysql_fetch_object($result_totalprice)) 
                    // {
                    //     $totalprice_1   = $row_totalprice->totalprice;
                    //     $o_totalprice   = bcadd($o_totalprice,$totalprice_1,2);
                    // }
					...
					if (count($res2) >0 ) {
						foreach($res2 as $k => $v)
	                    {
	                    	...
	                    	{	
		                    	$user_info          = $v;
								$o_name 			= htmlspecialchars_decode($user_info['name']);	//注册-名称
					...
					foreach($res3 as $k => $v)
                    {
                    	...
                    	{	
		                    ...
		                    $identityimgt       = $address_info['identityimgt'];
		                    $identityimgf       = $address_info['identityimgf'];
	                    }	
                    }
                    ...
                    /* 消耗积分类型 */
                    $cost_integral_type = 0;
                    if(!empty($i_cost_integral) && $i_cost_integral>0)
                    {
						//$query_integral  = "select type from ".WSY_SHOP.".integral_order_log where batchcode = '".$o_batchcode."'";
						// $result_integral = _mysql_query($query_integral) or die('query_integral failed: ' . mysql_error());
						// if($row_integral = mysql_fetch_object($result_integral))
						// {
						// 	$cost_integral_type = $row_integral->type;
						// }
						foreach($res5 as $k => $v)
	                    {
	                    	if($o_batchcode == $v['batchcode'])
	                    	{	
		                    	$cost_integral_type   = $v['type'];
		                    }	
	                    }
						
                    }
                    $integral_name = $shop_integral_name;
                    if($cost_integral_type == 3 || $cost_integral_type == 4)
                    {
                        $integral_name = $store_integral_name;
                    }
					...
					foreach($res6 as $k => $v)
                    {
                    	if($o_batchcode == $v['batchcode'])
                    	{	
	                    	$invoice_head = $v['invoice_head'];
	                    }	
                    }
					/*发票信息*/

                     /*是否有贺卡祝福语 taojin*/
                     if($is_blessing)//若开关开启
                     {
                         // $sql_blessing    = "select id from weixin_commonshop_order_blessing WHERE isvalid = true and batchcode='{$o_batchcode}' and customer_id='{$customer_id}' limit 1";
                         // $result_blessing = _mysql_query($sql_blessing) or die('Query_express failed: ' . mysql_error());
                         // $has_blessing    = false;
                         // $row_blessing    = mysql_fetch_assoc($result_blessing);
                         // if($row_blessing) $has_blessing = true;
					...
					/*  发货状态 */
					$sendstatusstr = "<img src=\"/weixinpl/common/images_V6.0/contenticon/notaffirm-icon.png\" /> <b>未发货</b>";
					switch ($o_sendstatus) {
						case 1:
							$sendstatusstr = "<img src=\"/weixinpl/common/images_V6.0/contenticon/affirm-icon.png\" /> <b style=\"color:#31B0D5\">已发货</b>";
							if ($o_is_delay == 1) {
								$sendstatusstr .= "<span style='color:red'> [申请延迟收货]</span>";
							}
							break;
						case 2:
							$sendstatusstr = "<img src=\"/weixinpl/common/images_V6.0/contenticon/confirm_delivery.png\" /> <b style=\"color:#337AB7\">顾客已收货</b>";
							break;
						case 3:
							$sendstatusstr = "<img src=\"/weixinpl/common/images_V6.0/contenticon/return-goods.png\" /> <b style=\"color:#C9302C\">顾客申请退货</b>";
							if ($o_return_type == 0) {
								$sendstatusstr = "<img src=\"/weixinpl/common/images_V6.0/contenticon/return-money-only.png\" /> <b style=\"color:#C9302C\">申请退货(仅退款)</b>";
							} else if ($o_return_type == 2) {
								$sendstatusstr = "<img src=\"/weixinpl/common/images_V6.0/contenticon/change-goods.png\" /> <b style=\"color:#C9302C\">申请退货(换货)</b>";
							}
						...
						case 4:
							...
							$sendstatusstr = "<img src=\"/weixinpl/common/images_V6.0/contenticon/confirm-return.png\" /> <b style=\"color:#1eaf4e\">退货已确认(" . $rt . ")</b>";
							break;
						case 5:
							$sendstatusstr = "<img src=\"/weixinpl/common/images_V6.0/contenticon/return-money.png\" /> <b style=\"color:#C9302C\">顾客申请退款</b>";
							...
							break;
						case 6:
							$sendstatusstr = "<img src=\"/weixinpl/common/images_V6.0/contenticon/refund-success.png\" /> <b style=\"color:#1eaf4e\">退款完成</b>";
							break;
					}
					/*  发货状态 End */
     ...
     //                }
					foreach($res8 as $k => $v)
                    {
                    	if($o_batchcode == $v['batchcode'])
                    	{	
	                    	$o_express_price = $v['ExpressPrice'];
	                    }	
                    }
					/* 查询运费End */
					...
					// $query_change  = "select price,CouponPrice,SupplyCostMoney,ExpressPrice,pay_currency,needScore,O_8reward,shareholder from weixin_commonshop_order_prices where isvalid=true and batchcode='".$o_batchcode."' limit 1";
					...
					// 	$O_8reward         	   = $row_change->O_8reward;
					// 	$shareholder 		   = $row_change->shareholder;
					// }
					...
					foreach($res9 as $k => $v)
                    {
                    	...
                    	{	
	                    	//获取订单的真实价格（可能是折扣总价）
						    $o_totalprice_last     = $v['price'];
						    $o_CouponPrice         = $v['CouponPrice'];
							$o_SupplyCostMoney     = $v['SupplyCostMoney'];
							$o_SupplyExpressPrice  = $v['ExpressPrice'];
							$pay_currency          = $v['pay_currency'];
							$needScore             = $v['needScore'];
							$O_8reward         	   = $v['O_8reward'];
					...
					foreach($res10 as $k => $v)
                    {
                    	if($o_batchcode == $v['batchcode'])
                    	{	
	                    	$o_totalprice_last = $v['totalprice'];
	                    	$change_id         = $v['id'];
	                    	if($o_totalprice_last>0)
							{
								$changeprice_str = '<span style="color:#dd514c;margin-left: 4px;">(改价后)</span>';
					...
					if($change_id > 0)
                    {
                    	$o_totalprice_last = $o_totalprice_last;
                    }	
                    else
                    {
					...
					if($o_exp_user_id>0)
					{
					 ...
					 //    $result_exp = _mysql_query($query_exp) or die('Query_exp failed: ' . mysql_error());
	                    ...
	                    {
	                    	if($o_exp_user_id == $v['id'])
	                    	{	
		                    	$exp_user_name   = $v['name'];
								$exp_weixin_name = $v['weixin_name'];
								$exp_fromuser    = $v['weixin_fromuser'];
								if(!empty($exp_weixin_name))
								{
									$exp_user_name = $exp_user_name."(".$exp_weixin_name.")";
								}
					...
					if($o_agent_id>0)
					{
						...
						foreach($res12 as $k => $v)
	                    {
	                    	if($o_agent_id == $v['id'])
	                    	{	
		                    	$agent_username        = $v['name'];
								$agent_weixin_fromuser = $v['weixin_fromuser'];
								$agent_weixin_name     = $v['weixin_name'];
								if(!empty($agent_weixin_fromuser))
								{
									$agent_username    = $agent_username."(".$agent_weixin_name.")";
								}
					...
					if($o_agent_id>0)
					{
						...
						foreach($res13 as $k => $v)
	                    {
	                    	if($o_agent_id == $v['user_id'])
	                    	{	
		                    	$p_sendway  = $v['sendway']; //1:代理商自己发货 0:平台发货
		                    }	
	                    }
	                    if($p_sendway==1 and $o_sendway==2 and $o_supply_id<0)
						{
					...
					if($o_supply_id>0)
					{
						...
						foreach($res14 as $k => $v)
	                    {
	                    	if($o_supply_id == $v['id'])
	                    	{	
		                    	$supply_username        = $v['name'];
								$supply_weixin_fromuser = $v['weixin_fromuser'];
								$supply_weixin_name     = $v['weixin_name'];
								if(!empty($supply_weixin_fromuser))
								{
									$supply_username = $supply_username."(".$supply_weixin_name.")";
								}
					...
					$cashback_rows   = 0;
					foreach($res16 as $k => $v)
                    {
                    	if($o_batchcode == $v['batchcode'])
                    	{	
	                    	$cashback_rows = count($v);
	                    }	
                    }
			 ...
			 <tr class="CP_table_bianhao" >
				...
				<td class="CP_table_bianhaoa" colspan="<?php echo $from_page > 0 ? 9 : 8 ;?>">
					...
					<img style="width:18px;height:18px;margin-left:2px" src="/weixinpl/common/images_V6.0/contenticon/dai.png" ondragstart="return false;" title="代理商订单" />
					<?php }
					if($o_supply_id>0){?>
					<img style="width:18px;height:18px;margin-left:2px" src="/weixinpl/common/images_V6.0/contenticon/gong.png" ondragstart="return false;" title="合作商订单" />
					<?php }
					if($o_yundian_id > 0 || $o_yundian_self == 1){?>
					<img style="width:40px;height:20px;margin-left:2px" src="/resources/yundian/yundian.png" ondragstart="return false;" title="云店订单" />
					<?php }
					if($o_shopactivity_mark == 3 || $o_bs_relate_prod_id != 0){?>
                        <img style="width:40px;height:20px;margin-left:2px" src="/resources/dingyue/dingyue.png" ondragstart="return false;" title="品牌订阅" />
                    <?php }
					if($o_is_QR==1){   ?>
					<img style="width:18px;height:18px;margin-left:2px" src="/weixinpl/common/images_V6.0/contenticon/coupon.png" ondragstart="return false;" title="二维码核销订单" />
					<?php }
					if($cashback_rows>0){?>
					...
					<img style="width:18px;height:18px;margin-left:2px" src="/weixinpl/common/images_V6.0/contenticon/pre_delivery.png" ondragstart="return false;" title="预配送订单" />
					<?php }
					if( $o_is_collageActivities ==1 || $o_is_collageActivities ==2){?>
					<img style="width:18px;height:18px;margin-left:2px" src="/weixinpl/common/images_V6.0/contenticon/is_head.png" ondragstart="return false;" title="拼团订单" />
					<?php } if( $o_shopactivity_mark == 1 ){?>
					<img style="width:18px;height:18px;margin-left:2px" src="/weixinpl/common/images_V6.0/contenticon/is_bargain.png" ondragstart="return false;" title="砍价订单" />
					<?php } if( $o_shopactivity_mark == 2 ){?>
					<img style="width:18px;height:18px;margin-left:2px" src="/weixinpl/common/images_V6.0/contenticon/is_crowdfund.png" ondragstart="return false;" title="众筹订单" />
					<? } if($from_page == 0 and $o_is_sendorder == 2 ){?>
					<img style="width:18px;height:18px;margin-left:2px" src="/weixinpl/common/images_V6.0/contenticon/zhuan.png" ondragstart="return false;" title="F2C拒单" />
					<?php } ?>
					<span class="zhibo mb_order_<?php echo $o_batchcode;?>" style="display:none;" ondragstart="return false;">直播</span>
					<!-- <img class="mb_order_<?php echo $o_batchcode; ?>" style="width:20px;height:20px;margin-left:2px;display:none;" src="/weixinpl/common/images_V6.0/contenticon/zb_icon.png" ondragstart="return false;" title="直播订单" /> -->
					<?php
					if($total_tax_type>1){?>
					...
					<?php
					if($o_CouponPrice>0){ ?>
					<span><img style="margin-right: -20px;" src="/weixinpl/common/images_V6.0/contenticon/pay-discount.png" /></span>
					<?php } ?>
					<span id="order_pay_<?php echo $o_batchcode; ?>" >
					<?php
					if($o_paystatus==0 ){ ?>
					<img src="/weixinpl/common/images_V6.0/contenticon/del-icon.png" /><span class="CP_table_bianhaoe">未支付</span>
					<a title="催单" style="cursor:pointer;" onclick="callPay('<?php echo $o_batchcode; ?>',<?php echo $o_totalprice_last; ?>)" ><img style="width:16px;height:18px;" src="/weixinpl/common/images_V6.0/contenticon/callback.png" /></a>
					<?php }elseif($o_paystatus==1 ){ ?>
					<img src="/weixinpl/common/images_V6.0/contenticon/pay-icon.png" /><span class="CP_table_bianhaof">已支付<?php if($o_Pay_Method==1){?><span style="color:red;">(后台支付)</span><?php }?></span>
				...
				if($o_paystatus==1 and $o_Pay_Method==0){
					$transaction_id=-1;
					if($o_paystyle=="通联支付"){
						echo "[<a href=\"allipay_detail.php?allipay_orderid=$o_allipay_orderid&from_page={$from_page}\">". $o_allipay_orderid ."(点击查看)</a>]";
					}elseif($o_paystyle=="支付宝支付"){
                        $alipaySql = "select pay_batchcode,transaction_id from system_order_pay_log where   pay_batchcode='".$pay_batchcode."' limit 0,1";
                        $result_alipaySql = _mysql_query($alipaySql) or die('paySql failed: ' . mysql_error());
                        while ($row_alipaySql = mysql_fetch_object($result_alipaySql)) {
                            $transaction_id = $row_alipaySql->transaction_id;
                            $pay_batchcode = $row_alipaySql->pay_batchcode;
                            break;
                        }
                        echo "[<a href=\"alipay_rsa_detail.php?pay_batchcode=$pay_batchcode&batchcode=$o_batchcode&from_page={$from_page}\">". $transaction_id ."(点击查看)</a>]";
                    }elseif($o_paystyle=="通联分期支付"){
                        $alipaySql = "select pay_batchcode,transaction_id from system_order_pay_log where pay_batchcode='".$pay_batchcode."' limit 0,1";
                        $result_alipaySql = _mysql_query($alipaySql) or die('paySql failed: ' . mysql_error());
                        while ($row_alipaySql = mysql_fetch_object($result_alipaySql)) {
                            $transaction_id = $row_alipaySql->transaction_id;
                        ...
                        echo "[<a href=\"allinpay_rsa_detail.php?pay_batchcode=$pay_batchcode&batchcode=$o_batchcode&from_page={$from_page}\">". $transaction_id ."(点击查看)</a>]";
						...
						// if($wxpay_version==2){
							echo "[<a href=\"weipay_detail.php?allipay_orderid=".$transaction_id."&pay_batchcode=".$pay_batchcode."&batchcode=$o_batchcode&from_page={$from_page}\">". $transaction_id ."(点击查看)</a>]";
						// }else{
							// echo "[<a href=\"weipay_detail2.php?pay_batchcode=".$pay_batchcode."&batchcode=$o_batchcode&from_page={$from_page}\">". $transaction_id ."(点击查看)</a>]";
						// }
						...
						{
							if($pay_batchcode == $v['pay_batchcode'])
							{
						...
						echo "[<a href=\"hxpay_detail.php?pay_batchcode=$pay_batchcode&batchcode=$o_batchcode&from_page={$from_page}\">". $callBackBatchcode ."(点击查看)</a>]";
					}elseif( $o_paystyle=="威富通支付" ){
						//$settlementprice = 0;
						$wftpay = "select transaction_id,wft_type,real_pay_price from system_order_pay_log where pay_batchcode='".$pay_batchcode."'";
						...
						while ($row_result_weipay = mysql_fetch_object($result_wftpay)) {
							...
							$settlementprice = $row_result_weipay->real_pay_price;
						}

						// $refund_sql = "select refund from weixin_commonshop_refunds where batchcode='".$batchcode."'";
						// $result_refund = _mysql_query($refund_sql) or die('Query_weipay failed: ' . mysql_error());
						// while ($result_refund = mysql_fetch_object($result_refund)) {
						// 	$settlementprice = $result_refund->refund;
						// }
						//echo $wft_type;
						echo "[<a href=\"wftpay_detail.php?allipay_orderid=".$transaction_id."&wft_type=".$wft_type."&pay_batchcode=".$pay_batchcode."&batchcode=$o_batchcode&from_page={$from_page}\">". $transaction_id ."(点击查看)</a>]";
                        ...
                        echo "[<a href=\"healthpay_detail.php?pay_batchcode=$pay_batchcode&batchcode=$o_batchcode&from_page={$from_page}\">". $callBackBatchcode ."(点击查看)</a>]";
						...
						echo "[<a href=\"yeepay_detail.php?pay_batchcode=$pay_batchcode&batchcode=$o_batchcode&from_page={$from_page}\">". $callBackBatchcode ."(点击查看)</a>]";
						...
						echo "[<a href=\"jdpay_detail.php?pay_batchcode=$pay_batchcode&batchcode=$o_batchcode&from_page={$from_page}\">". $callBackBatchcode ."(点击查看)</a>]";
					}else{
						$settlementprice=0; //可以结算的金额
						// $paySql = "select callBackBatchcode,price from paycallback_t where isvalid=true and pay_batchcode='".$pay_batchcode."' limit 0,1";
						// $result_paySql = _mysql_query($paySql) or die('paySql failed: ' . mysql_error());
						...
						echo "[<a href=\"pay_detail.php?pay_batchcode=$pay_batchcode&from_page={$from_page}\">". $callBackBatchcode ."(点击查看)</a>]";
					}
				}
				?>
				</span>
			...
			/* 产品信息 */
				...
				$o2_tax_type 		= 1;
				$o2_cost_price      = 0;//供货价
				$o2_foreign_mark    = '';//外部标识

				// $query_order2       = "SELECT orders2.rcount,orders2.prvalues,orders2.prvalues_name,orders2.totalprice,orders2.isvalid,orders2.merchant_remark,orders2.mb_order,orders2.is_exchange,orders2.need_score,product.name,product.foreign_mark,product.default_imgurl,product.id,product.cost_price,product.tax_type FROM weixin_commonshop_orders as orders2 LEFT JOIN weixin_commonshop_products as product on product.id=orders2.pid where orders2.batchcode='".$o_batchcode."' and orders2.isvalid=true";
				...
				// 	$o2_is_exchange   	= $row_order2->is_exchange;
				...
				// 	$o_need_score_sum   += $row_order2->need_score;
				// 	/* 产品图片 */
				// 	if(empty($o2_imgurl))
				// 	{
				...
				// 		if(empty($o2_imgurl))
				// 		{
				// 			$o2_imgurl = "/weixinpl/common/images_V6.0/contenticon/pic_miss.png";
				// 		}
				// 	}

				$o2_rows           = 0;
				$o_need_score_sum  = 0;
				foreach($res19 as $k => $row_order2)
			    {
			    	if($o_batchcode == $row_order2['batchcode'])
			    	{
			    		$o2_rows++;
			    	}
			    }	
				foreach($res19 as $k => $row_order2)
			    {
			    	...
			    	{	
						...
						if(empty($o2_imgurl))
						{
							...
							foreach($res20 as $k => $v)
						    {
						    	if($o2_pid == $v['product_id'])
						    	{
						    		$o2_imgurl  = $v['imgurl'];
						    	}
						    }
						...
						*/
						...
						//直接读数据库即可
						$o2_prvstr         = $o2_prvalues_name; //属性
						if(empty($o2_foreign_mark))
						{
							$query_prod2   = "select foreign_mark from weixin_commonshop_product_prices where product_id='".$o2_pid."' and proids='".$o2_prvalue."'";
							$result_prod2  = _mysql_query($query_prod2) or die('Query_prod2 failed: ' . mysql_error());
							while ($row_prod2 = mysql_fetch_object($result_prod2)) 
						...
						foreach($res21 as $k => $v)
					    {
					    	$is_Pinformation  = $v['is_Pinformation'];
					    }
						/*必填信息开关end*/
						...
						foreach($res21 as $k => $v)
					    {	
					    	if($buy_user_id == $v['user_id'])
					    	{	
					    		$is_promoter = $v['is_promoter'];
					    	}	
					    }
						/*是否推广员身份*/
						...
						<td class="CP_table_chanpina_one">
							...
							<span class="CP_table_chanpina_onep">
								<p><b><?php echo htmlspecialchars($o2_nane); ?></b> <?php if( $o2_is_exchange ) { ?><b class='ex-tip'>赠</b><?php } ?> </p>
								<p><b><?php if(OOF_P != 2) echo OOF_S ?><?php echo number_format($o2_totalprice,2); ?><?php if(OOF_P == 2) echo OOF_S ?></b><span class="CP_table_chanpina_onepa"> 数量：<b><?php echo $o2_rcount; ?></b></span></p>
								<p>
									<span class="CP_table_chanpina_onepa">【属 性】：<?php echo htmlspecialchars($o2_prvstr); ?> </span>
									</br><span class="CP_table_chanpina_onepa">【外部标识】：<?php echo htmlspecialchars($o2_foreign_mark); ?> </span>
								</p>
								<!-- <p><b><?php if( $o2_is_exchange ) echo '赠'; ?></b></p> -->
								<!-- 必填信息 开始 -->
								<?php
								...
								<div>
									<p><b>必填信息</b></p>
									<div class="mess_b_<?php echo $o_batchcode;?>" style="max-height: 48px;overflow: hidden;"><!-- 必填信息框架 -->
										<?php
									...
									if( 2 < $row_m ){
									?>
									<span id="div1_<?php echo $o_batchcode;?>" val='1' onClick="showMore('<?php echo $o_batchcode;?>')" style="cursor:pointer">更多</span>
									<?PHP } ?>
								</div>
								<?php } ?>
							</span>
						...
						<td class="CP_table_chanpina_two" rowspan="<?php echo $o2_rows; ?>">
							<p><?php 
				                ...
				                $o_weixin_name = emoji_softbank_to_unified($o_weixin_name);
				                //$weixin_name = emoji_unified_to_html($weixin_name);
				                //$weixin_name = emoji_html_to_unified($weixin_name);
								echo htmlspecialchars($o_expressName)."(".htmlspecialchars(strip_tags($o_weixin_name)).")"; ?>  
							<a title="微信对话" href="/weixinpl/weixin_inter/send_to_msg.php?fromuserid=<?php echo $o_weixin_fromuser; ?>&customer_id=<?php echo $customer_id_en; ?>" ><i class="order-comment"></i></a>
							</p>
						...
						<td class="CP_table_chanpina_three" <?php if($from_page == 2){ echo "style = 'text-align: center;'"; } ?> id="table_three_<?php echo $o_batchcode; ?>" rowspan="<?php echo $o2_rows; ?>">
							<b><?php echo number_format($o_totalprice_last,2)."<?php echo OOF_T ?>".$changeprice_str; ?></b><?php if($o_pay_currency>0){ echo "<br/><span>购物币：". $o_pay_currency ."</span>"; } ?><br/><span><?php if($o_express_price>0){ echo "(含运费 ". $o_express_price ."<?php echo OOF_T ?>)"; }else{ echo "免邮"; } ?></span>
							<?php if($o2_tax_type>1){?>
							</br><span>(含<?php echo $tax_name;?>税：<?php echo $total_tax;?><?php echo OOF_T ?>)</span>
							<?php }?>
						</td>
						...
						<?php
						    if($from_page > 0){
						        ...
						        if($from_page == 1){ //订货系统
									//查询零售负责人
						            $query_proxy = "select name, phone from orderingretail_proxy where id = ".$sso_proxy_id;
									//查询零售负责人 End
									//查询区域
						            $query_area = "select a.grade,a.areaname from orderingretail_area_set s inner join weixin_commonshop_team_area a on s.area_id = a.id  and s.id = ".$sso_store_id;
						            // $query_area = "select a.grade,a.areaname from orderingretail_area_set s inner join weixin_commonshop_team_area a on s.area_id = a.id where s.proxy_id = ".$sso_proxy_id." and s.isvalid=true";
									//查询区域 End
						            //查询订单系统门店
						            $query_or_shop = "select id,shop_name,shop_tel from orderingretail_shop where customer_id=".$customer_id." and proxy_id=".$sso_proxy_id." and isvalid=true and is_freeze=false";
						        }else{
									//查询零售负责人
								...
								if($from_page==1)
								{
						            $res_or_shop      = _mysql_query($query_or_shop) or die(" sso query or_shop error : ".mysql_error());
						            if($row_or_shop   = mysql_fetch_object($res_or_shop))
						            {
						                $or_shop_name = $row_or_shop -> shop_name;
						                $or_shop_tel  = $row_or_shop -> shop_tel;
						                $or_shop_id   = $row_or_shop -> id;
						                $is_or_shop  = 1;
						            }
						        }
						        ...
						        $proxy_name      = empty($proxy_name)? "无" : $proxy_name;
						        $proxy_phone     = empty($proxy_phone)? "无" : $proxy_phone;
						    ...
						    ?>
						    	<td class="CP_table_chanpina_five" id="table_five_<?php echo $o_batchcode; ?>" rowspan="<?php echo $o2_rows; ?>">
									子门店名称-<?php echo $branch_name; ?><br/>
									手机号-<?php echo $apply_phone_branch; ?><br/>
						            名称-<?php echo $apply_name_branch; ?>
						        </td>
						    <?php
									}else{
						    ?>
							<td class="CP_table_chanpina_five" id="table_five_<?php echo $o_batchcode; ?>" rowspan="<?php echo $o2_rows; ?>">
								订货商名称-<?php echo $proxy_name; ?>；手机号-<?php echo $proxy_phone; ?> <br/>
						        仓库区域-<?php echo $store_areaname; ?>；仓库级别-<?php echo $store_grade_name; ?> <br/>
						        <!--门店信息-->
								门店ID-<?php echo $or_shop_id; ?>；门店名-<?php echo $or_shop_name; ?> <br/>
						        联系电话-<?php echo $or_shop_tel; ?></td>
							<?php }} else{ ?>
						    <td class="CP_table_chanpina_five" id="table_five_<?php echo $o_batchcode; ?>" rowspan="<?php echo $o2_rows; ?>">
						    	订货商名称-<?php echo $proxy_name; ?>；手机号-<?php echo $proxy_phone; ?> <br/>
						        仓库区域-<?php echo $store_areaname; ?>；仓库级别-<?php echo $store_grade_name; ?> </td>
						<?php } }?>
							...
							<?php
								...
								else
								{
									...
									if($o_exp_user_id>0)
									{
										...
										$query_id = "SELECT id FROM promoters WHERE user_id = '{$o_exp_user_id}' AND isvalid = 1 AND status = 1 limit 0,1";
										$result_id = _mysql_query($query_id);
										$promoters_id = '';
										while ($row_id = mysql_fetch_object($result_id)) 
										{
											$promoters_id = $row_id -> id;
										}
										if ($promoters_id == '') 
										{
											echo "<p>无</p>";
										...
										{
											echo '<p>推广员:<a title="推广员" href="../../Users/promoter/promoter.php?search_user_id=' . $o_exp_user_id . '&customer_id=' . $customer_id_en . '">' . $exp_user_name . '</a> <a title="微信对话" href="/weixinpl/weixin_inter/send_to_msg.php?fromuserid=' . $exp_fromuser . '&customer_id=' . $customer_id_en . '"><i class="order-comment"></i></a></p>'; 
										}
									...
									if($o_agent_id>0)
									{ 
										echo '<p>代理商:<a title="代理商" href="../../Mode/agent/agent.php?search_user_id=' . $o_agent_id . '&customer_id=' . $customer_id_en . '">' . $agent_username . '</a> <a title="微信对话" href="/weixinpl/weixin_inter/send_to_msg.php?fromuserid=' .$agent_weixin_fromuser. '&customer_id=' . $customer_id_en . '"><i class="order-comment"></i></a></p>';
									}
									if($o_supply_id>0)
									{
										...
										if(!empty($supply_weixin_fromuser))
										{
											$p_str .= '<a title="微信对话" href="/weixinpl/weixin_inter/send_to_msg.php?fromuserid=' .$supply_weixin_fromuser. '&customer_id=' . $customer_id_en . '"><i class="order-comment"></i></a>';
										}
										$p_str .= '</p>';
										echo $p_str; 
									}
						...
						<td class="CP_table_chanpina_six" rowspan="<?php echo $o2_rows; ?>">
							<a title="订单详情" onclick="showDetail('<?php echo $o_batchcode; ?>')" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon44.png" /></a>
							<a title="订单日志" onclick="showLog('<?php echo $o_batchcode; ?>')" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon11.png" /></a>
							<?php
						if($o2_isvalid==1){
							if($o_sendstatus==1){
							?>
							<a title="延期收货" onclick="showDate('<?php echo $o_batchcode; ?>',<?php echo $o2_rows; ?>)" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon53.png" /></a>
						    <?php //添加条件：and ($search_integral != 1 || $o_is_sendorder != 1) 积分兑换的派单订单不显示该操作 djy ?>
							<?php  }   if($o_status==0 and $o_sendstatus==0 and ($from_page == 0 || $from_page == 2) and ($search_integral != 1 || $o_is_sendorder != 1)){ ?>
							<a class="change_add_<?php echo $o_batchcode; ?>" title="修改收件地址" onclick="showAddress('<?php echo $o_batchcode; ?>',<?php echo $o2_rows; ?>)" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon52.png" /></a>
							...
							<?php }
						        ...
						        <a title="修改分担运费" class="change_freight_<?php echo $o_batchcode; ?>"  onclick="editFreight('<?php echo $o_batchcode; ?>')" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon05.png" /></a>

						     <?php } ?>

						    <?php 
							if($o_status==0 and $o_paystatus==0 and $o_sendstatus ==0){
								if($o_agentcont_type==0 and $o_supply_id<0){
							?>
								<a title="修改价格" onclick="showPrice('<?php echo $o_batchcode; ?>',<?php echo $o2_rows; ?>)" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon05.png" /></a>
							<?php }
							} ?>
							<?php 
							...
							<a title="确认支付" data-batchcode="<?php echo $o_batchcode;?>" data-totalprice="<?php echo $o_totalprice_last;?>" onclick="payOrder(this)" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon39.png" /></a>

							<?php }
						    //添加条件：and ($search_integral != 1 || $o_is_sendorder != 1) 积分兑换的派单订单不显示该操作 djy
							if($o_sendstatus==0 and ($o_paystatus==1 or $o_paystyle=="货到付款") and $o_supply_id<0 and $o_open_sendway==0 and $from_page == 0 and ($search_integral != 1 || $o_is_sendorder != 1)){ ?>

							<a id="button_print_<?php echo $o_batchcode; ?>" title="打印快递运单" class="print_delivery" style="display:none;"  onclick="print_delivery('<?php echo $o_batchcode; ?>')" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon29.png" /></a>

							<a id="button_delivery_<?php echo $o_batchcode; ?>" title="发货"  onclick="showDelivery('<?php echo $o_batchcode; ?>')" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon42.png" /></a>

							<?php }

						    if( $o_paystatus==1 and  $o_sendstatus == 1){ ?>

						        <a id="change_courier_number_<?php echo $o_batchcode; ?>" title="修改快递单号" class="change_courier_number"   onclick="change_courier_number('<?php echo $o_batchcode; ?>')" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon52.png" /></a>

						    <?php }

							if($o_status==0 and $o_paystatus==1 and ($o_sendstatus==2 or $o_sendstatus==4 or $o_sendstatus==6) ){ ?>

							<a title="确认完成" class="confir" data-batchcode="<?php echo $o_batchcode;?>" data-totalprice="<?php echo $o_totalprice_last;?>" onclick="confirmOrder(this)" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon23.png" /></a>


							<?php if($o_sendstatus!=4 and $o_sendstatus!=6){?>

							<a title="红包确认" class="red_<?php echo $o_batchcode; ?>" href="order_send_redpack.php?customer_id=<?php echo $customer_id_en; ?>&batchcode=<?php echo $o_batchcode; ?>" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon55.png" /></a>

							<?php }} ?>

							<?php if($o_aftersale_state == 1){ ?>

								<a title="维权管理" data-batchcode="<?php echo $o_batchcode;?>"  onclick="returnAftersale(this)" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon58.png" /></a>

							<?php }else if($o_aftersale_state == 2){ ?>

								<a title="确认维权完毕" data-batchcode="<?php echo $o_batchcode;?>" onclick="confirmAftersale(this)" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon59.png" /></a>

							<?php } ?>

							<?php if($o_aftersale_state==4 && $o_supply_id>0 && !$o_isreducesupply){?>
								<a title="扣除合作商款项" class="reducesupply_btn" data-reduce-supply-batchcode="<?php echo $o_batchcode;?>"  onclick="reducesupply('<?php echo $o_batchcode;?>')" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon57.png" /></a>
							<?php }?>

							<!--确认签收和拒绝签收-->
							<?php if($o_payondelivery == 1 && $o_is_sign == 0){ ?>
							<a title="确认签收" data-batchcode="<?php echo $o_batchcode;?>" data-totalprice="<?php echo $o_totalprice_last;?>" onclick="sign_yes(this,1)" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon80.png" /></a>

							<a title="拒绝签收" data-batchcode="<?php echo $o_batchcode;?>" data-totalprice="<?php echo $o_totalprice_last;?>" onclick="sign_yes(this,2)" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon81.png" /></a>
							<?php } ?>

							<?php
						    //添加条件：and ($search_integral != 1 || $o_is_sendorder != 1) 积分兑换的派单订单不显示该操作 djy
							if($o_sendstatus==3 and $o_supply_id<0 and $from_page == 0 and $o_open_sendway==0 and ($search_integral != 1 || $o_is_sendorder != 1)){

								if($o_return_status == 0){  //申请退货后审批

								?>
								<a title="退货管理" data-batchcode="<?php echo $o_batchcode;?>" data-reason="<?php echo $o_backgoods_reason;?>"  onclick="returnGood(this,<?php echo $o_return_type;?>)" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon56.png" /></a>
								<?php
									...
									?>
									<a title="确定退款" data-refund-batchcode="<?php echo $o_batchcode;?>"  onclick="showGoodRefund('<?php echo $o_batchcode; ?>',1,this)" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon57.png" /></a>
									<?php

									}else if($o_return_type == 2){ // 申请换货并且商家已同意

									?>
									<a title="确定已退货" data-batchcode="<?php echo $o_batchcode;?>" onclick="confirmGoodRefund(this)" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon56.png" /></a>
									<a id="button_delivery_<?php echo $o_batchcode; ?>" title="发货" onclick="showDelivery('<?php echo $o_batchcode; ?>')" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon42.png" /></a>
									<?php
									...
									?>
									<a title="确定已退货" data-refund-all-batchcode="<?php echo $o_batchcode;?>" onclick="showGoodAll('<?php echo $o_batchcode;?>')" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon56.png" /></a>
									<?php
									...
									?>
									<a title="确定已收到退货" data-batchcode="<?php echo $o_batchcode;?>" onclick="confirmGoodRefund(this)" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon56.png" /></a>
									<a id="button_delivery_<?php echo $o_batchcode; ?>" title="发货" onclick="showDelivery('<?php echo $o_batchcode; ?>')" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon42.png" /></a>
									<?php
									...
									?>
									<a title="确定退款" data-refund-batchcode="<?php echo $o_batchcode;?>"  onclick="showGoodRefund('<?php echo $o_batchcode; ?>',2,this)" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon57.png" /></a>
									<?php
						            ...
						            if($o_return_type == 1 ) { //退货
						                ...
						                <a title="确定退款" data-refund-batchcode="<?php echo $o_batchcode;?>"
						                   onclick="showGoodRefund('<?php echo $o_batchcode; ?>',2,this)"><img
						                        src="/weixinpl/common/images_V6.0/operating_icon/icon57.png"/></a>
						            <?php
						            }
								}
							}
							...
							if($o_sendstatus==5){
						        //添加条件：and ($search_integral != 1 || $o_is_sendorder != 1) 积分兑换的派单订单不显示该操作 djy
								if($o_return_status == 0 && $o_supply_id < 0 && $from_page <= 0 and ($search_integral != 1 || $o_is_sendorder != 1)){  //申请退款后审批  18-4-2 whl 供应商没有管理

								?>
								<a title="退款管理" data-batchcode="<?php echo $o_batchcode;?>"  onclick="returnMoney(this)" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon56.png" /></a>
								<?php

								}else if($o_return_status == 8){ //退款

								?>
								<a title="确定退款" data-refund-batchcode="<?php echo $o_batchcode;?>"  onclick="showGoodRefund('<?php echo $o_batchcode; ?>',0,this)" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon57.png" /></a>
								<?php
								...
								?>
								<a title="删除推广员身份" data-refund-batchcode_id="<?php echo $o_batchcode;?>"  onclick="deletePromoter(<?php echo $buy_user_id;?>)" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon25.png" /></a>
								<?
								}
							}
							...
							?>
							<a class="shanchu" title="删除" data-batchcode="<?php echo $o_batchcode;?>"  onclick="delOrder(this)" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon04.png" /></a>
							<?php
								}
							}
						}?>
						...
						<a title="返佣记录" href="order_rebate_log.php?batchcode=<?php echo $o_batchcode; ?>&customer_id=<?php echo $customer_id_en;?>=&class=1&from_page=<?php echo $from_page; ?>" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon51.png" /></a>
						<?php }?>

						<?php if($cashback_rows>0){?>
						<a title="赠送记录" href="order_cashback_log.php?batchcode=<?php echo $o_batchcode; ?>&customer_id=<?php echo passport_encrypt($customer_id)?>&type=1"><img src="/weixinpl/common/images_V6.0/operating_icon/icon72.png" /></a>
						<?php }?>

						<?php if($is_blessing and $has_blessing and $o_status>=0 and $o_paystatus==1 and $o_sendstatus>=0 ){ ?>
						    <a title="贺卡二维码" data-batchcode="<?php echo $o_batchcode; ?>" id="show_blessing" href="/mshop/web/index.php?m=order_blessing&a=download&batchcode=<?php echo $o_batchcode; ?>&customer_id=<?php echo passport_encrypt($customer_id)?>"><img src="/weixinpl/common/images_V6.0/operating_icon/icon71.png" /></a>
						    <!--                     <div class="blessing_img" style="position:relative;z-index: 999"></div>-->
						<?php } ?>
						</td>
                	...
                	<dl class="order_dl01">
                        ...
                        <div class="order_div">
                           ...
                           <!-- <dd><b>合作商：</b><span><?php echo $supply_username; ?></span></dd> -->
							...
							<?php if($o_CouponPrice>0){?><dd><b>优惠券：</b><span class="WSY_red"><?php  echo  $o_CouponPrice."<?php echo OOF_T ?>";?></span></dd><?php }?>
							<?php if($o_store_id>0){ ?>
							...
							<?php
										if($o_express_price>0) echo  $o_express_price."<?php echo OOF_T ?>";else echo "免邮";
							...
							<?php
									echo  "<?php if(OOF_P != 2) echo OOF_S ?>".$total_tax."<?php if(OOF_P == 2) echo OOF_S ?>";
                        ...
                        <div class="order_div">
								<dd><b>订单金额：</b><span class="WSY_red"><?php echo $o_totalprice; ?><?php echo OOF_T ?></span></dd>
								<dd><b>总优惠金额：</b><span class="WSY_blue"><?php echo $o_discount_price; ?><?php echo OOF_T ?></span></dd>
								<dd><b>使用<?php echo defined('PAY_CURRENCY_NAME')? PAY_CURRENCY_NAME: '购物币'; ?>：</b><span><?php echo $o_pay_currency; ?></span></dd>
								<dd><b style="width: 130px;">使用商城/门店积分：</b><span><?php echo bcadd($i_cost_integral,0,2); ?></span></dd>
                                ...
                                <dd><b style="width: 110px;">产品兑换积分：</b><span><?php echo $o_needScore; ?></span></dd>
                                <dd><b style="width: 110px;">复购推广优惠金额：</b><span><?php echo bcadd($O_8reward,0,2); ?></span></dd>
                                <dd><b style="width: 110px;">复购店铺优惠金额：</b><span><?php echo bcadd($shareholder,0,2); ?></span></dd>
								<dd><b>实付金额：</b><span class="WSY_red" id="order_price_<?php echo $o_batchcode; ?>"><?php echo number_format($o_totalprice_last,2); ?><?php echo OOF_T ?></span></dd>
                        </div>
                    </dl>
                    <dl class="order_dl02">
                        <form>
                        ...
                        <div class="order_div01">
                            ...
                            <dd><b>订单备注：</b><span class="order_span_break" ><?php if($o_remark!=""){echo $o_remark;} else{echo "无";} ?></span></dd>
                        ...
                        <dd class="order_dd_hidden_<?php echo $o_batchcode; ?>" style="max-height: 150px;overflow-y: auto;width: 98%;display: inline-block;">
							<?php
							   ...
							   // while ($row_msg = mysql_fetch_object($result_msg)) {
								...
								foreach ($res31 as $k => $row_msg) 
								{
									if($o_batchcode == $row_msg['batchcode'])
									{
										$msg_message     = $row_msg['message'];
									    $msg_createtime  = $row_msg['createtime'];
									    $msg_type        = $row_msg['type'];
										   ...
										   //  }
										    ...
										    // $result_express = _mysql_query($query_express) or die('Query_express failed: ' . mysql_error());
										 	// while ($row_express = mysql_fetch_object($result_express)) {
												...
												// if($o_express_id == $e_id) $ee_name = $e_name;

										    foreach ($result_express as $k => $v) 
										    {	
										    	$e_id            = $v['id'];
											  	$e_name          = $v['expresses_name'];
											  	if($o_express_id == $e_id) $ee_name = $e_name;
										    	if($o_supply_id == $v['supply_id'] && $exp_id == 1)
										    	{
										    		$e_id            = $v['id'];
												  	$e_name          = $v['expresses_name'];
												  	if($o_express_id == $e_id) $ee_name = $e_name;
                ...
                if($o_status==0 and $o_sendstatus==0 and $from_page == 0 and $o_is_sendorder == 2){ ?>
                    ...
                    <div class="WSY_modifydiv order_hide div_show" id="freight_<?php echo $o_batchcode; ?>" >
                        <dl class="freight_dl_gaijia">
							...
							<?php
							if($f2c_freight_price>0){ echo $f2c_freight_price ."<?php echo OOF_T ?>"; }else{ echo "免邮"; }
...
<!-- 						<dd>
							...
							<b style="width:40px;"><?php echo OOF_T ?></b>
							<span style="font-size:12px;width:100%;text-align:left;color:#f00;">(最多结算給合作商 <?php echo $o_SupplyCostMoney+$o_SupplyExpressPrice;?><?php echo OOF_T ?>)</span>
						</dd> -->
						<?php }?>
						<?php
						if( $o_paystyle != "区块链积分支付"){
						?>
						<dd>
							<b style="width:145px;">实际退款金额：</b>
							<span>
								<input type="text" onkeyup="clearNoNum(this)" id="apply_good_refund_<?php echo $o_batchcode;?>" value="<?php echo $return_account = $row_order['return_account'];?>" disabled>
							</span>
							<b style="width:40px;"><?php echo OOF_T ?></b>
						</dd>
						<?php }?>
						...
						<dd>
							...
							<span>
								<input type="text" id="good_refund_<?php echo $o_batchcode;?>" onkeyup="clearNoNum(this)" onafterpaste="clearNoNum(this)" onchange="clearNoNum(this)" <?php if($o_paystyle=='通联分期支付') { echo 'readonly style="background-color: rgb(233,233,232);"'; ?>  value='<?php echo $return_account;?>'<?php } ?> >
							</span>
							<b style="width:40px;"><?php echo OOF_T ?></b>
							<span style="font-size:12px;width:100%;color:#f00;">(使用<?php echo $o_paystyle; ?>方式支付了 <?php echo  number_format($o_totalprice_last,2);//$settlementprice; ?><?php echo OOF_T ?>)</span>
						</dd>
							<?php }else{?>
								<b style="margin-left: 10px;float:left;display: block;line-height: 36px;height: 36px;"><?php echo $o_paystyle; ?>退款金额：</b>
								<span style="display: inline-block;width: 50px;line-height: 36px;height: 36px;margin-top: 5px;">
									<input type="text" id="good_refund_<?php echo $o_batchcode;?>" onkeyup="clearNoNum(this)" onafterpaste="clearNoNum(this)" onchange="clearNoNum(this)" style="width: 100%;height: 23px;text-align: center;line-height: 23px;border: solid 1px #e3e3e3;">
								</span>
								<b style="width:140px;margin-left: 5px;line-height: 36px;height: 36px;"><?php echo $block_chain_score; ?></b><br/>
								<span style="font-size:12px;width:100%;color:#f00;display: block;line-height: 36px;height: 36px;">(使用<?php echo $o_paystyle; ?>方式支付了 <?php echo  number_format($o_block_chain_price,2);//$settlementprice; ?><?php echo $block_chain_score; ?>)</span>
                        ...
                        >
							<b style="width:145px;"><?php echo defined('PAY_CURRENCY_NAME')? PAY_CURRENCY_NAME: '购物币'; ?>退款金额：</b>
							<span>
							...
							<b style="width:40px;"><?php echo OOF_T ?></b>
							<span style="font-size:12px;width:100%;color:#f00;">(该订单使用了 <?php echo $pay_currency;?><?php echo defined('PAY_CURRENCY_NAME')? PAY_CURRENCY_NAME: '购物币'; ?>)</span>
							<?php if($i_cost_integral>0){ ?>
								<b style="width:145px;"><?php echo $integral_name;?>退还数：</b>
								<span>
									<input type="text" id="integral_refund_<?php echo $o_batchcode;?>" value="0" onkeyup="clearNoNum(this)" onafterpaste="clearNoNum(this)" onchange="clearNoNum(this)">
								</span>
								<b style="width:40px;">点</b>
								<span style="font-size:12px;width:100%;color:#f00;">(该订单使用了 <?php echo $i_cost_integral;?><?php echo $integral_name;?>)</span>
							<?php }?>
						</dd>
						<?php if(!empty($o_need_score_sum)){ ?>
							<input type="hidden" id="need_score_<?php echo $o_batchcode;?>" value="<?php echo $o_need_score_sum;?>">
						<?php } ?>
                        <dd class="WSY_bottonli con-button"><input  type="button" value="确定" class="good_refund" data-batchcode="<?php echo $o_batchcode;?>" data-money="<?php echo $row_order['return_account'];?>" data-paystyle="<?php echo $o_paystyle; ?>"  data-costintegral="<?php echo $i_cost_integral;?>"  data-blockchainprice="<?php echo $o_block_chain_price;?>" data-paycurrency="<?php echo $pay_currency;?>"></dd>
                        <dd class="WSY_bottonli con-button2"><input  type="button" value="取消" onclick="hideDetail()"></dd>
                    </dl>
             </div>
				<!-- 退款 End -->
				...
				<!--修改价格·定位属性-->
					<div class="WSY_modifydiv order_hide div_show" id="price_<?php echo $o_batchcode; ?>" >
						<dl class="order_dl_gaijia">
							<dt><a>修改价格</a></dt>
							<dd><b>订单价格：</b><span class="WSY_red"><?php echo number_format($o_totalprice_last,2); ?><?php echo OOF_T ?></span></dd>
            	...
            	<div class="WSY_modifydiv order_hide div_show" id="refund_all_<?php echo $o_batchcode; ?>" >
                    <dl class="order_refund" style="width:25%;min-width: 280px;">
						...
						<dd>
							...
							<b style="width:40px;"><?php echo OOF_T ?></b>
						    <?php }else{ ?>
						    <b style="width:145px;">实际可退区块链积分：</b>
						    <span>
								<input type="text" id="good_refund_money_<?php echo $o_batchcode;?>" value="<?php echo $o_block_chain_price;?>">
							</span>
							<b style="width:40px;"><?php echo $block_chain_score; ?></b>
							<span style="font-size:12px;width:100%;text-align:left;color:#f00;">(最多可退区块链积分 <?php echo $o_block_chain_price;?><?php echo $block_chain_score; ?>)</span>
						    <?php } ?>
						</dd>
...
<!-- 						<dd>
                       ...
                       <dd><b style="width:145px;">备注：</b><span>
                        ...
                        <dd class="WSY_bottonli con-button"><input  type="button" value="确定" class="good_refund_all" data-batchcode="<?php echo $o_batchcode;?>" data-money="<?php echo $row_order['return_account'];?>" data-paystyle="<?php echo $o_paystyle; ?>"  data-blockchainprice="<?php echo $o_block_chain_price;?>" ></dd>
                        <dd class="WSY_bottonli con-button2"><input  type="button" value="取消" onclick="hideDetail()"></dd>
                    </dl>
             </div>
				<!-- 退货(确认收到退货) End -->
                    ...
                    <dl class="order_date">
                       <dt><a style="width:110px;">扣除合作商款项</a></dt>
						...
						<dd>
							...
							<b style="width:40px;"><?php echo OOF_T ?></b>
							<span style="font-size:12px;width:100%;color:#f00;">(最多扣除合作商 <?php echo $o_SupplyCostMoney+$o_SupplyExpressPrice;?><?php echo OOF_T ?>)</span>
						</dd>
                    ...
                    <dl class="order_log">
                        <dt><a>订单日志</a></dt>
							<?php
								...
								// 		continue;
								// 	}
								// 	$query_u   = "select weixin_name from weixin_users where isvalid = true and weixin_fromuser='".$operation_user."'";
								// 	$result_u  = _mysql_query($query_u) or die("Query_u failed : ".mysql_error());

								// 	//获取下单人的信息
								// 	$query_users  = "SELECT weixin_name FROM weixin_users WHERE id=".$buy_user_id;
								// 	$result_users = _mysql_query($query_users) or die('Query_users failed:'.mysql_error());

								// 	while($row_u = mysql_fetch_object($result_u))
								// 	{
								// 		if ($row_u->weixin_name !='')
								// 		{
								// 			$operation_user = $row_u->weixin_name;
								// 		}
								// 		else
								// 		{
								// 			$operation_user = '';
								...
								// 		if (strlen($operation_user) > 15) {
								// 			$operation_user = $row_users->weixin_name;
								// 		}
								// 	};
								foreach ($res23 as $k => $row_log) 
								{
									if($o_batchcode == $row_log['batchcode'])
									{
										...
										if($op == 0 || $op == 1 || $op == 2 || $op == 5 || $op == 7 || $op == 8 || $op == 10 || $op == 13 || $op == 17 || $op == 18){
...
<div class="searchform">
    <span class="cancal">×</span>
    <p style="text-align:left;line-height: 50px;">物流公司</p>
    <p>
        <input type="text" class="daochuipt" id="logistics_company" value="<?php echo $ee_name; ?>">
    </p>
    <p style="text-align:left;line-height: 50px;">快递单号</p>
    <p>
        <input type="text" class="daochuipt" id="courier_number" value="<?php echo $o_expressnum; ?>">
    </p>
    <input type="hidden" id="click_batchcode" value="<?php echo $o_batchcode; ?>">
    <div class="WSY_text_input">
        <button style="display: block;float: none;margin: 20px auto;cursor: pointer;width: 70px;height: 30px;line-height: 30px;" class="WSY_button" id="save_change_courier_number">确定</button>
    </div>
</div>
<div class="blessing_img">
</div>
...
<!-- 分页 --start-->
    ...
    var search_integral_str = "<?php echo $search_integral_str;?>";
    var shop_branch_id = "<?php echo $shop_branch_id?>";
	//pageCount：总页数
	//current：当前页
	<?php if($page > 1){ ?>
		$(".WSY_page").createPage({
			...
			current:pagenum,
			backFn:function(p){
			var url="order.php?customer_id="+customer_id+"&pagenum="+p+"&from_page="+from_page+search_integral_str+"&f2c_id="+f2c_id+"&store_id="+store_id;
			search_condition(url);
		   }
		});
	<?php }?>
    function change_courier_number(bat){
        $(".searchform").toggle();
        $(".searchform").css("left",($(window).width() - 450) / 2);
        var logistics_company = $("#express_id2_"+bat).text().trim();
        var courier_number    = $("#express_num2_"+bat).val();
        var batchcode         = bat;
        $("#logistics_company").val(logistics_company);
        $("#courier_number").val(courier_number);
        $("#click_batchcode").val(batchcode);
  ...
  function jumppage(){
	...
	if((a<1) || (a>count) || isNaN(a)){
		...
		var url="order.php?customer_id="+customer_id+"&pagenum="+a+"&from_page="+from_page+search_integral_str+"&f2c_id="+f2c_id+"&store_id="+store_id;
		search_condition(url);
...
function showPrice(batchcode,num){
	/*if(num>1){	//2017-8-31	crm9900	by	lzw
...
$blessing_url = $http_type.$_SERVER['HTTP_HOST']."/mshop/web/index.php?m=order_blessing&a=index&customer_id={$customer_id_en}&batchcode=";
?>
var blessing_url = "";
$('#show_blessing').on({
    mouseover : function(){
        var batchcode = $(this).data('batchcode');
        var blessing_url = '<?php echo $blessing_url;?>'+batchcode;
        $(".blessing_img").qrcode(blessing_url);
        $(".blessing_img").css('display','block');
    } ,
    mouseout : function(){
        $('.blessing_img').html('');
        $(".blessing_img").css('display','none');
    }
});
<!-- 贺卡祝福语 --end-->
...
function cancal_batchcode()
{
	$('.searchform').hide();
}

$("#save_change_courier_number").click(function(){
    var batchcode = $("#click_batchcode").val();
    ...
    //     layer.alert("请输入正确的快递单号！");
    //     return;
    // }
    if(!/^[A-Za-z0-9]+$/.test(courier_number)){
        layer.alert("请输入正确的快递单号！");
        return;
    }
    layer.confirm('您确认要更改 订单:'+batchcode+' 的</br>快递公司与快递单号吗？', {
        btn: ['确定','取消']
    }, function(confirm){
        layer.close(confirm);
        layer_open();
        $.ajax({
            url: "order.class.php",
            type:"POST",
            data:{'batchcode':batchcode,'logistics_company':logistics_company,'courier_number':courier_number,'op':"change_courier_number"},
            dataType:"json",
            success: function(res){
                layer.close(index_layer);
//                if(res.status==0){
...
//                }
                if(res.errcode>0){
                    ...
                    layer.alert(res.msg);
                    $()
                    $("#express_id2_"+batchcode).text(logistics_company);
                    $("#express_num2_"+batchcode).val(courier_number);
                    $('.searchform').hide();
                    var html = '<dd><b>时间：</b><span>'+res.time+'</span><b>操作：</b><span>修改快递单号</span><b>描述：</b><span>修改快递公司:'+logistics_company+'，修改快递单号:'+courier_number+'</span><b>操作人：</b><span>'+res.customer+'</span></dd>';
                    $('#log_'+batchcode+' dd').last().after(html);
                }
            },
            error:function(){
                layer.close(index_layer);
        ...
        $.ajax({
            ...
            success: function(res){
                ...
                if(res.status==0){
                    ...
                    $('#freight_p_'+batchcode).text(changePrice+'<?php echo OOF_T ?>(改价后)');
//                    $('#price_'+batchcode).find('dd').find('.WSY_red').text(changePrice+'<?php echo OOF_T ?>');
//                    $('#table_three_'+batchcode).find('b').html(changePrice+'<?php echo OOF_T ?><span style="color:#dd514c;margin-left: 4px;">(改价后)</span>');
//                    $('#order_price_'+batchcode).text(changePrice+'<?php echo OOF_T ?>');
				...
				success: function(res){
					...
					if(res.status==0){
						if(return_type==1){
							...
							$(obj).replaceWith('<a title="确定已退货" data-batchcode="'+batchcode+'" onclick="confirmGoodRefund(this)"><img src="/weixinpl/common/images_V6.0/operating_icon/icon56.png"></a><a id="button_delivery_19450514967343994280" title="发货" onclick="showDelivery(\''+batchcode+'\')"><img src="/weixinpl/common/images_V6.0/operating_icon/icon42.png"></a>');
						}else{
							$(obj).replaceWith('<a onclick="showGoodRefund('+batchcode+',1,this)" data-refund-batchcode="'+batchcode+'" title="确定退款"><img src="/weixinpl/common/images_V6.0/operating_icon/icon57.png"></a>');
						}
		...
		layer.prompt({
			...
			$.ajax({
				...
				success: function(res){
					...
					if(res.status==0){
						$(obj).remove();
						$("#table_four_"+batchcode+" p:first-child").html('<img src="/weixinpl/common/images_V6.0/contenticon/affirm-icon.png"><b style="color:#31B0D5"> 已发货</b>');
					}
					if(res.errcode>0){
						layer.alert(res.errmsg);
					}else{
		...
		layer.prompt({
			...
			$.ajax({
				...
				success: function(res){
					 ...
					 console.log(res);
					if(res.status==0){
						$(obj).replaceWith('<a title="确定退款" data-refund-batchcode="'+batchcode+'" onclick="showGoodRefund(\''+batchcode+'\',0,this)"><img src="/weixinpl/common/images_V6.0/operating_icon/icon57.png"></a>');
						$("#table_four_"+batchcode+" p:first-child").html('<img src="/weixinpl/common/images_V6.0/contenticon/return-money.png"> <b style="color:#C9302C">顾客申请退款</b><b style="color:#C9302C"> [已同意]</b>');
					}
					if(res.errcode>0){
						layer.alert(res.errmsg);
					}else{
		...
		layer.prompt({
			...
			$.ajax({
				...
				success: function(res){
					...
					if(res.status==0){
						$(obj).remove();
						$("#table_four_"+batchcode+" p:first-child").html('<img src="/weixinpl/common/images_V6.0/contenticon/notaffirm-icon.png"> <b>未发货</b>');
					}
					if(res.errcode>0){
						layer.alert(res.errmsg);
					}else{
		...
		layer.prompt({
			...
			$.ajax({
				...
				success: function(res){
					 layer.close(index_layer);
					if(res.status==0){
						$(obj).replaceWith('<a title="确认维权完毕" data-batchcode="'+batchcode+'" onclick="confirmAftersale(this)"><img src="/weixinpl/common/images_V6.0/operating_icon/icon59.png"></a>');
...
$(".good_refund").click(function(){
	...
	var cost_integral 		= $(this).data('costintegral');
	var refundMoney 		= $("#good_refund_"+batchcode).val();
	var refundIntegral		= $("#integral_refund_"+batchcode).val();
	var need_score			= $("#need_score_"+batchcode).val();
	var o_block_chain_price = $(this).data('blockchainprice');
	var paycurrency         = $(this).data('paycurrency');
	if( refundMoney == undefined){
		refundMoney = 0;
	}
	if( refundIntegral == undefined){
		refundIntegral = 0;
	}
	if(need_score == undefined){
		need_score = 0;
	}
	//alert('cost_integral:'+cost_integral+'refundIntegral:'+refundIntegral);
	var warning_str = '';
	if(paystyle == '健康钱包支付'){
		warning_str = '微信支付、威富通支付、健康钱包支付的订单请先到支付详情界面进行手动退款！';
	}else{
		warning_str = '微信支付、支付宝支付和威富通支付的订单请先到支付详情界面进行手动退款！';
	}

	var refundSupplyMoney 	= Math.abs($("#good_supply_refund_"+batchcode).val());//退回给供应商的金额
	if (isNaN(refundSupplyMoney)) {
		refundSupplyMoney = 0;
	}
	...
	var new_total_refundMoney = parseFloat(total_refundMoney).toFixed(2);
	var unit                = '<?php echo OOF_T;?>';
	if(paystyle == "区块链积分支付"){
		unit = '<?php echo $block_chain_score;?>';
		new_total_refundMoney = parseFloat(refundMoney).toFixed(2);
	}

	console.log(cost_price+expressfee);
	layer.confirm('订单:'+batchcode+'</br>将退款 <b style="color:red">'+new_total_refundMoney+'</b>'+unit+'</br><b style="color:red">'+warning_str+'</b>', {
		btn: ['退款','取消']
	}, function(confirm){

		if(paystyle != "区块链积分支付"){
			total_refundMoney = parseFloat(total_refundMoney).toFixed(2);
		...
		if( parseFloat(refundIntegral) > parseFloat(cost_integral) ){
			layer.alert("退还积分点不能大于订单消耗积分！");
			return;
		}
		...
		/*if(isNaN(refundSupplyMoney) || parseFloat(refundMoney) > parseFloat(cost_price+expressfee)){
			layer.alert("退款金额不能大于供货价("+parseFloat(cost_price)+"<?php echo OOF_T ?>)加运费("+parseFloat(expressfee)+"<?php echo OOF_T ?>)！");
			return;
		}*/
		...
		// if(isNaN(refundSupplyMoney) > parseFloat(refundSupplyMoney-cost_price+expressfee) ||
			...
			// layer.alert("退款金额不能大于可退金额【总金额减去供货价("+parseFloat(cost_price)+"<?php echo OOF_T ?>)和运费("+parseFloat(expressfee)+"<?php echo OOF_T ?>)】！");
			// return;
		// }
		...
		$.ajax({
			url: "order.class.php",
			type:"POST",
			data:{'batchcode':batchcode,'totalprice':refundMoney,'totalintegral':refundIntegral,'currencyMoney':currencyMoney,'refundSupplyMoney':refundSupplyMoney,'retype':returntype,'op':"goodRefund",'need_score':need_score},
			dataType:"json",
			success: function(res){
				 layer.close(index_layer);
				if(res.status==0){
					if(returntype==0){
						if(res.is_collageActivities){
                            // $('a[data-refund-batchcode='+batchcode+']').hide();
						...
						$('a[data-refund-batchcode='+batchcode+']').replaceWith('<a onclick="confirmOrder(this)" data-totalprice="'+refundMoney_old+'" data-batchcode="'+batchcode+'" title="确认完成"><img src="/weixinpl/common/images_V6.0/operating_icon/icon23.png"></a>');
						$("#table_four_"+batchcode+" p:first-child").html('<img src="/weixinpl/common/images_V6.0/contenticon/confirm-return.png"> <b style="color:#1eaf4e">退货已确认(仅退款)</b>');
					}else if(returntype==2){
						$('a[data-refund-batchcode='+batchcode+']').replaceWith('<a onclick="confirmOrder(this)" data-totalprice="'+refundMoney_old+'" data-batchcode="'+batchcode+'" title="确认完成"><img src="/weixinpl/common/images_V6.0/operating_icon/icon23.png"></a>');
						$("#table_four_"+batchcode+" p:first-child").html('<img src="/weixinpl/common/images_V6.0/contenticon/confirm-return.png"> <b style="color:#1eaf4e">退货已确认(退货)</b>');
					}
					$(".order_hide").fadeOut("slow");
				}
				if(res.errcode>0){
...
$(".good_refund_all").click(function(){
	...
	if(paystyle != "区块链积分支付"){
		if(parseFloat(refundGoodMoney_old) != parseFloat(refundGoodMoney)){
			msg += "并将退款金额修改为：<b style='color:red'>"+refundGoodMoney+"<b> <?php echo OOF_T ?>";
		}else{
			msg += "退款金额：<b style='color:red'>"+refundGoodMoney+"<b> <?php echo OOF_T ?>";
		}
	}else{
		if(parseFloat(o_block_chain_price) != parseFloat(refundGoodMoney)){
			msg += "并将退款金额修改为：<b style='color:red'>"+refundGoodMoney+"<b> <?php echo $block_chain_score; ?>";
		}else{
			msg += "退款金额：<b style='color:red'>"+refundGoodMoney+"<b> <?php echo $block_chain_score; ?>";
		}
	}
	layer.confirm(msg, {
		btn: ['确定修改','取消']
		...
		if(paystyle != "区块链积分支付"){
			...
			if( parseFloat(refundGoodMoney) > parseFloat(o_block_chain_price) ){
				layer.alert("退款金额不得大于区块链订单支付总额！");
				return;
			}
			if(isNaN(refundGoodMoney) || parseFloat(refundGoodMoney) > parseFloat(o_block_chain_price)){
				layer.alert("请输入正确的金额！");
				return;
			}
			if( parseFloat(refundGoodMoney) <= 0 ){
				layer.alert("区块链订单退款金额需大于0！");
				return;
			}
		}
		...
		$.ajax({
			...
			success: function(res){
				...
				if(res.status==0){
					...
					+'  onclick="showGoodRefund('+batchcode+',1,this)" ><img src="/weixinpl/common/images_V6.0/operating_icon/icon57.png" /></a>';
					currOp.replaceWith(newOp);
					$("#table_four_"+batchcode+" p:first-child").html('<img src="/weixinpl/common/images_V6.0/contenticon/affirm-icon.png"> <b style="color:#EC971F">顾客申请退货</b><span style="color:red"> [已收到退货]</span>');
					$(".order_hide").fadeOut("slow");
				}
				if(res.errcode>0){
					layer.alert(res.errmsg);
	...
	layer.confirm('确定 订单:'+batchcode+' 已收到退货，确定后不可更改！', {
		...
		$.ajax({
			...
			success: function(res){
				...
				if(res.status==0){
					$(obj).remove();
					$("#table_four_"+batchcode+" p:first-child").html('<img src="/weixinpl/common/images_V6.0/contenticon/change-goods.png"> <b style="color:#C9302C">申请退货(换货)</b><b style="color:#C9302C"> [已收到退货]</b>');
				}
				if(res.errcode>0){
					layer.alert(res.errmsg);
				}else{
	...
	layer.confirm('您确定要确认 订单:'+batchcode+' 交易完成吗？<br/>确认后，表示订单已经完成，并且无法撤销！', {
		...
		$.ajax({
			...
			success: function(res){
				...
				if(res.status==0){
                    //插入操作日志
                    var log_content = "1笔待完成，已完成；";
                    $.ajax({
                        type: "post",
                        url: "/wsy_pub/admin/index.php?m=security_sms&a=sys_log_insert",
                        data: {'sys_calss': 'shop_system_order_management','sys_content':log_content},
                        dataType: "json",
                        success: function (res) {
                            console.log('succrss');
                        },
					...
					$(obj).next('a').remove();
					$(obj).replaceWith('<a title="删除" data-batchcode="'+batchcode+'" onclick="delOrder(this)"><img src="/weixinpl/common/images_V6.0/operating_icon/icon04.png"></a>');
...
function payOrder(obj){
    ...
    //判断是否开启短信验证
    var param_arr = [batchcode,totalprice];
    sms_check("plat_pay","sms_continue",param_arr);
    // $.ajax({
    //     url: "/wsy_pub/admin/index.php?m=security_sms&a=sms_verification_check",
    //     dataType: 'json',
    //     type: 'post',
    //     data:{'sms_check_type':'moneybag_recharge'},
    //     success:function (res){
    //         // console.log(res);return;
    //         if(res.is_sms_check && !res.security_sms_ischeck && res.sms_check_info && !security_sms_key){       //开启短信验证且短信session为false且符合验证内容且安全限制全局参数为false，进行短信验证
    //             sms_layer(res.sms_check_phone,'sms_continue',batchcode,totalprice);
    //         }else{
    //             
    //         }
    //
...
function sms_continue(batchcode,totalprice){
    layer.confirm('您确定要确认支付订单号:'+batchcode+'吗？', {
        ...
        $.ajax({
            ...
            success: function(res){
                layer.close(index_layer);
                if(res.status==0){
                    $("#order_pay_"+batchcode).html('<img src="/weixinpl/common/images_V6.0/contenticon/pay-icon.png" /><span class="CP_table_bianhaof">已支付<span style="color:red;">(后台支付)</span></span>');
                    $(obj).prev("a").remove();
                    $(obj).next("a").replaceWith('<a title="返佣记录" href="order_rebate_log.php?batchcode='+batchcode+'&customer_id='+customer_id+'==&class=1"><img src="/weixinpl/common/images_V6.0/operating_icon/icon51.png"></a>');
                    if(res.supply==-1){
                        $(obj).replaceWith('<a id="button_delivery_'+batchcode+'" title="发货" onclick="showDelivery(\''+batchcode+'\')"><img src="/weixinpl/common/images_V6.0/operating_icon/icon42.png"></a>');
                    }else{
                        $(obj).remove();
                    }
		...
		$.ajax({
			...
			success: function(res){
				...
				if(res.status==0){
					...
					var batchcode_add = "'"+batchcode+"'";   		//必须加单引号
					$("#express_num2_"+batchcode).parent('span').parent('dd').append('<span onclick="KuaiDi100('+batchcode_add+')" class="order_kuaidi">(点击查看物流)</span>');
					$(".change_add_"+batchcode).remove();

					if(is_payondelivery == 1)
					{
						var payondeliveryhtml = "<a title='确认签收' data-batchcode='"+batchcode+"' data-totalprice='"+totalprice+"' onclick='sign_yes(this,1)' ><img src='/weixinpl/common/images_V6.0/operating_icon/icon80.png' /></a><a title='拒绝签收' data-batchcode='"+batchcode+"' data-totalprice='"+totalprice+"' onclick='sign_yes(this,2)' ><img src='/weixinpl/common/images_V6.0/operating_icon/icon81.png' /></a>";
					...
					if(expressID!=0){
						...
						if(from_page != 2){
							sendstr = '<p class="CP_table_chanpina_fourp"><img src="/weixinpl/common/images_V6.0/contenticon/confirm_delivery.png"><b style="color:#337AB7"> 顾客已收货</b></p><p>发货时间:'+res.time+'</p><p>收货时间:'+res.time+'</p>';

							$("#button_delivery_"+batchcode).replaceWith('<a title="确认完成" data-batchcode="'+batchcode+'" data-totalprice="'+totalprice+'" onclick="confirmOrder(this)"><img src="/weixinpl/common/images_V6.0/operating_icon/icon23.png"></a><a title="红包确认" class="red_'+batchcode+'"><img src="/weixinpl/common/images_V6.0/operating_icon/icon55.png"></a>');
						}
					}
					$("#table_four_"+batchcode).html(sendstr);
					$(".order_hide").fadeOut("slow");
					//虚拟物品发货后查询自动结算开关是否开启，开启则自动确认
...
function KuaiDi100(obj){
    ...
    // switch (KDName) {
    //     case '中通':
    //         KDName = 'zhongtongkuaidi';
    //     break;
    // }

    customer_id_en = '<?php echo $customer_id_en; ?>';
	kd_href = "<?php echo Protocol.$_SERVER['SERVER_NAME']; ?>/weixinpl/back_newshops/Distribution/settings/kuaidi_head.php?customer_id="+customer_id_en+"&batchcode="+obj+"&postid="+KDNum+"&type="+KDName;
	...
	// console.log('<?php echo Protocol ?>m.kuaidi100.com/index_all.html?type='+KDName+'&postid='+KDNum+'#result');
...
var from_page = '<?php echo $from_page; ?>';
var search_integral = '<?php echo $search_integral; ?>';
function export_excel(num){
	...
	switch(num){
		case 1: //订单导出
			...
			var excelArray = [
						...
						["b_name",b_name],
						["a_name",a_name],
						["confirm_order_time", "确认订单时间"],
						//["or_store_shop_info", "订货系统仓库/门店"]
						["weixin_name","微信名称"],
						["paytime","付款时间"],
						["exp_user_id","上级ID"],
						["exp_weixin_name","上级微信昵称"]
					 ];
			if (from_page == 1 && shop_branch_id=='') { excelArray.push(["or_store_shop_info", "订货系统仓库/门店"]) };
			//if (from_page == 1 && shop_branch_id=='') { excelArray.push(["exp_remark", "物流备注"]) };
			if(from_page == 1 && shop_branch_id!=''){excelArray.push(["branch_shop_info", "子门店信息"])}
		  break;
		case 2: //飞豆
		...
		case 4: //海关明细
		  ...
		  break;
		  case 5: //飞豆
			var name ="commonshop_feidou_excel2";
			var excelArray = [
						["batchcode","订单号"],
						["expressname","快递"],
						["expressnum","快递单号"],
		...
		case 6: //新订单导出
			...
			var excelArray = [
		  	...
		  	break;
		case 7: //易掌柜导出
			var name ="commonshop_yizhanggui_excel";
			var excelArray = [
						["batchcode","订单编号"],
						["express_price","运费"],
						["expressnum","运单号"],
						["paysid","交易号"],
						["merchant_remark","卖家备注"],
						["name","收货人姓名"],
						["identity","发票要求"],//
						["weixin_name","买家昵称"],
						["location_p","省"],
	...
	var confirm_order_time_l = $("#confirm_order_time_l").val();//确认订单时间开始
	var confirm_order_time_r = $("#confirm_order_time_r").val();//确认订单时间结束
	...
	var search_user_id = $("#search_user_id").val();//搜索用户id
	var search_product_name = $("#search_product_name").val();//搜索产品
	var search_agent_id = $("#search_agent_id").val();//搜索代理商ID
	...
	var search_attribution_type = $("#search_attribution_type").val();//订单归属分类
	var search_pre_delivery_type = $("#search_pre_delivery_type").val();//预配送订单
	var status = <?php echo $search_class; ?>;//订单状态
    var orgin_type = $('#orgin_type').val();//订单类型
    var store_lev = $('#store_lev').val();//订单类型
    var search_or_shop_id = $('#search_or_shop_id').val();//订货系统门店ID
    var search_shop_id = $('#search_shop_id').val(); //门店号
    var search_paystatus = '<?php echo $search_paystatus; ?>'; //支付状态
    var search_sendstyle = '<?php echo $search_sendstyle; ?>'; 
    // var search_or_shop_name = $('#search_or_shop_name').val();//订单类型
    // console.log(store_lev+";"+search_or_shop_id+";"+search_or_shop_name);
    // return;
    var shopactivity_mark = <?php echo $shopactivity_mark; ?>;
    ...
    if(orgin_type=="" || orgin_type==undefined){
        orgin_type = -1;
    }
	if(search_batchcode==""){
		search_batchcode = -1;
	...
	if(continued_day=="" || continued_day == undefined){
		continued_day = -1;
	}
	if(search_user_id==""){
		search_user_id = -1;
	...
	if(search_agent_id==""){
		search_agent_id = search_order_ascription;
	}
	if(begintime==""){
		begintime = 0;
	}
	...
	if( search_pre_delivery_type == undefined ){
		search_pre_delivery_type = -1;
	}
	if(confirm_order_time_l == "" || confirm_order_time_l == undefined){
		confirm_order_time_l = 0;
	}
	if(confirm_order_time_r == "" || confirm_order_time_r == undefined){
		confirm_order_time_r = 0;
	}
	if(search_or_shop_id == "" || search_or_shop_id == undefined){
		search_or_shop_id = 0;
	}
	if(search_sendstyle=="" || search_sendstyle == undefined){
		search_sendstyle = -1;
	}

	/*var url_base='/weixin/plat/app/index.php/Excel/'+name+'/customer_id/<?php echo passport_decrypt($customer_id); ?>/excel_fields/'+str+'/begintime/'+begintime+'/endtime/'+endtime+'/pay_begintime/'+pay_begintime+'/pay_endtime/'+pay_endtime+'/search_batchcode/'+search_batchcode+'/orgin_from/'+orgin_from+'/search_paystyle/'+search_paystyle+'/search_order_ascription/'+search_order_ascription+'/search_name/'+search_name+'/search_product_name/'+search_product_name+'/search_phone/'+search_phone+'/search_agent_id/'+search_agent_id+'/search_name_type/'+search_name_type+'/status/'+status+'/search_supply_id/'+search_supply_id+'/search_attribution_type/'+search_attribution_type+"/search_pre_delivery_type/"+search_pre_delivery_type+"/continued_day/"+continued_day+"/from_page/"+from_page+'/confirm_order_time_l/'+confirm_order_time_l+'/confirm_order_time_r/'+confirm_order_time_r+'/';
	...
	var parm =  '/customer_id/<?php echo passport_decrypt($customer_id); ?>/begintime/'+begintime+'/endtime/'+endtime+'/pay_begintime/'+pay_begintime+'/pay_endtime/'+pay_endtime+'/search_batchcode/'+search_batchcode+'/orgin_from/'+orgin_from+'/search_paystyle/'+search_paystyle+'/search_order_ascription/'+search_order_ascription+'/search_name/'+search_name+'/search_product_name/'+search_product_name+'/search_phone/'+search_phone+'/search_agent_id/'+search_agent_id+'/search_name_type/'+search_name_type+'/status/'+status+'/search_supply_id/'+search_supply_id+'/search_attribution_type/'+search_attribution_type+"/search_pre_delivery_type/"+search_pre_delivery_type+"/continued_day/"+continued_day+"/from_page/"+from_page+'/confirm_order_time_l/'+confirm_order_time_l+'/confirm_order_time_r/'+confirm_order_time_r+'/orgin_type/'+orgin_type+
	'/store_lev/'+store_lev+'/search_or_shop_id/'+search_or_shop_id+'/search_user_id/'+search_user_id+'/shopactivity_mark/'+shopactivity_mark+'/search_sendstyle/'+search_sendstyle+'/';
    ...
    if(shop_branch_id!=''){
        url_base = url_base + 'shop_branch_id/' +shop_branch_id+'/';
    }
	if(search_integral!=''){
        url_base = url_base + 'search_integral/' +search_integral+'/';
    }
    if(search_shop_id!=''){
        url_base = url_base + 'search_shop_id/' +search_shop_id+'/';
    }
    if(search_paystatus!='' || search_paystatus===0){
		if(search_paystatus==0){
			search_paystatus = '未支付';
		}
        url_base = url_base + 'search_paystatus/' +search_paystatus+'/';
    }
	...
	// if(name=='commonshop_feidou_excel2')
	// {
	// 	var emails = '';
	// 	var op     = 'iscount';
	...
	// 				return;
	// 			}
	// 			else if(res.errcode == 10001 || res.errcode == 40001)
	// 			{
	// 				type = 2;
	// 				tips = "导出数据超过10万，请根据邮箱发送的下载地址于明天进行下载";
	...
	// 			{
	// 				$("#loading").show();
	// 				$.ajax({type:'post', async:true, url:'../../../common/explore/jiaoben.php', data:{fields:excel_fields,function_name:name,param_json:obj,customer_id:<?php echo passport_decrypt($customer_id); ?>,op:op,type:type,},
	// 					success:function(data){
	// 						$("#loading").hide();
	// 						var res = JSON.parse(data);
	...
	// }
	if(name=='commonshop_feidou_excel2' || name=='commonshop_excel2' || name=='commonshop_yizhanggui_excel')
	{
		var emails = '';
		var op     = 'iscount';
...
function exportRecord(num){
	...
	var search_supply_id = $("#search_supply_id").val();//订单归属
	var status = <?php echo $search_class; ?>;//订单状态
	var search_sendstyle = $("#search_sendstyle").val();//配送方式
	if(search_batchcode==""){
		search_batchcode = -1;
	}
	if(orgin_from==""){
...
function showMore(batchcode){//必填信息更多显示/隐藏
	$('.mess_b_'+batchcode).css('max-height',"auto");
	$('#div1_'+batchcode).css('display',"none");
}
//passport_decrypt
$(function(){
	// $.ajax({
...
/***************回车提交**********************/
$('#search_form').find("input").bind('keypress',function(event){
    if(event.keyCode == "13")    
    {
    	if($("#endtime").val() < $("#begintime").val()){
	 	...
	 	if($("#pay_endtime").val() < $("#pay_begintime").val()){
			 alert("申请时间有误，结束时间不能比开始时间早！");
			 return;
		}
    	if(shop_branch_id!=''){
        	url=url+'&shop_branch_id='+shop_branch_id;
    	}
