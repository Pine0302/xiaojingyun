$(function() {
    ...
    var e = e ? e: {};
    e.slt_opts = $("#slt_opts"),
    e.slt_fts = $("#slt_fontsize"),
    e.slt_lts = $("#slt_letterSpacing"),
    e.pos_top = $("#ipt_posTop"),
    e.pos_left = $("#ipt_posLeft"),
    e.ckb_fontbold = $("#ckb_fontbold"),
    e.ckb_fontitalic = $("#ckb_fontitalic"),
    e.slt_opts.change(function() {
        var t = $("#" + $(this).val()),
        e = t.find("textarea");
        t.show(),
        e.focus()
    ...
    $("#btn_confirm").click(function() {		
		if($('#id').val() == ''){
			if($('#J_PrintTempName').val() == ''){alert('模板名称不能为空！');return;}
			var ax_result = false;	var ax_result_print_name = '';
			$.ajax({type:"GET", async:false, url:"ax_check.php?print_name="+$('#J_PrintTempName').val(),success:function(data){
				var obj_json = $.parseJSON(data);
				if(obj_json.id > 0){ax_result = true;ax_result_print_name=obj_json.print_name;}
				console.log(obj_json);
			}});
		}
		if(ax_result == true){alert('快递模板名字【'+ax_result_print_name+'】己存在');return;};
			
        return "" == $("#J_ImgUrl").val() ? alert('请上传快递单') : $(".textarea-item:visible").length <= 0 ? alert('请添加要打印的项目!') : ($(".textarea-item").each(
		...
		function() {
            ...
            if (t.is(":visible")) {
                ...
                $("#J_FormExpress").append('<input type="hidden" name="print_items_params[]" value="' + encodeURI(JSON.stringify(i)) + '" />')
            }
    ...
    $(function() {
        var e = $("#J_ImgUrl").val();
        "" != e && (a.attr("src", e), a.prop("complete") ? (a.removeClass("default-height"), t()) : a.on("load",
        function() {
