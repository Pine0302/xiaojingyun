function search_condition(url){
	...
	document.location = url+"&customer_id="+customer_id+"&charity_id="+charity_id;
}

function searchForm(){
	var url ="charity_record.php?";
 ...
 $(".WSY_page").createPage({
	...
	current:pagenum,
	backFn:function(p){
		var url ="charity_record.php?&pagenum="+p;
		search_condition(url); 
	}
});
...
function jumppage(){
	...
	if((a<1) || (a==pagenum) || (a>count) || isNaN(a)){
		...
		var url ="charity_record.php?pagenum="+a;
		search_condition(url); 
	}
}