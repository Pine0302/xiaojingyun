$(function() {
    ...
    custom_add = function(data) {
	  ...
	  if(data.type==14){//只允许创建一个头部引导页
		...
		add_mod(customer_id,diy_temid,data.type,data.id,data.content.css_type,title,pic,data.content.foot_position,data.content.placeholder,data.content.pro_numshow,data.content.pro_title_show,data.content.pro_title_twoline,data.content.show_sale,data.content.rolling_direction,data.content.rolling_speed,data.content.show_time_limit,data.content.city_name,start_time,end_time,data.content.location_p,sel_link_type);
     ...
     */
        ...
        switch (k) {
            ...
            case 15:custom_event_type15(ctrldom, data);break;//头像插件
            case 20:custom_event_type20(ctrldom, data);break;//联系电话
            case 21:custom_event_type21(ctrldom, data);break;//定位地址
            case 22:custom_event_type22(ctrldom, data);break;//店铺顶部信息
        }
    };
});


function add_mod(customer_id,diy_temid,type,diy_tem_contid,css_type,title,imgurl,foot_position,placeholder,pro_numshow,pro_title_show,pro_title_twoline,show_sale,rolling_direction,rolling_speed,show_time_limit,city_name,start_time,end_time,province,sel_link_type){ //添加模块
