...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	else
	{
		...
		for($i = 2; $i <= $rows; $i++) {
			...
			//gettype($reservation_date);
			//echo " type : ".gettype($reservation_date)." reservation_date : ".$reservation_date."<br/>";
			...
			,address,reservation_date,remark,install_cost,customer_id,isvalid,ordertype,product_name,product_count) 
			values('".$install_order."','".$ordernum."',now(),0,'".$contact."','".$phone."','".$location_p."','".$location_c."','".$location_a."',
			'".$address."','".$reservation_date."','".$remark."','".$install_cost."','".$customer_id."',1,1,'".$product_name."','".$product_count."')";
