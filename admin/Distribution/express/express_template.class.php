...
if($op=="add"){
	...
	if($express_id != ''){
		...
		//echo $query_add2;
		_mysql_query($query_add2)or die('Query failed'.mysql_error());
	}
...
if($op=="update"){//修改
	...
	while ($row = mysql_fetch_object($result)) {
		...
		//echo $query_date;
		_mysql_query($query_date)or die('Query failed'.mysql_error());
	}
...
if($op=="checkTitle"){
	//检查模板名称是否重名
	$tcount = 0;
	$query = "select count(1) as tcount from express_template_t where isvalid=true and title='".$title."' and customer_id=".$customer_id." and supply_id=-1";
