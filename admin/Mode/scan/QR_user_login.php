...
$start = ($pagenum-1) * 20;
$end = 20;
$query = 'select log.createtime,log.ip,clerk.confirm_name from weixin_commonshop_ticketlog as log LEFT JOIN weixin_commonshop_ticketclerk as clerk on clerk.id = log.clerk_id  where log.customer_id='.$customer_id." order by clerk.id desc limit ".$start.",".$end; 
$result = _mysql_query($query) or die('Query failed: ' . mysql_error());
 
$rcount_num = 0; 
$query_num = 'select count(1) as rcount from weixin_commonshop_ticketlog where customer_id='.$customer_id; 
	...
	<div class="WSY_content">
		...
		<div class="WSY_columnbox">
			...
			<div class="WSY_column_header">
				<div class="WSY_columnnav">
					...
					<a href="QR_user_login.php?customer_id=<?php echo passport_encrypt((string)$customer_id); ?>" class="white1">兑票员登录日志</a>
		...
		?>
			<tr>
				...
				<td align="center"><?php if(!empty($ip)){echo "登录";}else{echo "登出"; }; ?></td>
			</tr>
		  <?php
...
<script>
  ...
  var count =<?php echo $page ?>;//总页数
	...
	$(".WSY_page").createPage({
        ...
        current:pagenum,
        backFn:function(p){
		 document.location= "QR_user_login.php?pagenum="+p+"&customer_id=<?php echo passport_encrypt((string)$customer_id) ?>"; 
	   }
    });
  ...
  function jumppage(){
	...
	document.location= "QR_user_login.php?customer_id=<?php echo passport_encrypt((string)$customer_id) ?>&pagenum="+a;
	}
  }	
</script>	
</body>
