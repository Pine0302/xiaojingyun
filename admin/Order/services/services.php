<?php
	...
	$query = 'SELECT count(1) as wcount FROM weixin_commonshop_orders where isvalid=true and customer_id='.$customer_id;
	...
	$query_order = "
		SELECT
		orders.id,orders.user_id,orders.batchcode,orders.createtime,orders.paystyle,sum(orders.totalprice) as  totalprice,orders.paystatus,orders.sendstatus,orders.status,orders.exp_user_id,orders.supply_id,orders.allipay_orderid,orders.is_delay,
		...
		orders.aftersale_type,orders.aftersale_state,orders.return_status,orders.return_account,orders.is_QR,
		users.name,users.weixin_name,users.weixin_fromuser,users.phone,
		addr.name as expressName,addr.phone as expressPhone,addr.location_p,addr.location_c,addr.location_a,addr.address as expressAddress
		...
		WHERE orders.isvalid=true and orders.customer_id = ".$customer_id;
	...
	if(!empty($_GET["begintime"])){  //下单时间
	   ...
	   $query_order = $query_order." and UNIX_TIMESTAMP(orders.createtime)>=".strtotime($begintime);
	}
	$endtime = "";
	if(!empty($_GET["endtime"])){   //下单时间 End
	   $endtime = $_GET["endtime"];
	   $query_order = $query_order." and UNIX_TIMESTAMP(orders.createtime)<=".strtotime($endtime);
	}
	$pay_begintime = "";
	if(!empty($_GET["pay_begintime"])){  //支付时间
	   $pay_begintime = $_GET["pay_begintime"];
	   $query_order = $query_order." and UNIX_TIMESTAMP(orders.paytime)>=".strtotime($pay_begintime);
	}
	$pay_endtime = "";
	if(!empty($_GET["pay_endtime"])){   //支付时间 End
	   $pay_endtime = $_GET["pay_endtime"];
	   $query_order = $query_order." and UNIX_TIMESTAMP(orders.paytime)<=".strtotime($pay_endtime);
	}
	$search_batchcode = "";
	if(!empty($_GET["search_batchcode"])){    //订单号
	   $search_batchcode = $configutil->splash_new($_GET["search_batchcode"]);
	   $query_order = $query_order." and orders.batchcode = '".$search_batchcode."'";
	}
	...
	if(!empty($_GET["search_order_ascription"])){    //订单所属
	   ...
	   switch($search_order_ascription){
			...
			case -1:
				//平台订单
				$query_order=$query_order." and orders.supply_id=-1";
				break;
			default:
				//供应商订单
				$query_order=$query_order." and orders.supply_id=".$search_order_ascription;
		}
	}
	$search_name = "";
	$search_name_type = 1;
	if(!empty($_GET["search_name"])){    //名称
	   ...
	   $search_name = $configutil->splash_new($_GET["search_name"]);
		switch($search_name_type){
			case 1:
				$query_order .= " AND users.weixin_name like '%".$search_name."%'";
			break;
			case 2:
				$query_order .= " AND addr.name like '%".$search_name."%'";
			break;
		}
	}

	/* $search_sendstatus = "";	//发货状态
	if(!empty($_GET["search_sendstatus"])){
	   $search_sendstatus = $configutil->splash_new($_GET["search_sendstatus"]);  //支付状态查询  0:未支付 1:已支付 -1:支付失败
	   $query_order .= " AND paystyles.sendstatus=".$search_sendstatus;
	} */

	$search_paystyle = "";	//支付方式
	if(isset($_GET["search_paystyle"])){
	   $search_paystyle = $configutil->splash_new($_GET["search_paystyle"]);  //支付状态查询  0:未支付 1:已支付 -1:支付失败
	   $query_order .= " AND orders.paystyle='".$search_paystyle."'";
	}
	/* $search_status ="" ;
	if(isset($_GET["search_status"]) and $_GET["search_status"] !=""){   //订单状态
	   $search_status = $configutil->splash_new($_GET["search_status"]);
	   $query_order .= " AND orders.status=".$search_status;
	}	 */
	/* 搜索条件End */

	$query_order .=  " GROUP BY batchcode ORDER BY id DESC limit ".$start.",".$end;
	//echo $query_order;
			...
			<ul>
				<li class="WSY_position_text">
				 ...
				 <li class="WSY_bottonli WSY_bottonli2">
					<input type="button"  value="导出记录" onClick="exportRecord();" >
					<input type="button"  value="导出飞豆" onClick="exportRecord();" >
					<input type="button"  value="导出海关头部" onClick="exportRecord();" >
					<input type="button"  value="导出海关明细" onClick="exportRecord();" >
		...
		<table width="97%" class="CP_table " id="order_table">
			 ...
			 <?php
				...
				while ($row_order = mysql_fetch_object($result_order)) {
					...
					$has_change_price = false;
					$o_totalprice_last = 0;
					$query_change="select price from weixin_commonshop_order_prices where isvalid=true and batchcode='".$o_batchcode."' limit 1";
					...
					$exp_user_name ="无";
					if($o_exp_user_id>0){
					    $query_exp= "select name,phone,weixin_name,weixin_fromuser from weixin_users where isvalid=true and id=" . $o_exp_user_id . " limit 1";
					...
					if($o_agent_id>0){
						$query_agent = "SELECT name,weixin_name,weixin_fromuser FROM weixin_users WHERE isvalid=true and id=".$o_agent_id." limit 1";
					...
					if($o_supply_id>0){
						$query_supply = "SELECT name,weixin_name,weixin_fromuser FROM weixin_users WHERE isvalid=true and id=".$o_supply_id." limit 1";
			 ...
			 <tr class="CP_table_bianhao" >
				...
				<span class="CP_table_bianhaog">支付方式：<?php echo $o_paystyle;?>
				...
				if($o_paystatus==1 and $o_Pay_Method==0){
					...
					if($o_paystyle=="微信支付" or $o_paystyle=="找人代付" ){
						...
						if($wxpay_version==2){
							echo "[<a href=\"weipay_detail.php?allipay_orderid=$transaction_id\">". $transaction_id ."(点击查看)</a>]";
						}else{
							echo "[<a href=\"weipay_detail2.php?allipay_orderid=$transaction_id\">". $transaction_id ."(点击查看)</a>]";
						}
					}
					/* 微信支付End */
				}
			 ...
			 <?php
					/* 产品信息 */
						...
						$o2_num = 0;
						$o2_imgurl = 0;
						$query_order2 = "SELECT orders2.rcount,orders2.prvalues, orders2.totalprice,
							...
							where orders2.isvalid=true and orders2.batchcode='".$o_batchcode."'";
			 ...
			 <tr class="CP_table_chanpina">
				<td class="CP_table_chanpina_one">
					<img src="<?php echo "//".$new_baseurl.$o2_imgurl; ?>" />
				...
				<td class="CP_table_chanpina_two" rowspan="<?php echo $o2_rows; ?>">
					<?php echo $o_expressName; ?><br/>
					<?php echo $o_expressPhone; ?>
				</td>
				<td class="CP_table_chanpina_three" rowspan="<?php echo $o2_rows; ?>">
					<b>¥<?php echo number_format($o_totalprice_last,2); ?></b><br/><span><?php if($o_express_price>0){ echo "(含运费 ¥". $o_express_price .")"; }else{ echo "免邮"; } ?></span>
				</td>
				...
				<td class="CP_table_chanpina_five" rowspan="<?php echo $o2_rows; ?>"><?php echo $exp_user_name; ?></td>
				<td class="CP_table_chanpina_six" rowspan="<?php echo $o2_rows; ?>">
					...
					<a title="红包确认" ><img src="../../../common/images_V6.0/operating_icon/icon55.png" /></a>
					...
					if($o_sendstatus==3 and $o_supply_id<0 and $o_open_sendway==0){
						...
						?>
						<a title="退货管理" data-batchcode="<?php echo $o_batchcode;?>"  onclick="returnGood(this)" ><img src="../../../common/images_V6.0/operating_icon/icon56.png" /></a>
						<?php
							...
							?>
							<a title="确定退款" data-refund-batchcode="<?php echo $o_batchcode;?>"  onclick="showGoodRefund(<?php echo $o_batchcode; ?>,1)" ><img src="../../../common/images_V6.0/operating_icon/icon56.png" /></a>
							<?php
					...
					if($o_sendstatus==5){
						...
						?>
						<a title="确定退款" data-refund-batchcode="<?php echo $o_batchcode;?>"  onclick="showGoodRefund(<?php echo $o_batchcode; ?>,0)" ><img src="../../../common/images_V6.0/operating_icon/icon56.png" /></a>
						<?php
					...
					<a title="返佣记录" ><img src="../../../common/images_V6.0/operating_icon/icon51.png" /></a>
			...
			<!--修改价格·定位属性-->
          <tr class="WSY_positiontrhide1">
          	<td colspan="11">
            	<div class="WSY_modifydiv order_hide div_show" id="price_<?php echo $o_batchcode; ?>" >
                    <dl class="order_dl_gaijia">
                        <dt><a>修改价格</a></dt>
                        <dd><b>订单价格：</b><span class="WSY_red">￥<?php echo $o_totalprice; ?>元</span></dd>
                        <dd><b>现价：</b><span><input type="text" value=""></span></dd>
                        <dd class="WSY_bottonli con-button"><input  type="button" value="确定改价" class="order_price" data-batchcode="<?php echo $o_batchcode;?>"></dd>
          ...
          <tr class="WSY_positiontrhide">
          	<td colspan="11" class="order_td">
            	<div class="order order_hide div_show" id="order_<?php echo $o_batchcode; ?>" >
                	...
                	<dl class="order_dl01">
                        ...
                        <div class="order_div">
                            ...
                            <dd><b>实付金额：</b><span class="WSY_red">￥<?php echo number_format($o_totalprice_last,2); ?>元</span></dd>
                        </div>
                    </dl>
                    <dl class="order_dl02">
                        <form>
                        ...
                        <div class="order_div01">
                            ...
                            <dd><b>收货人：</b><span data-phone="<?php echo $o_batchcode; ?>"><?php echo $o_expressPhone; ?></span></dd>
                            <dd><b>收货地址：</b><span data-add="<?php echo $o_batchcode; ?>"><?php echo $o_location_p . $o_location_c . $o_location_a . $o_expressAddress; ?></span></dd>
                            <dd><b>订单备注：</b><span><?php echo $o_remark; ?></span></dd>
                        ...
                        <div class="order_div01">
                            <dd><b>物流单号：</b><span><input type="text" disabled="disabled"  id="express_num2_<?php echo $o_batchcode; ?>" ></span></dd>
                    ...
                    <dl class="order_dl04">
                        ...
                        <dd class="order_dd_hidden">
                            ...
                            <div class="order_dl04_div">
                                ...
                                <p><?php echo $msg_message; ?></p>
                            </div>
								  <?php  }  ?>
                        </dd>
                        <form class="form_words">
                            <dd><textarea class="textarea02" placeholder="输入您想留言的信息"></textarea></dd>
				...
				<?php if($o_sendstatus == 0 || ($o_sendstatus == 3 && $o_return_type == 2) ){ ?>
				<!--订单发货-->
            	<div class="order order_delivery_dl order_hide div_show" id="delivery_<?php echo $o_batchcode; ?>"  >
                    ...
                    <dl>
                        ...
                        <dd class="WSY_bottonli con-button"><input  type="button" value="确定发货" class="order_delivery" data-batchcode="<?php echo $o_batchcode;?>" ></dd>
				...
				<!-- 延期收货End -->
				<?php }
				if($o_sendstatus==3 ||( $o_sendstatus==5 && ($o_return_status==8 or $o_return_status==0) )){
				?>
				<!-- 退款 -->
            	...
            	<div class="WSY_modifydiv order_hide div_show" id="refund_all_<?php echo $o_batchcode; ?>" >
                    <dl class="order_refund">
                       ...
                       <dd><b style="width:135px;">退货备注：</b><span>
...
<!-- 分页 --start-->
	...
	$(".WSY_page").createPage({
		...
		current:pagenum,
		backFn:function(p){
		document.location= "order.php?customer_id=<?php echo $customer_id; ?>&pagesize=<?php echo $pagesize; ?>&pagenum="+p;
	   }
	});
  ...
  function jumppage(){
	...
	document.location= "order.php?customer_id=<?php echo $customer_id; ?>&pagesize=<?php echo $pagesize; ?>&pagenum="+a;
	}
  }
<!-- 分页 --end-->
...
$(".order_price").click(function(){
	...
	layer.confirm('您确认要修改 订单:'+batchcode+' 的价格吗', {
		...
		var expressPrice = $("#express_price_"+batchcode).val();

		if(expressNum=="" && expressNum<0){
		...
		$.ajax({
			url: "order.class.php",
			type:"POST",
			data:{'batchcode':batchcode,'expressID':expressID,'expressRemark':expressRemark,'expressNum':expressNum,'op':"changPirce"},
			dataType:"json",
			success: function(res){
				 layer.close(index_layer);
				if(res.status==0){
					...
					$("#table_four_"+batchcode).html('<p class="CP_table_chanpina_fourp"><img src="../../../common/images_V6.0/contenticon/affirm-icon.png"> 已发货</p><p>发货时间:'+res.time+'</p>');
...
function returnGood(obj){
	...
	layer.confirm('请选择 订单:'+batchcode+' 的申请退货操作', {
		btn: ['同意退货','拒绝']
	}, function(confirm){
		...
		layer.prompt({
			...
			$.ajax({
				...
				success: function(res){
					...
					if(res.status==0){
						$(obj).remove();
						$("#table_four_"+batchcode+" p:first-child").html('<img src="../../../common/images_V6.0/contenticon/affirm-icon.png"> 顾客申请退货<span style="color:red"> [已同意]</span>');
					}
					layer.alert(res.msg);
				},
				error:function(){
		...
		layer.prompt({
			...
			$.ajax({
				...
				success: function(res){
					 layer.close(index_layer);
					if(res.status==0){
						$(obj).replaceWith('<a title="确定退款" data-refund-batchcode="'+batchcode+'" onclick="showGoodRefund('+batchcode+',0)"><img src="../../../common/images_V6.0/operating_icon/icon56.png"></a>');
...
$(".good_refund").click(function(){
	...
	layer.confirm('订单:'+batchcode+'</br>将退款 <b style="color:red">'+refundMoney+'<b> 元', {
		btn: ['退款','取消']
	}, function(confirm){
			...
			success: function(res){
				...
				if(res.status==0){
					if(returntype==0){
						...
						$('a[data-refund-batchcode='+batchcode+']').replaceWith('<a onclick="confirmOrder(this)" data-totalprice="'+refundMoney_old+'" data-batchcode="'+batchcode+'" title="确认完成"><img src="../../../common/images_V6.0/operating_icon/icon23.png"></a>');
						$("#table_four_"+batchcode+" p:first-child").html('<img src="../../../common/images_V6.0/contenticon/affirm-icon.png"> <b style="color:#1eaf4e">退货已确认(退货)</b>');
					}
					$(".order_hide").fadeOut("slow");
				}
				layer.alert(res.msg);
...
$(".good_refund_all").click(function(){
	...
	layer.confirm('订单:'+batchcode+'</br>将退款 <b style="color:red">'+refundGoodMoney+'<b> 元且确认退货完成', {
		btn: ['退货(且退款)','取消']
	}, function(confirm){
		...
		$.ajax({
			...
			success: function(res){
				 layer.close(index_layer);
				if(res.status==0){
					$('a[data-refund-all-batchcode='+batchcode+']').remove();
...
function confirmOrder(obj){
	...
	layer.confirm('您确定要确认 订单:'+batchcode+' 完成吗？<br/>确认后，表示订单已经完成，并且无法撤销！', {
		btn: ['确认','取消']
	}, function(confirm){
		...
		$.ajax({
			...
			success: function(res){
				if(res.status==0){
					...
					$(obj).prev("a").remove();
					$(obj).replaceWith('<a title="发货" onclick=""><img src="../../../common/images_V6.0/operating_icon/icon42.png"></a>');
				}
				layer.alert(res.msg);
			},
			error:function(){
			...
			success: function(res){
				...
				if(res.status==0){
					...
					if(expressID!=0){
						...
						sendstr = '<p class="CP_table_chanpina_fourp"><img src="../../../common/images_V6.0/contenticon/confirm_delivery.png"><b style="color:#337AB7"> 顾客已收货</b></p><p>发货时间:'+res.time+'</p>';
					}
...
function exportRecord(){
	...
	var pay_begintime = $("#pay_begintime").val();   //支付开始时间
	var pay_endtime = $("#pay_endtime").val();  //支付结束时间
	var search_status = $("#search_status").val();   //订单状态
	var search_batchcode = $("#search_batchcode").val(); //订单号
	var search_name = $("#search_name").val();  //名称
	var search_name_type = $("#search_name_type").val();  //名称类型
	var search_paytype = $("#search_paytype").val();   //支付方式
	var search_order_ascription = $("#search_order_ascription").val();  //订单归属
	var url='../../weixin/plat/app/index.php/Excel/commonshop_excel';
	if(pay_begintime !== "" ){
	...
	if(search_paytype !==""){
		url = url + '/search_paytype/'+search_paytype;
	}
