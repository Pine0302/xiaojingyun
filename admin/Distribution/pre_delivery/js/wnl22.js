 ...
 * MIT Licensed (//www.opensource.org/licenses/mit-license.php)
 *
 * For more information: //thejacklawson.com/2011/06/mediators-for-modularized-asynchronous-programming-in-javascript/index.html
		...
		e.localStorage = {
			...
			methods: {
				...
				clear: function(e) {
					...
					else for (var t = localStorage.length - 1, n; n = localStorage.key(t--);) n && n.indexOf(e) === 0 && localStorage.removeItem(n)
				}
			}
		}, e.userData = {
			test: function() {
	...
	$.padWithZero = function(e) {
			...
			var S = new Date(v),
				x = $.map(new Array(42), function(s, a) {
					var c = S.getFullYear(),
						...
						z = c +"/" +(h + 1)+"/"+p;
						selectDate ='';
						$(".user-choice span").each(function(){selectDate=selectDate+$(this).attr('date')+"|" });
					$.inArray(x, f) != -1 && w.push("mh-fes"), h != o - 1 && w.push("mh-cross-month"), (a + 1) % 7 == 0 && w.push("mh-last"), p == m.getDate() && h == m.getMonth() && c == m.getFullYear() && w.push("mh-today"),selectDate.match(z)&& w.push('mh-on'),(d == 0 || d == 6) && w.push("mh-weekend");
					var T = v.festival();
		...
		handleClick: function() {
			...
			$(n.panel).delegate("li", "click", function() {
				 n.panel.find("li.reston").removeClass("reston").addClass("mh-rest"), n.panel.find("li.workon").removeClass("workon").addClass("mh-work");
                ...
                var newStart = $(this).attr('date'); 
                var dateYear = newStart.split('/')[0];
                var dateMouth =newStart.split('/')[1];
                var dateDate = newStart.split('/')[2];
                var dateStr = '';
				if( parseInt(dateMouth) < 10 ){
					dateMouth = '0'+dateMouth;
				}
				if( parseInt(dateDate) < 10 ){
					dateDate = '0'+dateDate;
				}
                dateStr += '<span date="'+newStart+'">'+dateYear+'-'+dateMouth+'-'+dateDate;
                dateStr += '</span>';
                 if($(this).hasClass("mh-on")){
                    $(this).removeClass("mh-on");
                    $(".user-choice span").each(function(i){
                		var $html=$(this).html();
                		if(dateStr.match($html)){
                			$(this).remove();
                		}
                	})   
                }else{
...
$(function(){
    $('.dateDelete').click(function(){
        $('.user-choice span').remove();
        $('ol li').removeClass("mh-on");
		$('#custom_date').val('');
		return false;	//防止表单提交
    })
    
    $('.dateConfirm').click(function(i){
        var qwer = $('.user-choice span').length;
		var custom_date = '';
		var custom_date_html = '已选日期：';
		$('.selected-date-content').text('');
        for(i=0;i<qwer;i++){
            custom_date += $('.user-choice span').eq(i).text()+',';
			custom_date_html += '<span class="selected-date-content-span" style="display: inline-block;margin-right: 10px;padding: 0 5px;background: #FFFFFF;border: 1px solid #d2d2d2;border-radius: 3px;margin-bottom: 5px;margin-top: 5px;">'+$('.user-choice span').eq(i).text()+'</span>';
            // console.log(aa);
        }
		custom_date = custom_date.slice(0,-1);
		$('.selected-date-content').append(custom_date_html);
