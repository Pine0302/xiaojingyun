...
require('../../../../../weixinpl/back_newshops/Base/personalization/config.php');
require('../../../../../weixinpl/back_newshops/Base/personalization/customer_id_decrypt.php'); //导入文件,获取customer_id_en[加密的customer_id]以及customer_id[已解密]
require('../../../../../weixinpl/back_newshops/Base/personalization/back_init.php');
$link = mysql_connect(DB_HOST,DB_USER,DB_PWD);
mysql_select_db(DB_NAME) or die('Could not select database');
_mysql_query("SET NAMES UTF8");
require('../../../../../weixinpl/back_newshops/Base/personalization/proxy_info.php');
require('../../../../../weixinpl/back_newshops/Base/personalization/auth_user.php');
require('../../../../../weixinpl/back_newshops/Base/personalization/home_decoration/product_type_utlity.php');
require('../../../../../weixinpl/back_newshops/Base/personalization/common/utility_4m.php');
...
if(!empty($_GET["op"])){
   ...
   switch($op){
	   case "del":
		   ...
		   _mysql_query($query) or die('Query failed67: ' . mysql_error());
		   if($owner_general and !empty($customer_ids)){
			   _mysql_query("update weixin_commonshop_types set isvalid=false where customer_id in (".$customer_ids.") and create_type=".$owner_general." and create_parent_id=".$producttype_id);
		   }
		   //删除后，可以重新创建
		   $producttype_id=-1;
		   break;
	   case "no_shelves":
			...
			foreach ($re as $key => $value) {
				...
				_mysql_query($query_product) or die('Query failed45: ' . mysql_error());
	   ...
	   case "is_shelves":
			...
			if($producttype_parent_id>0){   // 判断是否顶级
			   ...
			   $result = _mysql_query($query_up) or die('Query failed57: ' . mysql_error());
			   while ($row = mysql_fetch_object($result)) {
			...
			foreach ($re as $key => $value) {
				...
				_mysql_query($query_product) or die('Query failed72: ' . mysql_error());
...
<div id="iframe_page">
	...
	<div class="r_nav">
		<ul>
			...
			<?php if($is_supplierstr){?><li id="auth_page5" class=""><a href="supply.php?customer_id=<?php echo $customer_id_en; ?>">合作商</a></li><?php }?>
