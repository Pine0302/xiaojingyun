...
if($key_id){
	$query="select * from ".WSY_SHOP.".weixin_commonshop_distributor_article where id=$key_id";
...
//file_put_contents($_SERVER["DOCUMENT_ROOT"]."/weixinpl/log/xhhx.log", "=====query=".$query.PHP_EOL,FILE_APPEND);
...
<body>
	...
	<div class="WSY_content">
		<form action="save_distritor_article.php?key_id=<?php echo $key_id ?>"  method="post"  enctype="multipart/form-data" id="upform" name="upform">
		<!--列表内容大框-->
		<div class="WSY_columnbox">
			<!--列表头部切换开始-->
			<div class="WSY_column_header">
				<div class="WSY_columnnav">
					<a class="white1">添加文章</a>
				</div>
			</div>
			<!--列表头部切换结束-->
  ...
  <!--关注用户开始-->
	<div class="WSY_data">
        <dl class="WSY_bulkbox"> 
        	<dt>文章标题：</dt>
			<dd><input type="text" value="<?php echo $title ?>" name="title" id="title" /></dd></dd>		
        </dl>
       <dl class="WSY_bulkbox">
        	<dt>分享描述：</dt>
			<dd><input type="text" value="<?php echo $share_description ?>" name="share_description" id="share_description" /></dd>		
        </dl>
        <dl class="WSY_member" >
			<dt>分享图片：</dt>
            <iframe id="frm_articleImage" src="add_distributor_article_image.php?customer_id=<?php echo $customer_id_en; ?>&article_id=<?php echo $key_id; ?>&article_imgurl=<?php echo $share_img; ?>" height=300 width=1024 FRAMEBORDER=0 SCROLLING=no></iframe>
			<input type=hidden name="imgurl" id="imgurl" value="<?php echo $share_img ; ?>" />
        </dl>
		<dl class="WSY_bulkbox">
			<dt>关联产品：</dt>
			<?php 
