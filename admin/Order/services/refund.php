			...
			<div class="WSY_agentsbox">
				<form class="search" style="display:block" method="get" action="services.php?customer_id=<?php echo $customer_id_en; ?>">
					<div class="WSY_search_q">
						<li>退款申请列表&nbsp&nbsp&nbsp&nbsp&nbsp <input type="text" name="keyword" value="请输入买家手机号或者订单号/退货编号进行搜索" class="form_input" size="15" onfocus="if(this.value==defaultValue) {this.value='<?php echo $keyword; ?>';this.type='text'}" onblur="if(!value) {value=defaultValue; this.type='text';} " style="width:265px; color:#ccc"></li>	
				  ...
				  <?php
					...
					<tr id="WSY_q1" >
						...
						<td><a href="addcardshop.php?keyid=<?php echo $shop_id ?>&card_id=<?php echo $shop_card_id ?>&customer_id=<?php echo passport_encrypt((string)$customer_id); ?>" style="cursor:pointer;" title="编辑">
							...
							<a href="refund.php?shop_id=<?php echo $shop_id ?>&op=del&card_id=<?php echo $shop_card_id; ?>&customer_id=<?php echo passport_encrypt((string)$customer_id); ?>" onclick="if(!confirm(&#39;删除后不可恢复，继续吗？&#39;)){return false};" title="删除">
