...
//	$save_destination = "/weixinpl/".$save_destination;
	...
	$save_defaultimg="update weixin_commonshop_supply_diy_template set default_img='".$save_destination."' where id='".$temid."' and  customer_id=".$customer_id." and  supplier_id=".$supplier_id." and isvalid=true ";
	$result_save_defaultimg=_mysql_query($save_defaultimg) or die ('save_defaultimg faild' .mysql_error());
	$error =mysql_error();  
	mysql_close($link);  
	echo "<script>location.href='default_img.php?customer_id=".$customer_id_en."&default_img=".$save_destination."&temid=".$temid."&supplier_id=".$supplier_id."';</script>";
}
?>