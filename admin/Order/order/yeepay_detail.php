...
$sendstatus = 0;
$query="select status,paystatus,sendstatus,return_status,payother_trade_no,paystyle  from weixin_commonshop_orders where isvalid=true and pay_batchcode='".$pay_batchcode."' group by batchcode";
...
$price				= 0;
$weipay = "select price,callBackBatchcode from paycallback_t where isvalid=true and pay_batchcode='".$pay_batchcode."'";
...
while ($row_result_weipay = mysql_fetch_object($result_weipay)) {
	$transaction_id = $row_result_weipay->callBackBatchcode;
	$price = $row_result_weipay->price;
}

$totalprice		= 0;
$sql = "select sum(origin_price) as totalprice from weixin_commonshop_order_prices where isvalid=true and pay_batchcode='".$pay_batchcode."'";
//echo $sql;
	...
	<div class="WSY_columnbox">
		...
		<div class="WSY_remind_main" style="min-height:250px;">
			...
			<dl class="WSY_remind_dl02" style="margin-top:20px;margin-left:100px;">
				<dt style="line-height:20px;" class="WSY_left">易宝交易单号：</dt>
				<dd><span class="input">
			...
			<dl class="WSY_remind_dl02" style="margin-top:20px;margin-left:100px;">
				<dt style="line-height:20px;" class="WSY_left">易宝支付：</dt>
				<dd><span class="input">
					<?php  echo $price; ?>
				</span></dd>
			</dl>
				...
				<dl class="WSY_remind_dl02" style="margin-top:20px;margin-left:100px;">
					...
					<span class="input">
					   <?php
						...
						$query5="select refund,createtime from weixin_commonshop_refunds where isvalid=true and batchcode='".$pay_batchcode."'";
						$result5 = _mysql_query($query5) or die('Query failed: ' . mysql_error());
						while ($row5 = mysql_fetch_object($result5)) {
							$refund 	= $row5->refund;
			...
			<?php } ?>
			<?php /*if($paystatus==1 and $status<1 and ($sendstatus == 3 || $sendstatus == 5) and $check_refund_status==0)*/if(3<2){ //只有申请退款或退货的才能显示退款操作
			?>
			<dl class="WSY_remind_dl02" style="margin-top:20px;margin-left:100px;">
				...
				<span class="input">
					...
					<?php if($return_status_str==0){ ?>
						&nbsp;&nbsp;<a href="javascript:showReturn();" style="color:blue" >退款</a>
					<?php } ?>
			...
			<dl class="WSY_remind_dl02" id="div_return" style="display:none;margin-top:20px;margin-left:100px;">
				...
				<dd style="line-height:40px;">
					<span><input class="not_agent_tip" value="0" name="return_money" id="return_money" style="width:50px;height:24px;text-align:center;border:solid 1px #ccc;border-radius:2px;" type="text"><?php echo $currency_text;?>(不超过订单金额）</span>
				</dd>
				<dd class="WSY_bottonli con-button" style="float:left;margin-left:80px;"> <input value="退款"  type="button" onclick="subReturn();" /></dd>
			</dl>
			<?php  }else{  ?>
...
function subReturn(){
   ...
   document.location = "../../../yeepay/sendRefund.php?customer_id=<?php echo $customer_id_en; ?>&pay_batchcode=<?php echo $pay_batchcode; ?>&total_fee=<?php echo $price; ?>&batchcode=<?php echo $batchcode?>&out_trade_no=<?php echo $out_trade_no; ?>&refund_fee="+return_money;
}
</script>
