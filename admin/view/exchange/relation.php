        ...
        <div class="WSY_columnbox"> 
            ...
            <div class="WSY_data">
                <div class="WSY_agentsbox">
                    <?php if($arr['status'] != 4) {?>
                    <a href="index.php?m=exchange&a=get_add_relation&activity_id=<?php echo $param['activity_id'];?>" class="WSY-skin-bg form-btn form-add-btn">添加产品</a>
                    <?php }?>
                    <table width="97%" class="WSY_table" id="WSY_t1">
                        <thead class="WSY_table_header">
                            ...
                            <th width="13%" nowrap="nowrap"align="center">产品名称</th>
                            ...
                            <th width="10%" nowrap="nowrap"align="center">换购价</th>
                            <th width="8%" nowrap="nowrap"align="center">每人次数</th>
                            <th width="8%" nowrap="nowrap"align="center">每次数量</th>
                            <?php if($arr['status'] != 4) {?>
                        ...
                        <tbody class="tbody-main">
                            ...
                            <tr id="id_<?php echo $row['id'];?>">
                                ...
                                <td></span><input <?php if($arr['status'] == 4) { echo 'disabled';}?> oninput='exint1(this)' onblur="saveExchange(this,'storenum','<?php echo $row['id'];?>')" type="text" data-val="<?php echo $row['storenum'];?>" name="storenum" class="ipt" value="<?php echo $row['storenum'];?>" /></td>
                                <td><input <?php if($arr['status'] != 1) { echo 'disabled';}?> oninput='checkint(this)' onblur="saveExchange(this,'exchange_price','<?php echo $row['id'];?>')" type="text" data-val="<?php echo $row['exchange_price'];?>" name="change_price" class="ipt" value="￥<?php echo $row['exchange_price'];?>"/></td>
                                <td><input <?php if($arr['status'] != 1) { echo 'disabled';}?> oninput='exint2(this)' onblur="saveExchange(this,'num_per_person','<?php echo $row['id'];?>')" type="text" data-val="<?php echo $row['num_per_person'];?>" name="times" class="ipt" value="<?php echo $row['num_per_person'];?>"/></td>
                                <td><input <?php if($arr['status'] != 1) { echo 'disabled';}?> oninput='exint2(this)'  onblur="saveExchange(this,'num_per_time','<?php echo $row['id'];?>')" type="text" data-val="<?php echo $row['num_per_time'];?>" name="num" class="ipt" value="<?php echo $row['num_per_time'];?>"/></td>
                                <?php if($arr['status'] != 4) {?>
                                <td>
                                    <button onclick="delExchange('<?php echo $row['id'];?>')" class="table-btn WSY-skin-bg">删除</button>
                                </td>
                                <?php }?>
                            </tr>
                            <?php }?>
...
$(".WSY_page").createPage({
    ...
    current:pagenum,
    backFn:function(p){
    var url="/mshop/admin/index.php?m=exchange&a=exchange_activity_relation&activity_id="+activity_id+"&pagenum="+p;    
    location.href = url;
   }
});
...
function jumppage(){
    ...
    if((a<1) || (a>count) || isNaN(a)){
        ...
        var url="/mshop/admin/index.php?m=exchange&a=exchange_activity_relation&activity_id="+activity_id+"&pagenum="+a;    
        location.href = url;
    }
}
<!-- 分页 end -->
...
function saveExchange(obj,str,id) {
    ...
    $.ajax({
        url: '/mshop/admin/index.php?m=exchange&a=save_relation',
        dataType: 'json',
        type: 'post',
        data: {
        ...
        success: function(res){
            if( res.errcode == '1' ){
                ...
                $(ooo).after('<span id="eximg" style="position: absolute;margin-top:-23px;margin-left: 58px"><img src="/weixin/plat/Public/img/loading/s_success.png"><span></span></span>');
                setTimeout(function () { 
                    $('#eximg').remove();
                }, 1500);
            }else{
...
function delExchange(i) {
    ...
    var pid          = i;
    $.ajax({
        url: '/mshop/admin/index.php?m=exchange&a=del_relation',
        dataType: 'json',
        type: 'post',
        data: {
            activity_id:activity_id,
        ...
        success: function(res){
            if( res.errcode == '1' ){
                alert(res.errmsg);
                window.location = "/mshop/admin/index?m=exchange&a=exchange_activity_relation&activity_id="+activity_id+"&pagenum=1";
            }else{
                alert(res.errmsg);
            }
        }
