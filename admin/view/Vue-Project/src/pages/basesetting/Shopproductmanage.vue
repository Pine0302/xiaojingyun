      ...
      <div class="WSY_columnbox">
        <div class="WSY_search_q">
          <div class="WSY_search_div">
            ...
            <div class="WSY_small_btn" @click="searchFun">搜索</div>
            <!-- <div class="WSY_small_btn" @click="">导出</div> -->
            <div class="WSY_small_btn" @click="addproducts">添加产品</div>
          </div>
        </div>
        <table width="97%" class="WSY_table table2excel">
          <thead class="WSY_table_header" :class="skin">
            <tr>
              ...
              <th width="5%">产品原价</th>
              <th width="5%">产品现价</th>
              <th width="5%">可获{{storeintegralname}}</th>
              <th width="5%">操作</th>
            </tr>
          </thead>
          <tbody>
...
export default {
  ...
  methods: {
    ...
    edit: function(obj) {
      this.$router.push({ path: '/basesetting/shopedit', query: { 'product_id': obj,"integral_type": "1"  } });
    },
    ...
    addproducts() {
      this.$router.push({ path: '/activityset/Tianjiachanpin', query: { 'act_id': '-1', 'act_type': 0, "integral_type": "1" } });
    }
  },
  created: function() {
    var self = this;
...
console.log(err)
    ...
    self.$http.post(urlget, {
      ...
      .then(res => {
        ...
        self.total = parseInt(datajsonget.total);
        self.tableData = datajsonget.list;
        self.join_product = datajsonget.join_product;
        if (self.join_product == 1) {
          self.centerDialogVisible = true;
          // alert("您已选的功能无法自定义商品参数！");
        } else {
