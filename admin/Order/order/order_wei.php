			 ...
			 <tr class="CP_table_bianhao" >
				...
				if($o_paystatus==1 and $o_Pay_Method==0){
					...
					if($o_paystyle=="通联支付"){
						...
						/* 微信支付End */
					}else if($o_paystyle=="环迅快捷支付" || $o_paystyle=="环迅微信支付"){
...
$(".good_refund").click(function(){
	...
	layer.confirm('订单:'+batchcode+'</br>将退款 <b style="color:red">'+parseFloat(total_refundMoney).toFixed(2)+'</b> 元</br><b style="color:red">微信支付和威富通支付订单请先到支付详情界面进行手动退款！</b>', {
		btn: ['退款','取消']
	}, function(confirm){
...
function export_excel(num){
	...
	var _obj = {
		...
		param: {
			begintime: 					begintime,
			endtime: 					endtime,
			pay_begintime: 				pay_begintime,
			pay_endtime: 				pay_endtime,
			orgin_from: 				orgin_from,
			continued_day: 				continued_day,
			search_paystyle: 			search_paystyle,
			search_batchcode: 			search_batchcode,
			search_name: 				search_name,
			search_product_name: 		search_product_name,
			search_agent_id: 			search_agent_id,
			search_phone: 				search_phone,
			search_name_type: 			search_name_type,
			search_order_ascription: 	search_order_ascription,
			search_supply_id: 			search_supply_id,
			search_attribution_type: 	search_attribution_type,
			search_pre_delivery_type: 	search_pre_delivery_type,
			search_supply_id: 			search_supply_id,
			status: 					status
		}
	}
