                        ...
                        <tbody>
                            <tr v-for='item in tableData'>
                                ...
                                <td class="WSY_t4">
                                    ...
                                    <a title="关联产品" @click="relevance(item.act_id)">
                                        <img src="../../assets/images_V6.0/operating_icon/icon43.png"></a>
...
export default {
    created: function() {
        ...
        self.$http.post('mshop/admin/index.php?m=activity&a=activity_index&customer_id=' + self.customer_id, { system_code: '', data: { "act_type": 2, "count": self.count, "page": self.page - 1 } }
        ).then(function(res) {
    ...
    methods: {
        ...
        search: function() {
            ...
            that.$http.post('mshop/admin/index.php?m=activity&a=activity_index&customer_id=' + that.customer_id, { 'system_code': '', 'data': that.form }
            ).then(function(res) {
                // that.value = ''
                for (var i = 0; i < res.data.data.total && i < res.data.data.list_num; i++) {
        ...
        add: function() {
            this.$router.push({ path: '/activityset/Tjduihuanhuodong', query: { 'act_type': 2 } });
        },
        edit: function(obj) {
            this.$router.push({ path: '/activityset/Tjduihuanhuodong', query: { 'act_id': obj, 'act_type': 2 } });
        },
        relevance: function(act_id) {
             var that = this;
            that.$http.post('mshop/admin/index.php?m=activity&a=get_exchange_status&customer_id=' + that.customer_id, { system_code: '', data: { 'activity_id': act_id, 'act_type': 2 } }
            ).then(function(res) {
               
                 that.$router.push({ path: '/activityset/Tjduihuanhuodong', query: { 'act_id': act_id, 'act_type': 2 ,'active':2,'status':res.data.data.status} });
        ...
        delect: function(act_id) {
            var that = this;
            if (confirm("确认是否删除？")) {
                that.$http.post('mshop/admin/index.php?m=activity&a=save_activity&customer_id=' + that.customer_id, { system_code: '', data: { 'op': 'del', 'act_id': act_id, 'act_type': 2 } }
                ).then(function(res) {
