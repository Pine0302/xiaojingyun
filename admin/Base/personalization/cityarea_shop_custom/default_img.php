...
if(!empty($_GET["default_img"])){
	...
	if($customer_id>0){
		$query2="select default_img from weixin_cityarea_diy_template where isvalid=true and id=".$temid." and customer_id=".$customer_id;
...
if(!empty($_GET["op"])){
	...
	if($op=="del"){  
		$temid = $configutil->splash_new($_GET["temid"]);
		$query="update weixin_cityarea_diy_template set default_img='' where id=".$temid;	
		_mysql_query($query);
		$default_img="";
	}  
}
