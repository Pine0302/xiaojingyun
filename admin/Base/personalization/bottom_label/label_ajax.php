<?php
	...
	switch ($op) {
		case 'up':
			$query = "select id,sort from bottom_label_setting_t where isvalid=true and sort>".$key_sort." and customer_id=".$customer_id." order by sort limit 1";
			...
			$query1 = "update bottom_label_setting_t set sort=".$switch_sort." where id=".(int)$keyid;
			$query2 = "update bottom_label_setting_t set sort=".$key_sort." where id=".(int)$switch_id;
		...
		case 'down':
			$query = "select id,sort from bottom_label_setting_t where isvalid=true and sort<".$key_sort." and customer_id=".$customer_id." order by sort desc limit 1";
			$res = _mysql_query($query) or $error=mysql_error();
			while ($row = mysql_fetch_object($res)) {
			    $switch_id = $row->id;
			    $switch_sort = $row->sort;
