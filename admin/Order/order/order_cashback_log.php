...
if($batchcode_status){
	...
	$query_r = "select id from cashback where batchcode='{$batchcode}' and customer_id='{$customer_id}'"; 
	$result_r = _mysql_query($query_r) or die('L18: '.mysql_error());
	$result = _mysql_query($query) or die('L18: '.mysql_error());
	$obj = mysql_fetch_assoc($result_r);
	while($row = mysql_fetch_object($result)){
		$cashback_id 	 = $row->id;
...
<title>赠送说明 订单号:<?php echo $batchcode;?></title>
</head>
<body> 
	<!--内容框架-->
			...
			<div class="WSY_remind_main">
				<form class="search" id="search_form" method="post" action="cash.php?customer_id=AzBVZ1UzVGk=">
					...
					<div class="WSY_list" style="margin-top: 18px;">
						<li style="margin-left: 20px;">
							<a style="color: #646464;font-size: 14px;">总奖励金额：<span style="font-size:16px;color:red;margin-right: 20px;"><?php echo $cashback; ?>元 </a>
					...
					<div class="WSY_list" style="float: right;margin-top: -45px;margin-right: 8px;">
						<ul class="WSY_righticon" id="butt" style="margin-right: 44px;">
						   <li><a style="background:red;cursor: pointer;" onclick="delCashback();">删除此笔赠送</a></li>
						</ul>
					</div>
					<?php }else{?>
					<div class="WSY_list" style="float: right;margin-top: -45px;margin-right: 8px;">
						<ul class="WSY_righticon" style="margin-right: 44px;">
				...
				<table width="97%" class="WSY_table" id="WSY_t1">
					<thead class="WSY_table_header">
						<th width="25%" nowrap="nowrap">推广员信息</th>
						<th width="25%" nowrap="nowrap">赠送状态</th>
						<th width="25%" nowrap="nowrap">奖励金额</th>
						<th width="25%" nowrap="nowrap">领取时间</th>
					</thead>
					<tbody>
					<?php 
					if( $cashback_status ){
...
function delCashback(){
	...
	layer.confirm('订单：'+batchcode+'，您确定要删除此笔赠送吗？删除后，用户将无法继续领取，但已领取部分不能退回！', {
		btn: ['确认','取消'] 
	}, function(confirm){
		...
		$.ajax({
			...
			type:"POST",
			data:{'batchcode':batchcode,'op':"del_s",'type':type},
			dataType:"json",
